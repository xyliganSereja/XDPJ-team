...
struct cv::ocl::internal::ProgramEntry stereobm_oclsrc = {
    moduleName, "stereobm",
    ...
    "void calcDisp(__local short * cost, __global short * disp, int "
    "uniquenessRatio,\n"
    "__local int * bestDisp, __local int * bestCost, int d, int x, int y, int "
    "cols, int rows)\n"
    "{\n"
    ...
    "bool notUniq = ( (c <= thresh) && (d < (best_disp - 1) || d > (best_disp "
    "+ 1) ) );\n"
    "if (notUniq)\n"
    "*bestCost = FILTERED;\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "if( *bestCost != FILTERED && x < cols - WSZ2 - MIN_DISP && y < rows - "
    "WSZ2 && d == best_disp)\n"
    "{\n"
    "int d_aprox = 0;\n"
    "int yp =0, yn = 0;\n"
    ...
    "}\n"
    "disp[0] = (short)(((best_disp + MIN_DISP)*256 + (d_aprox != 0 ? (yp - yn) "
    "* 256 / d_aprox : 0) + 15) >> 4);\n"
    "}\n"
    "}\n"
    "short calcCostBorder(__global const uchar * leftptr, __global const uchar "
    "* rightptr, int x, int y, int nthread,\n"
    "short * costbuf, int *h, int cols, int d, short cost)\n"
    "{\n"
    "int head = (*h) % WSZ;\n"
    "__global const uchar * left, * right;\n"
    "int idx = mad24(y + WSZ2 * (2 * nthread - 1), cols, x + WSZ2 * (1 - 2 * "
    "nthread));\n"
    "left = leftptr + idx;\n"
    ...
    "return cost;\n"
    "}\n"
    "short calcCostInside(__global const uchar * leftptr, __global const uchar "
    "* rightptr, int x, int y,\n"
    ...
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "int dispIdx = mad24(gy, disp_step, mad24((int)sizeof(short), gx, "
    "disp_offset));\n"
    "disp = (__global short *)(dispptr + dispIdx);\n"
    "calcDisp(cost, disp, uniquenessRatio, best_disp + 1, best_cost + 1, "
    "disp_idx, x, y, cols, rows);\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "lx = 1 - nthread;\n"
    "ly = nthread;\n"
    ...
    "best_disp[nthread] = -1;\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "costIdx = mad24(2 * BLOCK_SIZE_Y, disp_idx, (BLOCK_SIZE_Y - 1 - ly + "
    "lx));\n"
    "if (0 > costIdx)\n"
    ...
    "calcCostBorder(leftptr, rightptr, x, y, nthread, costbuf, &head, cols, "
    "disp_idx, cost[2*nthread-1]) :\n"
    "calcCostInside(leftptr, rightptr, x, y, cols, disp_idx, cost[0], cost[1], "
    "cost[-1]);\n"
    "}\n"
    "cost[0] = tempcost;\n"
    ...
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "dispIdx = mad24(gy + ly, disp_step, mad24((int)sizeof(short), (gx + lx), "
    "disp_offset));\n"
    "disp = (__global short *)(dispptr + dispIdx);\n"
    "calcDisp(cost, disp, uniquenessRatio, best_disp + nthread, best_cost + "
    "nthread, disp_idx, x, y, cols, rows);\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "if (lx + nthread - 1 == ly)\n"
    "{\n"
    ...
    "}\n"
    "#endif\n"
    "__kernel void prefilter_norm(__global unsigned char *input, __global "
    ...
    "if(x < cols && y < rows)\n"
    "{\n"
    "int cov1 =                                   input[   max(y-1, 0)   * "
    "cols + x] * 1 +\n"
    "input[y * cols + max(x-1,0)] * 1 + input[      y          * cols + x] * 4 "
    "+ input[y * cols + min(x+1, cols-1)] * 1 +\n"
    ...
    "}\n"
    "}\n"
    "__kernel void prefilter_xsobel(__global unsigned char *input, __global "
    "unsigned char *output,\n"
    ...
    "{\n"
    "int cov = input[ ((y > 0) ? y-1 : y+1)  * cols + (x-1)] * (-1) + input[ "
    "((y > 0) ? y-1 : y+1)  * cols + ((x<cols-1) ? x+1 : x-1)] * (1) +\n"
    "input[              (y)       * cols + (x-1)] * (-2) + input[        (y)  "
    "           * cols + ((x<cols-1) ? x+1 : x-1)] * (2) +\n"
    "input[((y<rows-1)?(y+1):(y-1))* cols + (x-1)] * (-1) + "
    "input[((y<rows-1)?(y+1):(y-1))* cols + ((x<cols-1) ? x+1 : x-1)] * (1);\n"
