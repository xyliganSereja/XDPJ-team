...
struct cv::ocl::internal::ProgramEntry accumulate_oclsrc = {
    moduleName, "accumulate",
    ...
    "#define noconvert\n"
    "__kernel void accumulate(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "#ifdef ACCUMULATE_PRODUCT\n"
    "__global const uchar * src2ptr, int src2_step, int src2_offset,\n"
    "#endif\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols\n"
    ...
    "if (y < dst_rows)\n"
    "{\n"
    "__global const srcT1 * src = (__global const srcT1 *)(srcptr + "
    "src_index);\n"
    "#ifdef ACCUMULATE_PRODUCT\n"
    "__global const srcT1 * src2 = (__global const srcT1 *)(src2ptr + "
    "src2_index);\n"
    "#endif\n"
...
struct cv::ocl::internal::ProgramEntry bilateral_oclsrc = {
    ...
    "#endif\n"
    "__kernel void bilateral(__global const uchar * src, int src_step, int "
    "src_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "{\n"
    "int src_index = mad24(y + radius, src_step, mad24(x + radius, TSIZE, "
    "src_offset));\n"
    ...
    "#endif\n"
    "float w = space_weight[k] * native_exp((float)(diff * diff * "
    "gauss_color_coeff));\n"
    ...
    "#ifdef INTEL_DEVICE\n"
    "#if cn == 1\n"
    "__kernel void bilateral_float4(__global const uchar * src, int src_step, "
    "int src_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__constant float * space_weight, __constant int * space_ofs)\n"
    ...
    "{\n"
    "int src_index = ((y + radius) * src_step) + x * 4  + (radius + "
    "src_offset);\n"
    ...
    "float4 w = space_weight[k] * native_exp((val - val0) * (val - val0) * "
    "gauss_color_coeff);\n"
    "sum += val * w;\n"
    "wsum += w;\n"
    "}\n"
...
struct cv::ocl::internal::ProgramEntry blend_linear_oclsrc = {
    moduleName, "blend_linear",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "#endif\n"
    "#define noconvert\n"
    "__kernel void blendLinear(__global const uchar * src1ptr, int src1_step, "
    "int src1_offset,\n"
    "__global const uchar * src2ptr, int src2_step, int src2_offset,\n"
    ...
    "{\n"
    "int src1_index = mad24(y, src1_step, src1_offset + x * cn * "
    "(int)sizeof(T));\n"
    "int src2_index = mad24(y, src2_step, src2_offset + x * cn * "
    "(int)sizeof(T));\n"
...
struct cv::ocl::internal::ProgramEntry boxFilter_oclsrc = {
    moduleName, "boxFilter",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "#define EXTRAPOLATE(x, y, minX, minY, maxX, maxY) EXTRAPOLATE_(x, y, "
    ...
    "int x1, y1, x2, y2;\n"
    "};\n"
    "inline WT readSrcPixel(int2 pos, __global const uchar * srcptr, int "
    "src_step, const struct RectCoords srcCoords)\n"
    "{\n"
    "#ifdef BORDER_ISOLATED\n"
    "if (pos.x >= srcCoords.x1 && pos.y >= srcCoords.y1 && pos.x < "
    "srcCoords.x2 && pos.y < srcCoords.y2)\n"
    "#else\n"
    "if (pos.x >= 0 && pos.y >= 0 && pos.x < srcCoords.x2 && pos.y < "
    "srcCoords.y2)\n"
    "#endif\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void boxFilter(__global const uchar * srcptr, int src_step, int "
    "srcOffsetX, int srcOffsetY, int srcEndX, int srcEndY,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols\n"
    "#ifdef NORMALIZE\n"
    ", float alpha\n"
    "#endif\n"
    ")\n"
    "{\n"
    "const struct RectCoords srcCoords = { srcOffsetX, srcOffsetY, srcEndX, "
    "srcEndY };\n"
    "int x = get_local_id(0) + (LOCAL_SIZE_X - (KERNEL_SIZE_X - 1)) * "
    "get_group_id(0) - ANCHOR_X;\n"
    ...
    "{\n"
    "if (local_id >= ANCHOR_X && local_id < LOCAL_SIZE_X - (KERNEL_SIZE_X - 1 "
    "- ANCHOR_X) &&\n"
    "x >= 0 && x < cols)\n"
    "{\n"
    "WT total_sum = (WT)(0);\n"
...
struct cv::ocl::internal::ProgramEntry boxFilter3x3_oclsrc = {
    moduleName, "boxFilter3x3",
    "__kernel void boxFilter3x3_8UC1_cols16_rows2(__global const uint* src, "
    "int src_step,\n"
    ...
    "line[3] = (y == (rows - 2)) ? (uint4)0 : vload4(0, src + src_index + 3 * "
    "(src_step / 4));\n"
    "#elif defined BORDER_REFLECT_101\n"
    "line[0] = (y == 0) ? line[2] : vload4(0, src + src_index);\n"
    "line[3] = (y == (rows - 2)) ? line[1] : vload4(0, src + src_index + 3 * "
    "(src_step / 4));\n"
    "#elif defined (BORDER_REPLICATE) || defined(BORDER_REFLECT)\n"
    "line[0] = (y == 0) ? line[1] : vload4(0, src + src_index);\n"
    "line[3] = (y == (rows - 2)) ? line[2] : vload4(0, src + src_index + 3 * "
    "(src_step / 4));\n"
    "#endif\n"
    "ushort16 sum, mid;\n"
    ...
    "d = (block_x == 0) ? 0 : convert_ushort(src_p[src_index + src_step - "
    "1]);\n"
    "f = line_end ? 0 : convert_ushort(src_p[src_index + src_step + 16]);\n"
    "g = (block_x == 0) ? 0 : convert_ushort(src_p[src_index + 2 * src_step - "
    "1]);\n"
    "i = line_end ? 0 : convert_ushort(src_p[src_index + 2 * src_step + 16]);\n"
    "j = (block_x == 0 || y == (rows - 2)) ? 0 : "
    "convert_ushort(src_p[src_index + 3 * src_step - 1]);\n"
    "l = (line_end || y == (rows - 2))? 0 : convert_ushort(src_p[src_index + 3 "
    "* src_step + 16]);\n"
    "#elif defined BORDER_REFLECT_101\n"
    "int offset;\n"
    "offset = (y == 0) ? (2 * src_step) : 0;\n"
    "a = (block_x == 0) ? convert_ushort(src_p[src_index + offset + 1]) : "
    "convert_ushort(src_p[src_index + offset - 1]);\n"
    "c = line_end ? convert_ushort(src_p[src_index + offset + 14]) : "
    "convert_ushort(src_p[src_index + offset + 16]);\n"
    "d = (block_x == 0) ? convert_ushort(src_p[src_index + src_step + 1]) : "
    "convert_ushort(src_p[src_index + src_step - 1]);\n"
    "f = line_end ? convert_ushort(src_p[src_index + src_step + 14]) : "
    "convert_ushort(src_p[src_index + src_step + 16]);\n"
    "g = (block_x == 0) ? convert_ushort(src_p[src_index + 2 * src_step + 1]) "
    ": convert_ushort(src_p[src_index + 2 * src_step - 1]);\n"
    "i = line_end ? convert_ushort(src_p[src_index + 2 * src_step + 14]) : "
    "convert_ushort(src_p[src_index + 2 * src_step + 16]);\n"
    "offset = (y == (rows - 2)) ? (1 * src_step) : (3 * src_step);\n"
    "j = (block_x == 0) ? convert_ushort(src_p[src_index + offset + 1]) : "
    "convert_ushort(src_p[src_index + offset - 1]);\n"
    "l = line_end ? convert_ushort(src_p[src_index + offset + 14]) : "
    "convert_ushort(src_p[src_index + offset + 16]);\n"
    "#elif defined (BORDER_REPLICATE) || defined(BORDER_REFLECT)\n"
    "int offset;\n"
    "offset = (y == 0) ? (1 * src_step) : 0;\n"
    "a = (block_x == 0) ? convert_ushort(src_p[src_index + offset]) : "
    "convert_ushort(src_p[src_index + offset - 1]);\n"
    "c = line_end ? convert_ushort(src_p[src_index + offset + 15]) : "
    "convert_ushort(src_p[src_index + offset + 16]);\n"
    "d = (block_x == 0) ? convert_ushort(src_p[src_index + src_step]) : "
    "convert_ushort(src_p[src_index + src_step - 1]);\n"
    "f = line_end ? convert_ushort(src_p[src_index + src_step + 15]) : "
    "convert_ushort(src_p[src_index + src_step + 16]);\n"
    "g = (block_x == 0) ? convert_ushort(src_p[src_index + 2 * src_step]) : "
    "convert_ushort(src_p[src_index + 2 * src_step - 1]);\n"
    "i = line_end ? convert_ushort(src_p[src_index + 2 * src_step + 15]) : "
    "convert_ushort(src_p[src_index + 2 * src_step + 16]);\n"
    "offset = (y == (rows - 2)) ? (2 * src_step) : (3 * src_step);\n"
    "j = (block_x == 0) ? convert_ushort(src_p[src_index + offset]) : "
    "convert_ushort(src_p[src_index + offset - 1]);\n"
    "l = line_end ? convert_ushort(src_p[src_index + offset + 15]) : "
    "convert_ushort(src_p[src_index + offset + 16]);\n"
    "#endif\n"
    "mid = (ushort16)(d, e.s0123, e.s456789ab, e.scde) + e + "
    "(ushort16)(e.s123, e.s4567, e.s89abcdef, f) +\n"
    "(ushort16)(g, h.s0123, h.s456789ab, h.scde) + h + (ushort16)(h.s123, "
    "h.s4567, h.s89abcdef, i);\n"
    "sum = (ushort16)(a, b.s0123, b.s456789ab, b.scde) + b + "
    "(ushort16)(b.s123, b.s4567, b.s89abcdef, c) +\n"
    "mid;\n"
    "#ifdef NORMALIZE\n"
    "line_out[0] = as_uint4(convert_uchar16_sat_rte((convert_float16(sum) * "
    "alpha)));\n"
    "#else\n"
    "line_out[0] = as_uint4(convert_uchar16_sat_rte(sum));\n"
    "#endif\n"
    "sum = mid +\n"
    "(ushort16)(j, k.s0123, k.s456789ab, k.scde) + k + (ushort16)(k.s123, "
    "k.s4567, k.s89abcdef, l);\n"
    "#ifdef NORMALIZE\n"
    "line_out[1] = as_uint4(convert_uchar16_sat_rte((convert_float16(sum) * "
    "alpha)));\n"
    "#else\n"
...
struct cv::ocl::internal::ProgramEntry calc_back_project_oclsrc = {
    moduleName, "calc_back_project",
    "#define OUT_OF_RANGE -1\n"
    "#define ROUNDING_EPS 0.000001f\n"
    "#if histdims == 1\n"
    "__kernel void calcLUT(__global const uchar * histptr, int hist_step, int "
    "hist_offset, int hist_bins,\n"
    ...
    "__global const float * hist = (__global const float *)(histptr + "
    "hist_index);\n"
    "lut[x] = (int)convert_uchar_sat_rte(hist[0] * scale);\n"
    "}\n"
    "}\n"
    "}\n"
    "__kernel void LUT(__global const uchar * src, int src_step, int "
    "src_offset,\n"
    "__constant int * lut,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols)\n"
    ...
    "}\n"
    "}\n"
    "__kernel void LUT(__global const uchar * src1, int src1_step, int "
    "src1_offset,\n"
    ...
    "mad24(hist_step, bin1, hist_offset + bin2 * (int)sizeof(float))) * "
    "scale);\n"
    "}\n"
    "}\n"
    "#else\n"
...
struct cv::ocl::internal::ProgramEntry canny_oclsrc = {
    moduleName, "canny",
    ...
    "#else\n"
    "#define loadpix(addr) convert_floatN(vload3(0, (__global const TYPE "
    "*)(addr)))\n"
    "#endif\n"
    ...
    "{\n"
    "float3 res;\n"
    "floatN dx = fma((floatN)2, smem[idx + GRP_SIZEX + 6] - smem[idx + "
    "GRP_SIZEX + 4],\n"
    "smem[idx + 2] - smem[idx] + smem[idx + 2 * GRP_SIZEX + 10] - smem[idx + 2 "
    "* GRP_SIZEX + 8]);\n"
    "floatN dy = fma((floatN)2, smem[idx + 1] - smem[idx + 2 * GRP_SIZEX + "
    "9],\n"
    "smem[idx + 2] - smem[idx + 2 * GRP_SIZEX + 10] + smem[idx] - smem[idx + 2 "
    "* GRP_SIZEX + 8]);\n"
    "#ifdef L2GRAD\n"
    ...
    "return res;\n"
    "}\n"
    "__kernel void stage1_with_sobel(__global const uchar *src, int src_step, "
    "int src_offset, int rows, int cols,\n"
    ...
    "for (int j = i;  j < (GRP_SIZEX + 4) * (GRP_SIZEY + 4); j += GRP_SIZEX * "
    "GRP_SIZEY)\n"
    "{\n"
    "int x = clamp(start_x - 2 + (j % (GRP_SIZEX + 4)), 0, cols - 1);\n"
    "int y = clamp(start_y - 2 + (j / (GRP_SIZEX + 4)), 0, rows - 1);\n"
    "smem[j] = loadpix(src + mad24(y, src_step, mad24(x, cn * "
    "(int)sizeof(TYPE), src_offset)));\n"
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    ...
    "mag[i + grp_sizey * (GRP_SIZEX + 2)] = (sobel(i + grp_sizey * (GRP_SIZEX "
    "+ 4), smem)).z;\n"
    "}\n"
    "if (i < GRP_SIZEY + 2)\n"
    "{\n"
    ...
    "mag[i * (GRP_SIZEX + 2) + grp_sizex] = (sobel(i * (GRP_SIZEX + 4) + "
    "grp_sizex, smem)).z;\n"
    "}\n"
    ...
    "int dir = a * b + 2 * dir3;\n"
    "float prev_mag = mag[(lidy + prev[dir][0]) * (GRP_SIZEX + 2) + lidx + "
    "prev[dir][1]];\n"
    "float next_mag = mag[(lidy + next[dir][0]) * (GRP_SIZEX + 2) + lidx + "
    "next[dir][1]] + (dir & 1);\n"
    ...
    "}\n"
    "}\n"
    "storepix(value, map + mad24(gidy, map_step, mad24(gidx, (int)sizeof(int), "
    "map_offset)));\n"
    "}\n"
    ...
    "{ 1, -1 }\n"
    "};\n"
    "__kernel void stage1_without_sobel(__global const uchar *dxptr, int "
    "dx_step, int dx_offset,\n"
    ...
    "#pragma unroll\n"
    "for (int i = lidx + lidy * GRP_SIZEX; i < (GRP_SIZEX + 2) * (GRP_SIZEY + "
    "2); i += GRP_SIZEX * GRP_SIZEY)\n"
    "{\n"
    "int x = clamp(start_x - 1 + i % (GRP_SIZEX + 2), 0, cols - 1);\n"
    "int y = clamp(start_y - 1 + i / (GRP_SIZEX + 2), 0, rows - 1);\n"
    "int dx_index = mad24(y, dx_step, mad24(x, cn * (int)sizeof(short), "
    "dx_offset));\n"
    "int dy_index = mad24(y, dy_step, mad24(x, cn * (int)sizeof(short), "
    "dy_offset));\n"
    ...
    "int dir3 = (a * b) & (((x ^ y) & 0x80000000) >> 31);\n"
    "int dir = a * b + 2 * dir3;\n"
    "int prev_mag = mag[(lidy + prev[dir][0]) * (GRP_SIZEX + 2) + lidx + "
    "prev[dir][1]];\n"
    "int next_mag = mag[(lidy + next[dir][0]) * (GRP_SIZEX + 2) + lidx + "
    "next[dir][1]] + (dir & 1);\n"
    "if (mag0 > prev_mag && mag0 >= next_mag)\n"
    "{\n"
    "value = (mag0 > high_thr) ? 2 : 0;\n"
    ...
    "{ -1, 0, 1, -1, 1, -1, 0, 1 }\n"
    "};\n"
    "__kernel void stage2_hysteresis(__global uchar *map_ptr, int map_step, "
    "int map_offset, int rows, int cols)\n"
    "{\n"
    "map_ptr += map_offset;\n"
    "int x = get_global_id(0);\n"
    ...
    "__kernel void getEdges(__global const uchar *mapptr, int map_step, int "
    "map_offset, int rows, int cols,\n"
    ...
    "if (x < cols)\n"
    "{\n"
    "int map_index = mad24(map_step, y, mad24(x, (int)sizeof(int), "
    "map_offset));\n"
...
struct cv::ocl::internal::ProgramEntry clahe_oclsrc = {
    moduleName, "clahe",
    ...
    "{\n"
    "__global const uchar* srcPtr = src + mad24(ty * tileSize.y + i, srcStep, "
    "tx * tileSize.x + src_offset);\n"
    ...
    "}\n"
    "__kernel void transform(__global __const uchar * src, const int srcStep, "
    ...
    "float res = 0;\n"
    "res += lut[mad24(ty1 * tilesX + tx1, lutStep, srcVal + lut_offset)] * "
    "((1.0f - xa) * (1.0f - ya));\n"
    "res += lut[mad24(ty1 * tilesX + tx2, lutStep, srcVal + lut_offset)] * "
    "((xa) * (1.0f - ya));\n"
    "res += lut[mad24(ty2 * tilesX + tx1, lutStep, srcVal + lut_offset)] * "
    "((1.0f - xa) * (ya));\n"
    "res += lut[mad24(ty2 * tilesX + tx2, lutStep, srcVal + lut_offset)] * "
    "((xa) * (ya));\n"
    "uint ires = (uint)convert_int_rte(res);\n"
    "dst[mad24(y, dstStep, x + dst_offset)] = convert_uchar(clamp(ires, "
    "(uint)0, (uint)255));\n"
    "}\n",
    "7399905b8fbf557bd2d5336b25327a7c", NULL};
struct cv::ocl::internal::ProgramEntry color_hsv_oclsrc = {
    moduleName, "color_hsv",
    "#if depth == 0\n"
    ...
    "{ 2, 1, 0 } };\n"
    "#ifdef DEPTH_0\n"
    "__kernel void RGB2HSV(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols,\n"
    ...
    "h = (vr & (g - b)) +\n"
    "(~vr & ((vg & mad24(diff, 2, b - r)) + ((~vg) & mad24(4, diff, r - "
    "g))));\n"
    ...
    "}\n"
    "}\n"
    "__kernel void HSV2RGB(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "__kernel void RGB2HSV(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    ...
    "if (y < rows)\n"
    "{\n"
    "__global const float * src = (__global const float *)(srcptr + "
    "src_index);\n"
    ...
    "}\n"
    "}\n"
    "__kernel void HSV2RGB(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "#endif\n"
    "#ifdef DEPTH_0\n"
    "__kernel void RGB2HLS(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "{\n"
    "uchar4 src_pix = vload4(0, src + src_index);\n"
    "float b = src_pix.B_COMP*(1/255.f), g = src_pix.G_COMP*(1/255.f), r = "
    "src_pix.R_COMP*(1/255.f);\n"
    "float h = 0.f, s = 0.f, l;\n"
    ...
    "}\n"
    "}\n"
    "__kernel void HLS2RGB(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "{\n"
    "uchar4 src_pix = vload4(0, src + src_index);\n"
    "float h = src_pix.x, l = src_pix.y*(1.f/255.f), s = "
    "src_pix.z*(1.f/255.f);\n"
    "float b, g, r;\n"
    "if (s != 0)\n"
    "{\n"
    ...
    "}\n"
    "#elif defined DEPTH_5\n"
    "__kernel void RGB2HLS(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void HLS2RGB(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry color_lab_oclsrc = {
    moduleName, "color_lab",
    "#if depth == 0\n"
    "#define DATA_TYPE uchar\n"
    "#define MAX_NUM  255\n"
    "#define HALF_MAX_NUM 128\n"
    ...
    "__kernel void RGB2XYZ(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    ...
    "#else\n"
    "int x = CV_DESCALE(mad24(r, coeffs[0], mad24(g, coeffs[1], b * "
    "coeffs[2])), xyz_shift);\n"
    "int y = CV_DESCALE(mad24(r, coeffs[3], mad24(g, coeffs[4], b * "
    "coeffs[5])), xyz_shift);\n"
    "int z = CV_DESCALE(mad24(r, coeffs[6], mad24(g, coeffs[7], b * "
    "coeffs[8])), xyz_shift);\n"
    "#endif\n"
    "dst[0] = SAT_CAST(x);\n"
    "dst[1] = SAT_CAST(y);\n"
    ...
    "}\n"
    "}\n"
    "__kernel void XYZ2RGB(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols, __constant COEFF_TYPE * coeffs)\n"
    "{\n"
    ...
    "#else\n"
    "int b = CV_DESCALE(mad24(x, coeffs[0], mad24(y, coeffs[1], z * "
    "coeffs[2])), xyz_shift);\n"
    "int g = CV_DESCALE(mad24(x, coeffs[3], mad24(y, coeffs[4], z * "
    "coeffs[5])), xyz_shift);\n"
    "int r = CV_DESCALE(mad24(x, coeffs[6], mad24(y, coeffs[7], z * "
    "coeffs[8])), xyz_shift);\n"
    "#endif\n"
    ...
    "inline float splineInterpolate(float x, __global const float * tab, int "
    "n)\n"
    "{\n"
    ...
    "}\n"
    "#ifdef DEPTH_0\n"
    "__kernel void BGR2Lab(__global const uchar * src, int src_step, int "
    "src_offset,\n"
    ...
    "int R = gammaTab[src_pix.x], G = gammaTab[src_pix.y], B = "
    "gammaTab[src_pix.z];\n"
    "int fX = LabCbrtTab_b[CV_DESCALE(mad24(R, C0, mad24(G, C1, B*C2)), "
    "lab_shift)];\n"
    "int fY = LabCbrtTab_b[CV_DESCALE(mad24(R, C3, mad24(G, C4, B*C5)), "
    "lab_shift)];\n"
    "int fZ = LabCbrtTab_b[CV_DESCALE(mad24(R, C6, mad24(G, C7, B*C8)), "
    "lab_shift)];\n"
    "int L = CV_DESCALE( Lscale*fY + Lshift, lab_shift2 );\n"
    "int a = CV_DESCALE( mad24(500, fX - fY, 128*(1 << lab_shift2)), "
    "lab_shift2 );\n"
    "int b = CV_DESCALE( mad24(200, fY - fZ, 128*(1 << lab_shift2)), "
    "lab_shift2 );\n"
    ...
    "}\n"
    "#elif defined DEPTH_5\n"
    "__kernel void BGR2Lab(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols,\n"
    "#ifdef SRGB\n"
    ...
    "}\n"
    "#ifdef DEPTH_0\n"
    "__kernel void Lab2BGR(__global const uchar * src, int src_step, int "
    "src_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int rows, int cols,\n"
    "#ifdef SRGB\n"
    "__global const float * gammaTab,\n"
    ...
    "}\n"
    "#elif defined DEPTH_5\n"
    "__kernel void Lab2BGR(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols,\n"
    "#ifdef SRGB\n"
    ...
    "#ifdef DEPTH_5\n"
    "__kernel void BGR2Luv(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols,\n"
    "#ifdef SRGB\n"
    "__global const float * gammaTab,\n"
    "#endif\n"
    "__global const float * LabCbrtTab, __constant float * coeffs, float _un, "
    "float _vn)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y = get_global_id(1) * PIX_PER_WI_Y;\n"
    ...
    "float Y = fma(R, coeffs[3], fma(G, coeffs[4], B*coeffs[5]));\n"
    "float Z = fma(R, coeffs[6], fma(G, coeffs[7], B*coeffs[8]));\n"
    "float L = splineInterpolate(Y*LabCbrtTabScale, LabCbrtTab, "
    "LAB_CBRT_TAB_SIZE);\n"
    "L = fma(116.f, L, -16.f);\n"
    ...
    "__kernel void BGR2Luv(__global const uchar * src, int src_step, int "
    "src_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int rows, int cols,\n"
    "#ifdef SRGB\n"
    "__global const float * gammaTab,\n"
    ...
    "#endif\n"
    "#ifdef DEPTH_5\n"
    "__kernel void Luv2BGR(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols,\n"
    "#ifdef SRGB\n"
    ...
    "}\n"
    "#elif defined DEPTH_0\n"
    "__kernel void Luv2BGR(__global const uchar * src, int src_step, int "
    "src_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int rows, int cols,\n"
    "#ifdef SRGB\n"
    "__global const float * gammaTab,\n"
...
struct cv::ocl::internal::ProgramEntry color_rgb_oclsrc = {
    moduleName, "color_rgb",
    "#if depth == 0\n"
    "#define DATA_TYPE uchar\n"
    "#define MAX_NUM  255\n"
    "#define HALF_MAX_NUM 128\n"
    ...
    "#define DATA_TYPE_4 CAT(DATA_TYPE, 4)\n"
    "#define DATA_TYPE_3 CAT(DATA_TYPE, 3)\n"
    "__kernel void RGB2Gray(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "#ifdef DEPTH_5\n"
    "dst[0] = fma(src_pix.B_COMP, B2YF, fma(src_pix.G_COMP, G2YF, "
    "src_pix.R_COMP * R2YF));\n"
    "#else\n"
    "dst[0] = (DATA_TYPE)CV_DESCALE(mad24(src_pix.B_COMP, BY15, "
    "mad24(src_pix.G_COMP, GY15, mul24(src_pix.R_COMP, RY15))), gray_shift);\n"
    "#endif\n"
    "++y;\n"
    "src_index += src_step;\n"
    "dst_index += dst_step;\n"
    ...
    "}\n"
    "}\n"
    "__kernel void Gray2RGB(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void RGB(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void RGB5x52RGB(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void RGB2RGB5x5(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "uchar4 src_pix = vload4(0, src + src_index);\n"
    "#if greenbits == 6\n"
    "*((__global ushort*)(dst + dst_index)) = (ushort)((src_pix.B_COMP >> "
    "3)|((src_pix.G_COMP&~3) << 3)|((src_pix.R_COMP&~7) << 8));\n"
    "#elif scn == 3\n"
    "*((__global ushort*)(dst + dst_index)) = (ushort)((src_pix.B_COMP >> "
    "3)|((src_pix.G_COMP&~7) << 2)|((src_pix.R_COMP&~7) << 7));\n"
    "#else\n"
    "*((__global ushort*)(dst + dst_index)) = (ushort)((src_pix.B_COMP >> "
    "3)|((src_pix.G_COMP&~7) << 2)|\n"
    ...
    "}\n"
    "}\n"
    "__kernel void BGR5x52Gray(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "#if greenbits == 6\n"
    "dst[dst_index] = (uchar)CV_DESCALE(mad24((t << 3) & 0xf8, BY15, mad24((t "
    ">> 3) & 0xfc, GY15, ((t >> 8) & 0xf8) * RY15)), gray_shift);\n"
    "#else\n"
    "dst[dst_index] = (uchar)CV_DESCALE(mad24((t << 3) & 0xf8, BY15, mad24((t "
    ">> 2) & 0xf8, GY15, ((t >> 7) & 0xf8) * RY15)), gray_shift);\n"
    "#endif\n"
    "++y;\n"
    "dst_index += dst_step;\n"
    "src_index += src_step;\n"
    ...
    "}\n"
    "}\n"
    "__kernel void Gray2BGR5x5(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "int t = src[src_index];\n"
    "#if greenbits == 6\n"
    "*((__global ushort*)(dst + dst_index)) = (ushort)((t >> 3) | ((t & ~3) << "
    "3) | ((t & ~7) << 8));\n"
    "#else\n"
    "t >>= 3;\n"
    ...
    "}\n"
    "#ifdef DEPTH_0\n"
    "__kernel void RGBA2mRGBA(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void mRGBA2RGBA(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry color_yuv_oclsrc = {
    moduleName, "color_yuv",
    "#if depth == 0\n"
    "#define DATA_TYPE uchar\n"
    "#define MAX_NUM  255\n"
    "#define HALF_MAX_NUM 128\n"
    ...
    "__kernel void RGB2YUV(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    ...
    "const int Y = CV_DESCALE(mad24(b, coeffs[0], mad24(g, coeffs[1], mul24(r, "
    "coeffs[2]))), yuv_shift);\n"
    ...
    "__kernel void YUV2RGB(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dt_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "float g = fma(V - HALF_MAX_NUM, coeffs[2], fma(U - HALF_MAX_NUM, "
    "coeffs[1], Y));\n"
    "float b = fma(U - HALF_MAX_NUM, coeffs[0], Y);\n"
    "#else\n"
    "__constant int * coeffs = c_YUV2RGBCoeffs_i;\n"
    "const int r = Y + CV_DESCALE(mul24(V - HALF_MAX_NUM, coeffs[3]), "
    "yuv_shift);\n"
    "const int g = Y + CV_DESCALE(mad24(V - HALF_MAX_NUM, coeffs[2], mul24(U - "
    "HALF_MAX_NUM, coeffs[1])), yuv_shift);\n"
    "const int b = Y + CV_DESCALE(mul24(U - HALF_MAX_NUM, coeffs[0]), "
    "yuv_shift);\n"
    "#endif\n"
    "dst[bidx] = SAT_CAST( b );\n"
    "dst[1] = SAT_CAST( g );\n"
    ...
    "}\n"
    "}\n"
    "__constant float c_YUV2RGBCoeffs_420[5] = { 1.163999557f, 2.017999649f, "
    "-0.390999794f,\n"
    "-0.812999725f, 1.5959997177f };\n"
    "__kernel void YUV2RGB_NVx(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dt_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "src_offset);\n"
    "__global const uchar* usrc = srcptr + mad24(rows + y, src_step, (x << 1) "
    "+ src_offset);\n"
    "__global uchar*       dst1 = dstptr + mad24(y << 1, dst_step, mad24(x, "
    "dcn<<1, dt_offset));\n"
    ...
    "}\n"
    "#if uidx < 2\n"
    "__kernel void YUV2RGB_YV12_IYUV(__global const uchar* srcptr, int "
    "src_step, int src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dt_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    ...
    "__global const uchar* ysrc = srcptr + mad24(y << 1, src_step, (x << 1) + "
    "src_offset);\n"
    "__global uchar*       dst1 = dstptr + mad24(y << 1, dst_step, x * "
    "(dcn<<1) + dt_offset);\n"
    "__global uchar*       dst2 = dst1 + dst_step;\n"
    "float Y1 = ysrc[0];\n"
    "float Y2 = ysrc[1];\n"
    ...
    "float Y4 = ysrc[src_step + 1];\n"
    "#ifdef SRC_CONT\n"
    "__global const uchar* uvsrc = srcptr + mad24(rows, src_step, "
    "src_offset);\n"
    "int u_ind = mad24(y, cols >> 1, x);\n"
    "float uv[2] = { ((float)uvsrc[u_ind]) - HALF_MAX_NUM, ((float)uvsrc[u_ind "
    "+ ((rows * cols) >> 2)]) - HALF_MAX_NUM };\n"
    "#else\n"
    "int vsteps[2] = { cols >> 1, src_step - (cols >> 1)};\n"
    "__global const uchar* usrc = srcptr + mad24(rows + (y>>1), src_step, "
    "src_offset + (y%2)*(cols >> 1) + x);\n"
    "__global const uchar* vsrc = usrc + mad24(rows >> 2, src_step, rows % 4 ? "
    "vsteps[y%2] : 0);\n"
    "float uv[2] = { ((float)usrc[0]) - HALF_MAX_NUM, ((float)vsrc[0]) - "
    "HALF_MAX_NUM };\n"
    "#endif\n"
    "float U = uv[uidx];\n"
    "float V = uv[1-uidx];\n"
    ...
    "#endif\n"
    "#if uidx < 2\n"
    "__constant float c_RGB2YUVCoeffs_420[8] = { 0.256999969f, 0.50399971f, "
    "0.09799957f, -0.1479988098f, -0.2909994125f,\n"
    "0.438999176f, -0.3679990768f, -0.0709991455f };\n"
    "__kernel void RGB2YUV_YV12_IYUV(__global const uchar* srcptr, int "
    "src_step, int src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "if (x < cols/2)\n"
    "{\n"
    "int src_index  = mad24(y << 1, src_step, mad24(x << 1, scn, "
    "src_offset));\n"
    ...
    "__global uchar* udst = dstptr + mad24(y_rows + (y>>1), dst_step, "
    "dst_offset + (y%2)*(cols >> 1) + x);\n"
    "__global uchar* vdst = udst + mad24(y_rows >> 2, dst_step, y_rows % 4 ? "
    "vsteps[y%2] : 0);\n"
    "#if PIX_PER_WI_X == 2\n"
    ...
    "#endif\n"
    "uchar4 y1, y2;\n"
    "y1.x = convert_uchar_sat(fma(coeffs[0], src_pix1[      2-bidx], "
    "fma(coeffs[1], src_pix1[      1], fma(coeffs[2], src_pix1[      bidx], "
    "16.5f))));\n"
    "y1.y = convert_uchar_sat(fma(coeffs[0], src_pix1[  scn+2-bidx], "
    "fma(coeffs[1], src_pix1[  scn+1], fma(coeffs[2], src_pix1[  scn+bidx], "
    "16.5f))));\n"
    "y1.z = convert_uchar_sat(fma(coeffs[0], src_pix1[2*scn+2-bidx], "
    "fma(coeffs[1], src_pix1[2*scn+1], fma(coeffs[2], src_pix1[2*scn+bidx], "
    "16.5f))));\n"
    "y1.w = convert_uchar_sat(fma(coeffs[0], src_pix1[3*scn+2-bidx], "
    "fma(coeffs[1], src_pix1[3*scn+1], fma(coeffs[2], src_pix1[3*scn+bidx], "
    "16.5f))));\n"
    "y2.x = convert_uchar_sat(fma(coeffs[0], src_pix2[      2-bidx], "
    "fma(coeffs[1], src_pix2[      1], fma(coeffs[2], src_pix2[      bidx], "
    "16.5f))));\n"
    "y2.y = convert_uchar_sat(fma(coeffs[0], src_pix2[  scn+2-bidx], "
    "fma(coeffs[1], src_pix2[  scn+1], fma(coeffs[2], src_pix2[  scn+bidx], "
    "16.5f))));\n"
    "y2.z = convert_uchar_sat(fma(coeffs[0], src_pix2[2*scn+2-bidx], "
    "fma(coeffs[1], src_pix2[2*scn+1], fma(coeffs[2], src_pix2[2*scn+bidx], "
    "16.5f))));\n"
    "y2.w = convert_uchar_sat(fma(coeffs[0], src_pix2[3*scn+2-bidx], "
    "fma(coeffs[1], src_pix2[3*scn+1], fma(coeffs[2], src_pix2[3*scn+bidx], "
    "16.5f))));\n"
    "*((__global int*) ydst1) = as_int(y1);\n"
    "*((__global int*) ydst2) = as_int(y2);\n"
    "float uv[4] = { fma(coeffs[3], src_pix1[      2-bidx], fma(coeffs[4], "
    "src_pix1[      1], fma(coeffs[5], src_pix1[      bidx], 128.5f))),\n"
    "fma(coeffs[5], src_pix1[      2-bidx], fma(coeffs[6], src_pix1[      1], "
    "fma(coeffs[7], src_pix1[      bidx], 128.5f))),\n"
    "fma(coeffs[3], src_pix1[2*scn+2-bidx], fma(coeffs[4], src_pix1[2*scn+1], "
    "fma(coeffs[5], src_pix1[2*scn+bidx], 128.5f))),\n"
    "fma(coeffs[5], src_pix1[2*scn+2-bidx], fma(coeffs[6], src_pix1[2*scn+1], "
    "fma(coeffs[7], src_pix1[2*scn+bidx], 128.5f))) };\n"
    ...
    "ydst1[0] = convert_uchar_sat(fma(coeffs[0], src_pix1.R_COMP, "
    "fma(coeffs[1], src_pix1.G_COMP, fma(coeffs[2], src_pix1.B_COMP, "
    "16.5f))));\n"
    "ydst1[1] = convert_uchar_sat(fma(coeffs[0], src_pix2.R_COMP, "
    "fma(coeffs[1], src_pix2.G_COMP, fma(coeffs[2], src_pix2.B_COMP, "
    "16.5f))));\n"
    "ydst2[0] = convert_uchar_sat(fma(coeffs[0], src_pix3.R_COMP, "
    "fma(coeffs[1], src_pix3.G_COMP, fma(coeffs[2], src_pix3.B_COMP, "
    "16.5f))));\n"
    "ydst2[1] = convert_uchar_sat(fma(coeffs[0], src_pix4.R_COMP, "
    "fma(coeffs[1], src_pix4.G_COMP, fma(coeffs[2], src_pix4.B_COMP, "
    "16.5f))));\n"
    "float uv[2] = { fma(coeffs[3], src_pix1.R_COMP, fma(coeffs[4], "
    "src_pix1.G_COMP, fma(coeffs[5], src_pix1.B_COMP, 128.5f))),\n"
    "fma(coeffs[5], src_pix1.R_COMP, fma(coeffs[6], src_pix1.G_COMP, "
    "fma(coeffs[7], src_pix1.B_COMP, 128.5f))) };\n"
    "udst[0] = convert_uchar_sat(uv[uidx]  );\n"
    "vdst[0] = convert_uchar_sat(uv[1-uidx]);\n"
    "#endif\n"
    "++y;\n"
    ...
    "}\n"
    "#endif\n"
    "__kernel void YUV2RGB_422(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "if (x < cols / 2)\n"
    "{\n"
    "__global const uchar* src = srcptr + mad24(y, src_step, (x << 2) + "
    "src_offset);\n"
    "__global uchar*       dst = dstptr + mad24(y, dst_step, mad24(x << 1, "
    "dcn, dst_offset));\n"
    "#pragma unroll\n"
    "for (int cy = 0; cy < PIX_PER_WI_Y; ++cy)\n"
    "{\n"
    ...
    "float vec_src[4] = { load_src & 0xff, (load_src >> 8) & 0xff, (load_src "
    ">> 16) & 0xff, (load_src >> 24) & 0xff};\n"
    ...
    "}\n"
    "}\n"
    "__constant float c_RGB2YCrCbCoeffs_f[5] = {R2YF, G2YF, B2YF, YCRF, "
    "YCBF};\n"
    "__constant int   c_RGB2YCrCbCoeffs_i[5] = {R2Y, G2Y, B2Y, YCRI, YCBI};\n"
    "__kernel void RGB2YCrCb(__global const uchar* srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar* dstptr, int dst_step, int dt_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "int Y =  CV_DESCALE(mad24(b, coeffs[2], mad24(g, coeffs[1], mul24(r, "
    "coeffs[0]))), yuv_shift);\n"
    ...
    "};\n"
    "__kernel void YCrCb2RGB(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    "__global uchar* dst, int dst_step, int dst_offset,\n"
    "int rows, int cols)\n"
    "{\n"
    ...
    "if (y < rows)\n"
    "{\n"
    "__global const DATA_TYPE * srcptr = (__global const DATA_TYPE*)(src + "
    "src_index);\n"
    ...
    "float g = fma(coeff[1], cr - HALF_MAX_NUM, fma(coeff[2], cb - "
    "HALF_MAX_NUM, yp));\n"
    ...
    "int g = yp + CV_DESCALE(mad24(coeff[1], cr - HALF_MAX_NUM, coeff[2] * (cb "
    "- HALF_MAX_NUM)), yuv_shift);\n"
...
struct cv::ocl::internal::ProgramEntry corner_oclsrc = {
    moduleName, "corner",
    "#ifdef BORDER_CONSTANT\n"
    "#elif defined BORDER_REPLICATE\n"
    ...
    "#endif\n"
    "#define THREADS 256\n"
    "__kernel void corner(__global const float * Dx, int dx_step, int "
    "dx_offset, int dx_whole_rows, int dx_whole_cols,\n"
    "__global const float * Dy, int dy_step, int dy_offset, int dy_whole_rows, "
    "int dy_whole_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols, float k)\n"
    "{\n"
    ...
    "{\n"
    "bool dx_con = dx_startX+col >= 0 && dx_startX+col < dx_whole_cols && "
    "dx_startY+i >= 0 && dx_startY+i < dx_whole_rows;\n"
    "int indexDx = mad24(dx_startY+i, dx_step>>2, dx_startX+col);\n"
    "float dx_s = dx_con ? Dx[indexDx] : 0.0f;\n"
    "bool dy_con = dy_startX+col >= 0 && dy_startX+col < dy_whole_cols && "
    "dy_startY+i >= 0 && dy_startY+i < dy_whole_rows;\n"
    ...
    "for (int i=0; i < ksY+1; i++)\n"
    "{\n"
    "int dx_selected_row = dx_startY+i, dx_selected_col = "
    ...
    "int dy_selected_row = dy_startY+i, dy_selected_col = "
    ...
    "{\n"
    "int dst_index = mad24(dst_step, dst_startY, (int)sizeof(float) * "
    "(dst_startX + col - anX));\n"
    "*(__global float *)(dst + dst_index) =\n"
    "tmp_sum[0] * tmp_sum[4] - tmp_sum[2] * tmp_sum[2] - k * (tmp_sum[0] + "
    "tmp_sum[4]) * (tmp_sum[0] + tmp_sum[4]);\n"
    "}\n"
    "if (posX < dst_cols && (posY + 1) < dst_rows)\n"
    "{\n"
    "int dst_index = mad24(dst_step, dst_startY + 1, (int)sizeof(float) * "
    "(dst_startX + col - anX));\n"
    "*(__global float *)(dst + dst_index) =\n"
    "tmp_sum[1] * tmp_sum[5] - tmp_sum[3] * tmp_sum[3] - k * (tmp_sum[1] + "
    "tmp_sum[5]) * (tmp_sum[1] + tmp_sum[5]);\n"
    "}\n"
    ...
    "*(__global float *)(dst + dst_index) = (float)((a+c) - "
    "native_sqrt((a-c)*(a-c) + b*b));\n"
    "}\n"
    "if (posX < dst_cols && (posY + 1) < dst_rows)\n"
    "{\n"
    "int dst_index = mad24(dst_step, dst_startY + 1, (int)sizeof(float) * "
...
struct cv::ocl::internal::ProgramEntry covardata_oclsrc = {
    moduleName, "covardata",
    "#ifdef BORDER_CONSTANT\n"
    "#define EXTRAPOLATE(x, maxV)\n"
    "#elif defined BORDER_REPLICATE\n"
    "#define EXTRAPOLATE(x, maxV) \\\n"
    ...
    "#error No extrapolation method\n"
    "#endif\n"
    "#define SRC(_x,_y) convert_float(((global "
    "SRCTYPE*)(Src+(_y)*src_step))[_x])\n"
    "#ifdef BORDER_CONSTANT\n"
    "#define ELEM(_x,_y,r_edge,t_edge,const_v) (_x)<0 | (_x) >= (r_edge) | "
    "(_y)<0 | (_y) >= (t_edge) ? (const_v) : SRC((_x),(_y))\n"
    "#else\n"
    "#define ELEM(_x,_y,r_edge,t_edge,const_v) SRC((_x),(_y))\n"
    "#endif\n"
    "#define DSTX(_x,_y) (((global "
    "float*)(DstX+DstXOffset+(_y)*DstXPitch))[_x])\n"
    "#define DSTY(_x,_y) (((global "
    "float*)(DstY+DstYOffset+(_y)*DstYPitch))[_x])\n"
    "#define INIT_AND_READ_LOCAL_SOURCE(width, height, fill_const, "
    "kernel_border) \\\n"
    ...
    "EXTRAPOLATE(xb,(width)); \\\n"
    "EXTRAPOLATE(yb,(height)); \\\n"
    "lsmem[liy+BLK_Y][lix+BLK_X] = ELEM(xb, yb, (width), (height), "
    "(fill_const) ); \\\n"
    "}\n"
    "__kernel void sobel3(__global const uchar * Src, int src_step, int "
    "srcOffsetX, int srcOffsetY,\n"
    "__global uchar * DstX, int DstXPitch, int DstXOffset,\n"
    "__global uchar * DstY, int DstYPitch, int DstYOffset, int dstHeight, int "
    "dstWidth,\n"
    ...
    "#endif\n"
    "}\n"
    "__kernel void sobel5(__global const uchar * Src, int src_step, int "
    "srcOffsetX, int srcOffsetY,\n"
    "__global uchar * DstX, int DstXPitch, int DstXOffset,\n"
    "__global uchar * DstY, int DstYPitch, int DstYOffset, int dstHeight, int "
    "dstWidth,\n"
    ...
    ");\n"
    "}\n"
    "__kernel void sobel7(__global const uchar * Src, int src_step, int "
    "srcOffsetX, int srcOffsetY,\n"
    "__global uchar * DstX, int DstXPitch, int DstXOffset,\n"
    "__global uchar * DstY, int DstYPitch, int DstYOffset, int dstHeight, int "
    "dstWidth,\n"
...
struct cv::ocl::internal::ProgramEntry filter2D_oclsrc = {
    moduleName, "filter2D",
    ...
    "__kernel void filter2D(__global const uchar * srcptr, int src_step, int "
    "srcOffsetX, int srcOffsetY, int srcEndX, int srcEndY,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols, float delta)\n"
    "{\n"
    "int local_id = get_local_id(0);\n"
    "int x = local_id + (LOCAL_SIZE - (KERNEL_SIZE_X - 1)) * get_group_id(0) - "
    "ANCHOR_X;\n"
    "int y = get_global_id(1);\n"
    "WT data[KERNEL_SIZE_Y];\n"
    ...
    "__global dstT *dst = (__global dstT *)(dstptr + mad24(y, dst_step, "
    "mad24(x, DSTSIZE, dst_offset)));\n"
    "#ifdef BORDER_CONSTANT\n"
    ...
    "data[sy + sy_index] = convertToWT(loadpix(srcptr + mad24(srcY, src_step, "
    "srcX * SRCSIZE)));\n"
    "else\n"
    ...
    "data[sy + sy_index] = convertToWT(loadpix(srcptr + mad24(tempY, src_step, "
    "srcX * SRCSIZE)));\n"
    "}\n"
    "#endif\n"
    "WT total_sum = 0;\n"
    ...
    "UPDATE_TOTAL_SUM(sx);\n"
    "}\n"
    "if (local_id >= ANCHOR_X && local_id < LOCAL_SIZE - (KERNEL_SIZE_X - 1 - "
    "ANCHOR_X) && x >= 0 && x < cols)\n"
    "storepix(convertToDstT(total_sum + (WT)(delta)), dst);\n"
    "}\n",
    "77e935928055f243ff9082b1879a0b2c", NULL};
struct cv::ocl::internal::ProgramEntry filter2DSmall_oclsrc = {
    moduleName, "filter2DSmall",
    "#ifdef BORDER_REPLICATE\n"
    "#define ADDR_L(i, l_edge, r_edge)  ((i) <  (l_edge) ? (l_edge)   : (i))\n"
    "#define ADDR_R(i, r_edge, addr)    ((i) >= (r_edge) ? (r_edge)-1 : "
    "(addr))\n"
    "#define ADDR_H(i, t_edge, b_edge)  ((i) <  (t_edge) ? (t_edge)   :(i))\n"
    "#define ADDR_B(i, b_edge, addr)    ((i) >= (b_edge) ? (b_edge)-1 "
    ":(addr))\n"
    "#endif\n"
    "#ifdef BORDER_REFLECT\n"
    "#define ADDR_L(i, l_edge, r_edge)  ((i) <  (l_edge) ? -(i)-1              "
    " : (i))\n"
    "#define ADDR_R(i, r_edge, addr)    ((i) >= (r_edge) ? "
    "-(i)-1+((r_edge)<<1) : (addr))\n"
    "#define ADDR_H(i, t_edge, b_edge)  ((i) <  (t_edge) ? -(i)-1 : (i))\n"
    "#define ADDR_B(i, b_edge, addr)    ((i) >= (b_edge) ? "
    "-(i)-1+((b_edge)<<1) : (addr))\n"
    "#endif\n"
    "#ifdef BORDER_REFLECT_101\n"
    "#define ADDR_L(i, l_edge, r_edge)  ((i) <  (l_edge) ? -(i)                "
    " : (i))\n"
    "#define ADDR_R(i, r_edge, addr)    ((i) >= (r_edge) ? "
    "-(i)-2+((r_edge)<<1) : (addr))\n"
    "#define ADDR_H(i, t_edge, b_edge)  ((i) <  (t_edge) ? -(i)                "
    " : (i))\n"
    "#define ADDR_B(i, b_edge, addr)    ((i) >= (b_edge) ? "
    "-(i)-2+((b_edge)<<1) : (addr))\n"
    "#endif\n"
    "#ifdef BORDER_WRAP\n"
    "#define ADDR_L(i, l_edge, r_edge)  ((i) <  (l_edge) ? (i)+(r_edge) : "
    "(i))\n"
    "#define ADDR_R(i, r_edge, addr)    ((i) >= (r_edge) ? (i)-(r_edge) : "
    "(addr))\n"
    "#define ADDR_H(i, t_edge, b_edge)  ((i) <  (t_edge) ? (i)+(b_edge) : "
    "(i))\n"
    "#define ADDR_B(i, b_edge, addr)    ((i) >= (b_edge) ? (i)-(b_edge) : "
    "(addr))\n"
    "#endif\n"
    "#ifdef BORDER_ISOLATED\n"
    ...
    "};\n"
    "#ifdef BORDER_ISOLATED\n"
    "inline bool isBorder(const struct RectCoords bounds, int2 coord, int "
    "numPixels)\n"
    "{\n"
    "return (coord.x < bounds.x1 || coord.y < bounds.y1 || coord.x + numPixels "
    "> bounds.x2 || coord.y >= bounds.y2);\n"
    "}\n"
    "#else\n"
    "inline bool isBorder(const struct RectCoords bounds, int2 coord, int "
    "numPixels)\n"
    "{\n"
    "return (coord.x < 0 || coord.y < 0 || coord.x + numPixels > bounds.x2 || "
    "coord.y >= bounds.y2);\n"
    "}\n"
    "#endif\n"
    ...
    "#define float1 float\n"
    "inline PX_LOAD_FLOAT_VEC_TYPE readSrcPixelGroup(int2 pos, __global const "
    "uchar* srcptr,\n"
    "int srcstep, const struct RectCoords srcCoords)\n"
    "{\n"
    ...
    "#define DIG(a) a,\n"
    "__constant WT1 kernelData[] = { COEFF };\n"
    "__kernel void filter2DSmall(__global const uchar * srcptr, int src_step, "
    "int srcOffsetX, int srcOffsetY, int srcEndX, int srcEndY,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols, float delta)\n"
    "{\n"
    "const struct RectCoords srcCoords = { srcOffsetX, srcOffsetY, srcEndX, "
    ...
    "int2 srcPos = (int2)(srcCoords.x1 + x - ANCHOR_X, srcCoords.y1 + y - "
    "ANCHOR_Y);\n"
    "if (!isBorder(srcCoords, srcPos, PX_LOAD_NUM_PX))\n"
    "{\n"
    "PX_LOAD_FLOAT_VEC_TYPE p = readSrcPixelGroup(srcPos, srcptr, src_step, "
    "srcCoords);\n"
    ...
    "{\n"
    "total_sum = mad(kernelData[kernelIndex++], privateData[py + sy][px + sx], "
    "total_sum);\n"
    "});\n"
    "});\n"
    "__global dstT* dstPtr = (__global dstT*)(dstptr + y * dst_step + "
    "dst_offset + x * DSTSIZE);\n"
    "storepix(convertToDstT(total_sum + (WT)(delta)), dstPtr);\n"
    "});\n"
    "});\n"
    "}\n",
    "030d23b1d64d51e6485f8941af1e3fc3", NULL};
struct cv::ocl::internal::ProgramEntry filterSepCol_oclsrc = {
    moduleName, "filterSepCol",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "#endif\n"
    "__kernel void col_filter(__global const uchar * src, int src_step, int "
    "src_offset, int src_whole_rows, int src_whole_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols, float delta)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y = get_global_id(1);\n"
    ...
    "int end_addr = mad24(src_whole_rows - 1, src_step, src_whole_cols * "
    "SRCSIZE);\n"
    ...
    "#if defined(INTEGER_ARITHMETIC)\n"
    "sum += mad24(temp[0],mat_kernel[RADIUSY - i], temp[1] * "
    "mat_kernel[RADIUSY + i]);\n"
    "#else\n"
    "sum += mad(temp[0], mat_kernel[RADIUSY - i], temp[1] * mat_kernel[RADIUSY "
    "+ i]);\n"
    "#endif\n"
    "}\n"
    "if (x < dst_cols && y < dst_rows)\n"
    ...
    "dstT result = convertToDstT(convertToFloatT(sum) * (floatT)(1.0f / (1 << "
    "SHIFT_BITS)));\n"
    "#else\n"
...
struct cv::ocl::internal::ProgramEntry filterSepRow_oclsrc = {
    moduleName, "filterSepRow",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "#ifdef EXTRA_EXTRAPOLATION\n"
    "#ifdef BORDER_CONSTANT\n"
    "#define ELEM(i,l_edge,r_edge,elem1,elem2) (i)<(l_edge) | (i) >= (r_edge) "
    "? (elem1) : (elem2)\n"
    "#elif defined BORDER_REPLICATE\n"
    ...
    "#endif\n"
    "__kernel void row_filter_C1_D0(__global const uchar * src, int "
    "src_step_in_pixel, int src_offset_x, int src_offset_y,\n"
    ...
    "#ifdef BORDER_CONSTANT\n"
    "int end_addr = mad24(src_whole_rows - 1, src_step_in_pixel, "
    "src_whole_cols);\n"
    "for (int i = 0; i < READ_TIMES_ROW; ++i)\n"
    "{\n"
    "int current_addr = mad24(i, LSIZE0 << 2, start_addr);\n"
    "current_addr = current_addr < end_addr && current_addr > 0 ? current_addr "
    ": 0;\n"
    ...
    "for (int i = 0; i < READ_TIMES_ROW; ++i)\n"
    "{\n"
    "temp[i].x = ELEM(start_x+i*LSIZE0*4,   src_offset_x, src_offset_x + "
    "src_cols, 0,         temp[i].x);\n"
    "temp[i].y = ELEM(start_x+i*LSIZE0*4+1, src_offset_x, src_offset_x + "
    "src_cols, 0,         temp[i].y);\n"
    "temp[i].z = ELEM(start_x+i*LSIZE0*4+2, src_offset_x, src_offset_x + "
    "src_cols, 0,         temp[i].z);\n"
    "temp[i].w = ELEM(start_x+i*LSIZE0*4+3, src_offset_x, src_offset_x + "
    "src_cols, 0,         temp[i].w);\n"
    "temp[i]   = ELEM(start_y,              src_offset_y, src_offset_y + "
    "src_rows, (uchar4)0, temp[i]);\n"
    "}\n"
    "#else\n"
    "for (int i = 0; i < READ_TIMES_ROW; ++i)\n"
    "{\n"
    "temp[i].x = ELEM(start_x+i*LSIZE0*4,   0, src_whole_cols, 0,         "
    "temp[i].x);\n"
    "temp[i].y = ELEM(start_x+i*LSIZE0*4+1, 0, src_whole_cols, 0,         "
    "temp[i].y);\n"
    "temp[i].z = ELEM(start_x+i*LSIZE0*4+2, 0, src_whole_cols, 0,         "
    "temp[i].z);\n"
    "temp[i].w = ELEM(start_x+i*LSIZE0*4+3, 0, src_whole_cols, 0,         "
    "temp[i].w);\n"
    "temp[i]   = ELEM(start_y,              0, src_whole_rows, (uchar4)0, "
    "temp[i]);\n"
    "}\n"
    "#endif\n"
    "#else\n"
    "#ifdef BORDER_ISOLATED\n"
    "int not_all_in_range = (start_x<src_offset_x) | (start_x + "
    "READ_TIMES_ROW*LSIZE0*4+4>src_offset_x + src_cols)| "
    "(start_y<src_offset_y) | (start_y >= src_offset_y + src_rows);\n"
    "#else\n"
    "int not_all_in_range = (start_x<0) | (start_x + "
    "READ_TIMES_ROW*LSIZE0*4+4>src_whole_cols)| (start_y<0) | (start_y >= "
    "src_whole_rows);\n"
    "#endif\n"
    ...
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "sum = convertDstVec(vload4(0,(__local uchar "
    "*)&LDS_DAT[l_y][l_x]+RADIUSX+offset)) * mat_kernel[RADIUSX];\n"
    "for (int i = 1; i <= RADIUSX; ++i)\n"
    "{\n"
    "temp[0] = vload4(0, (__local uchar*)&LDS_DAT[l_y][l_x] + RADIUSX + offset "
    "- i);\n"
    "temp[1] = vload4(0, (__local uchar*)&LDS_DAT[l_y][l_x] + RADIUSX + offset "
    "+ i);\n"
    "#if defined(INTEGER_ARITHMETIC)\n"
    "sum += mad24(convertDstVec(temp[0]), mat_kernel[RADIUSX-i], "
    "convertDstVec(temp[1]) * mat_kernel[RADIUSX + i]);\n"
    "#else\n"
    "sum += mad(convertDstVec(temp[0]), mat_kernel[RADIUSX-i], "
    "convertDstVec(temp[1]) * mat_kernel[RADIUSX + i]);\n"
    "#endif\n"
    "}\n"
    ...
    "dst[start_addr] = sum.x;\n"
    "}\n"
    "__kernel void row_filter(__global const uchar * src, int src_step, int "
    "src_offset_x, int src_offset_y,\n"
    "int src_cols, int src_rows, int src_whole_cols, int src_whole_rows,\n"
    ...
    "for (int i = 0; i < READ_TIMES_ROW; ++i)\n"
    "{\n"
    "temp[i] = ELEM(mad24(i, LSIZE0, start_x), src_offset_x, src_offset_x + "
    "src_cols, (srcT)(0), temp[i]);\n"
    "temp[i] = ELEM(start_y,                   src_offset_y, src_offset_y + "
    "src_rows, (srcT)(0), temp[i]);\n"
    "}\n"
    "#else\n"
    "for (int i = 0; i < READ_TIMES_ROW; ++i)\n"
    "{\n"
    "temp[i] = ELEM(mad24(i, LSIZE0, start_x), 0, src_whole_cols, (srcT)(0), "
    "temp[i]);\n"
    "temp[i] = ELEM(start_y,                   0, src_whole_rows, (srcT)(0), "
    "temp[i]);\n"
    "}\n"
    "#endif\n"
    "#else\n"
    ...
    "#if defined(INTEGER_ARITHMETIC)\n"
    "sum += mad24(convertToDstT(temp[0]), mat_kernel[RADIUSX - i], "
    "convertToDstT(temp[1]) * mat_kernel[RADIUSX + i]);\n"
    "#else\n"
    "sum += mad(convertToDstT(temp[0]), mat_kernel[RADIUSX - i], "
    "convertToDstT(temp[1]) * mat_kernel[RADIUSX + i]);\n"
    "#endif\n"
    "}\n"
    "if (x < dst_cols && y < dst_rows)\n"
...
struct cv::ocl::internal::ProgramEntry filterSep_singlePass_oclsrc = {
    moduleName, "filterSep_singlePass",
    "#ifdef BORDER_CONSTANT\n"
    "#define EXTRAPOLATE(x, maxV)\n"
    "#elif defined BORDER_REPLICATE\n"
    "#define EXTRAPOLATE(x, maxV) \\\n"
    ...
    "#define DSTSIZE (int)sizeof(dstT1)*3\n"
    "#endif\n"
    "#define SRC(_x,_y) convertToWT(loadpix(Src + mad24(_y, src_step, SRCSIZE "
    "* _x)))\n"
    "#ifdef BORDER_CONSTANT\n"
    "#define ELEM(_x,_y,r_edge,t_edge,const_v) (_x)<0 | (_x) >= (r_edge) | "
    "(_y)<0 | (_y) >= (t_edge) ? (const_v) : SRC((_x),(_y))\n"
    ...
    "__kernel void sep_filter(__global uchar* Src, int src_step, int "
    "srcOffsetX, int srcOffsetY, int height, int width,\n"
    "__global uchar* Dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols, float delta)\n"
    "{\n"
    ...
    "#endif\n"
    "#endif\n"
    "storepix(convertToDstT(sum), Dst + mad24(y + liy, dst_step, mad24(x, "
    "DSTSIZE, dst_offset)));\n"
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "for (int i = liy * BLK_X + lix; i < (RADIUSY*2) * (BLK_X+(RADIUSX*2)); i "
    "+= BLK_X * BLK_Y)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry filterSmall_oclsrc = {
    moduleName, "filterSmall",
    "#ifdef BORDER_REPLICATE\n"
    "#define ADDR_L(i, l_edge, r_edge)  ((i) <  (l_edge) ? (l_edge)   : (i))\n"
    "#define ADDR_R(i, r_edge, addr)    ((i) >= (r_edge) ? (r_edge)-1 : "
    "(addr))\n"
    ...
    "LOOP(KERNEL_SIZE_X, sx, \\\n"
    "{ \\\n"
    "total_sum = fma(kernelData[kernelIndex++], privateData[py + sy][px + sx], "
    "total_sum); \\\n"
    "}); \\\n"
    "})\n"
    ...
    "total_sum = convertToWT(MORPH_OP(convertToWT(total_sum), "
    "convertToWT(privateData[py + _y][px + _x])));\n"
    "#define PROCESS_ELEM \\\n"
    ...
    "#define EXTRA_PARAMS , __global const uchar * matptr, int mat_step, int "
    "mat_offset\n"
    "#else\n"
    "#define EXTRA_PARAMS\n"
    "#endif\n"
    ...
    "bounds.x1, bounds.y1,\n"
    "bounds.x2, bounds.y2);\n"
    "__global const uchar* ptr = srcptr + mad24(selected_row, srcstep, "
    "selected_col * SRCSIZE);\n"
    "return convertToWT(loadpix(ptr));\n"
    "#endif\n"
    "}\n"
    "inline WT readSrcPixelSingle(int2 pos, __global const uchar * srcptr,\n"
    ...
    "if (!isBorder(srcCoords, pos, 1))\n"
    "{\n"
    "__global const uchar * ptr = srcptr + mad24(pos.y, srcstep, pos.x * "
    "SRCSIZE);\n"
    "return convertToWT(loadpix(ptr));\n"
    "}\n"
    "else\n"
    "return getBorderPixel(srcCoords, pos, srcptr, srcstep);\n"
    "}\n"
    "__kernel void filterSmall(__global const uchar * srcptr, int src_step, "
    "int srcOffsetX, int srcOffsetY, int srcEndX, int srcEndY,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols\n"
    "#ifdef NORMALIZE\n"
    ...
    "#ifdef OP_GRADIENT\n"
    "storepix(convertToDstT(convertToWT(total_sum) - convertToWT(value)), "
    "dstPtr );\n"
    "#elif defined OP_TOPHAT\n"
    "storepix(convertToDstT(convertToWT(value) - convertToWT(total_sum)), "
    "dstPtr );\n"
...
struct cv::ocl::internal::ProgramEntry gaussianBlur3x3_oclsrc = {
    moduleName, "gaussianBlur3x3",
    "#define DIG(a) a,\n"
    "__constant float kx[] = { KERNEL_MATRIX_X };\n"
    "__constant float ky[] = { KERNEL_MATRIX_Y };\n"
    "#define OP(delta, y, x) (convert_float16(arr[(y + delta) * 3 + x]) * "
    "ky[y] * kx[x])\n"
    "__kernel void gaussianBlur3x3_8UC1_cols16_rows2(__global const uint* src, "
    "int src_step,\n"
    "__global uint* dst, int dst_step, int rows, int cols)\n"
    "{\n"
    "int block_x = get_global_id(0);\n"
    ...
    "j = (block_x == 0 || y == (rows - 2)) ? 0 : src_p[src_index + 3 * "
    "src_step - 1];\n"
    "l = (line_end || y == (rows - 2))? 0 : src_p[src_index + 3 * src_step + "
    "16];\n"
    "#elif defined BORDER_REFLECT_101\n"
    "int offset;\n"
    "offset = (y == 0) ? (2 * src_step) : 0;\n"
    "a = (block_x == 0) ? src_p[src_index + offset + 1] : src_p[src_index + "
    "offset - 1];\n"
    "c = line_end ? src_p[src_index + offset + 14] : src_p[src_index + offset "
    "+ 16];\n"
    "d = (block_x == 0) ? src_p[src_index + src_step + 1] : src_p[src_index + "
    "src_step - 1];\n"
    "f = line_end ? src_p[src_index + src_step + 14] : src_p[src_index + "
    "src_step + 16];\n"
    "g = (block_x == 0) ? src_p[src_index + 2 * src_step + 1] : "
    "src_p[src_index + 2 * src_step - 1];\n"
    "i = line_end ? src_p[src_index + 2 * src_step + 14] : src_p[src_index + 2 "
    "* src_step + 16];\n"
    "offset = (y == (rows - 2)) ? (1 * src_step) : (3 * src_step);\n"
    "j = (block_x == 0) ? src_p[src_index + offset + 1] : src_p[src_index + "
    "offset - 1];\n"
    "l = line_end ? src_p[src_index + offset + 14] : src_p[src_index + offset "
    "+ 16];\n"
    "#elif defined (BORDER_REPLICATE) || defined(BORDER_REFLECT)\n"
    "int offset;\n"
    "offset = (y == 0) ? (1 * src_step) : 0;\n"
    "a = (block_x == 0) ? src_p[src_index + offset] : src_p[src_index + offset "
    "- 1];\n"
    "c = line_end ? src_p[src_index + offset + 15] : src_p[src_index + offset "
    "+ 16];\n"
    "d = (block_x == 0) ? src_p[src_index + src_step] : src_p[src_index + "
    "src_step - 1];\n"
    "f = line_end ? src_p[src_index + src_step + 15] : src_p[src_index + "
    "src_step + 16];\n"
    "g = (block_x == 0) ? src_p[src_index + 2 * src_step] : src_p[src_index + "
    "2 * src_step - 1];\n"
    "i = line_end ? src_p[src_index + 2 * src_step + 15] : src_p[src_index + 2 "
    "* src_step + 16];\n"
    "offset = (y == (rows - 2)) ? (2 * src_step) : (3 * src_step);\n"
    "j = (block_x == 0) ? src_p[src_index + offset] : src_p[src_index + offset "
    "- 1];\n"
    "l = line_end ? src_p[src_index + offset + 15] : src_p[src_index + offset "
    "+ 16];\n"
    "#endif\n"
    "uchar16 arr[12];\n"
    "float16 sum[2];\n"
...
struct cv::ocl::internal::ProgramEntry gaussianBlur5x5_oclsrc = {
    moduleName, "gaussianBlur5x5",
    "#define DIG(a) a,\n"
    "__constant float kx[] = { KERNEL_MATRIX_X };\n"
    "__constant float ky[] = { KERNEL_MATRIX_Y };\n"
    ...
    "            \\\n"
    "arr[5 * n + 0] = row_s ? (uchar4)(s1, s2, line[n].s23) : "
    "(uchar4)(line[n].s0123); \\\n"
    "arr[5 * n + 1] = row_s ? (uchar4)(s2, line[n].s234) : "
    "(uchar4)(line[n].s1234);    \\\n"
    "arr[5 * n + 2] = (uchar4)(line[n].s2345);                                 "
    "        \\\n"
    "arr[5 * n + 3] = row_e ? (uchar4)(line[n].s345, e1) : "
    "(uchar4)(line[n].s3456);    \\\n"
    "arr[5 * n + 4] = row_e ? (uchar4)(line[n].s45, e1, e2) : "
    "(uchar4)(line[n].s4567);\n"
    "__kernel void gaussianBlur5x5_8UC1_cols4(__global const uchar* src, int "
    "src_step,\n"
    "__global uint* dst, int dst_step, int rows, int cols)\n"
    "{\n"
    ...
    "}\n"
    "line[3] = (y == (rows - 1)) ? tmp : vload8(0, src + src_index + 3 * "
    "src_step);\n"
    "line[4] = (y >= (rows - 2)) ? tmp : vload8(0, src + src_index + 4 * "
    "src_step);\n"
    "#elif BORDER_REFLECT\n"
    "int t;\n"
    "t = (y <= 1) ? (abs(y - 1) - y + 2) : 0;\n"
    ...
    "}\n"
    "line[3] = (y == (rows - 1)) ? line[2] : vload8(0, src + src_index + 3 * "
    "src_step);\n"
    "t = (y >= (rows - 2)) ? (abs(y - (rows - 1)) - (y - (rows - 2)) + 2) : "
    "4;\n"
    ...
    "if (offset == 0) line[0] = (uchar8)(0, 0, line[0].s0123, line[0].s45);\n"
    "}\n"
    "offset = (y == 0) ? (src_index + 3 * src_step) : max(0, src_index + 1 * "
    "src_step);\n"
    "line[1] = vload8(0, src + offset);\n"
    "if (offset == 0) line[1] = (uchar8)(0, 0, line[1].s0123, line[1].s45);\n"
    "line[3] = vload8(0, src + src_index + ((y == (rows - 1)) ? 1 : 3) * "
    "src_step);\n"
    "if (y == (rows - 2))\n"
    "line[4] = line[2];\n"
    "else\n"
    "{\n"
    "line[4] = vload8(0, src + src_index + ((y == (rows - 1)) ? 1 : 4) * "
    "src_step);\n"
    "}\n"
    "#endif\n"
    "bool row_s = (x == 0);\n"
...
struct cv::ocl::internal::ProgramEntry gftt_oclsrc = {
    moduleName, "gftt",
    "#ifdef OP_MAX_EIGEN_VAL\n"
    "__kernel void maxEigenVal(__global const uchar * srcptr, int src_step, "
    "int src_offset, int cols,\n"
    ...
    "{\n"
    "int src_index = mad24(id / cols, src_step, mad24((id % cols), "
    "(int)sizeof(float), src_offset));\n"
    "#ifdef HAVE_MASK\n"
    ...
    "localmem_max[lid - WGS2_ALIGNED] = max(maxval, localmem_max[lid - "
    "WGS2_ALIGNED]);\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    ...
    "}\n"
    "if (lid == 0)\n"
    "*(__global float *)(dstptr + (int)sizeof(float) * gid) = "
    "localmem_max[0];\n"
    "}\n"
    ...
    "#define GET_SRC_32F(_y, _x) *(__global const float *)(eigptr + (_y) * "
    "eig_step + (_x) * (int)sizeof(float) )\n"
    "__kernel void findCorners(__global const uchar * eigptr, int eig_step, "
    "int eig_offset,\n"
    "#ifdef HAVE_MASK\n"
    "__global const uchar * mask, int mask_step, int mask_offset,\n"
    "#endif\n"
    ...
    "__global float2 * corners = (__global float2 *)(cornersptr + "
    "(int)sizeof(float2));\n"
    "if (y < rows && x < cols\n"
    "#ifdef HAVE_MASK\n"
...
struct cv::ocl::internal::ProgramEntry histogram_oclsrc = {
    ...
    "#endif\n"
    "#define noconvert\n"
    "__kernel void calculate_histogram(__global const uchar * src_ptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    ...
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "__global int * hist = (__global int *)(histptr + gid * BINS * "
    "(int)sizeof(int));\n"
    "#pragma unroll\n"
    "for (int i = lid; i < BINS; i += WGS)\n"
    "hist[i] = localhist[i];\n"
    ...
    "#endif\n"
    "__kernel void merge_histogram(__global const int * ghist, __global uchar "
    "* histptr, int hist_step, int hist_offset)\n"
    "{\n"
    "int lid = get_local_id(0);\n"
    ...
    "#endif\n"
    "}\n"
    "__kernel void calcLUT(__global uchar * dst, __global const int * ghist, "
    "int total)\n"
    "{\n"
    "int lid = get_local_id(0);\n"
...
struct cv::ocl::internal::ProgramEntry hough_lines_oclsrc = {
    moduleName, "hough_lines",
    "#define ACCUM(ptr) *((__global int*)(ptr))\n"
    "#ifdef MAKE_POINTS_LIST\n"
    "__kernel void make_point_list(__global const uchar * src_ptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * list_ptr, int list_step, int list_offset, __global int* "
    "global_offset)\n"
    "{\n"
    ...
    "__kernel void fill_accum_global(__global const uchar * list_ptr, int "
    "list_step, int list_offset,\n"
    ...
    "sinVal *= irho;\n"
    "cosVal *= irho;\n"
    "__global const int * list = (__global const int*)(list_ptr + "
    "list_offset);\n"
    "__global int* accum = (__global int*)(accum_ptr + mad24(theta_idx + 1, "
    "accum_step, accum_offset));\n"
    ...
    "__kernel void fill_accum_local(__global const uchar * list_ptr, int "
    "list_step, int list_offset,\n"
    "__global uchar * accum_ptr, int accum_step, int accum_offset,\n"
    "int total_points, float irho, float theta, int numrho, int numangle)\n"
    "{\n"
    ...
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "__global int* accum = (__global int*)(accum_ptr + mad24(theta_idx, "
    "accum_step, accum_offset));\n"
    "for (int i=count_idx; i<BUFFER_SIZE; i+=LOCAL_SIZE)\n"
    "accum[i] = l_accum[i];\n"
    "}\n"
    ...
    "__kernel void get_lines(__global uchar * accum_ptr, int accum_step, int "
    "accum_offset, int accum_rows, int accum_cols,\n"
    "__global uchar * lines_ptr, int lines_step, int lines_offset, __global "
    "int* lines_index_ptr,\n"
    ...
    "if (y < accum_rows-2)\n"
    "{\n"
    "__global uchar* accum = accum_ptr + mad24(y+1, accum_step, mad24(x0+1, "
    "(int) sizeof(int), accum_offset));\n"
    ...
    "if (curVote > threshold && curVote > ACCUM(accum - sizeof(int)) && "
    "curVote >= ACCUM(accum + sizeof(int)) &&\n"
    "curVote > ACCUM(accum - accum_step) && curVote >= ACCUM(accum + "
    "accum_step))\n"
    "{\n"
    ...
    "__kernel void get_lines(__global const uchar * accum_ptr, int accum_step, "
    "int accum_offset, int accum_rows, int accum_cols,\n"
    "__global const uchar * src_ptr, int src_step, int src_offset, int "
    "src_rows, int src_cols,\n"
    "__global uchar * lines_ptr, int lines_step, int lines_offset, __global "
    "int* lines_index_ptr,\n"
    "int linesMax, int threshold, int lineLength, int lineGap, float rho, "
    "float theta)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y = get_global_id(1);\n"
    "if (y < accum_rows-2)\n"
    "{\n"
    "__global const uchar* accum = accum_ptr + mad24(y+1, accum_step, "
    "mad24(x+1, (int) sizeof(int), accum_offset));\n"
    ...
    "float2 pb[4] = { (float2)(-1, -1), (float2)(-1, -1), (float2)(-1, -1), "
    "(float2)(-1, -1) };\n"
    "float a;\n"
    "if (dir.x != 0)\n"
    "{\n"
    ...
    "dir = -dir;\n"
    "}\n"
    "else if (pb[1].x == src_cols - 1 && (pb[1].y >= 0 && pb[1].y < "
    "src_rows))\n"
    "{\n"
    "p0 = pb[1];\n"
    "if (dir.x > 0)\n"
    ...
    "dir = -dir;\n"
    "}\n"
    "else if (pb[3].y == src_rows - 1 && (pb[3].x >= 0 && pb[3].x < "
    "src_cols))\n"
    "{\n"
    "p0 = pb[3];\n"
    "if (dir.y > 0)\n"
    ...
    "int index = atomic_inc(lines_index);\n"
    "if (index < linesMax)\n"
    "lines[index] = (int4)(line_end[0].x, line_end[0].y, line_end[1].x, "
    "line_end[1].y);\n"
    "}\n"
    "gap = 0;\n"
    "inLine = false;\n"
...
struct cv::ocl::internal::ProgramEntry integral_sum_oclsrc = {
    moduleName, "integral_sum",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "kernel void integral_sum_cols(__global const uchar *src_ptr, int "
    "src_step, int src_offset, int rows, int cols,\n"
    ...
    "int lsum_index = lid;\n"
    "#pragma unroll\n"
    "for (int yin = 0; yin < LOCAL_SUM_SIZE; yin++, src_index+=src_step, "
    "lsum_index += LOCAL_SUM_STRIDE)\n"
    "{\n"
    ...
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "int buf_index = mad24(buf_step, LOCAL_SUM_SIZE * gid, "
    "mad24((int)sizeof(sumT), y + lid, buf_offset));\n"
    "#ifdef SUM_SQUARE\n"
    "int buf_sq_index = mad24(buf_sq_step, LOCAL_SUM_SIZE * gid, "
    "mad24((int)sizeof(sumSQT), y + lid, buf_sq_offset));\n"
    "#endif\n"
    ...
    "}\n"
    "}\n"
    "kernel void integral_sum_rows(__global const uchar *buf_ptr, int "
    "buf_step, int buf_offset,\n"
    "#ifdef SUM_SQUARE\n"
    "__global uchar *buf_sq_ptr, int buf_sq_step, int buf_sq_offset,\n"
    "#endif\n"
    "__global uchar *dst_ptr, int dst_step, int dst_offset, int rows, int "
    "cols\n"
    "#ifdef SUM_SQUARE\n"
    ...
    "sumT accum = 0;\n"
    "#ifdef SUM_SQUARE\n"
    "__global sumSQT *dst_sq = (__global sumSQT *)(dst_sq_ptr + "
    "dst_sq_offset);\n"
    "for (int xin = x; xin < cols; xin += gs)\n"
    "{\n"
    "dst_sq[xin] = 0;\n"
    ...
    "if (x < rows - 1)\n"
    "{\n"
    "dst_sq = (__global sumSQT *)(dst_sq_ptr + mad24(x, dst_sq_step, "
    "dst_sq_offset));\n"
    "dst_sq[0] = 0;\n"
    "}\n"
    ...
    "buf_index += buf_step;\n"
    "#ifdef SUM_SQUARE\n"
    "__global const sumSQT *buf_sq = (__global const sumSQT *)(buf_sq_ptr + "
    "buf_sq_index);\n"
    "accum_sq += buf_sq[0];\n"
    "lm_sum_sq[lsum_index] = accum_sq;\n"
    "buf_sq_index += buf_sq_step;\n"
    ...
    "if (y + lid < cols)\n"
    "{\n"
    "int dst_index = mad24(dst_step, LOCAL_SUM_SIZE * gid, "
    "mad24((int)sizeof(sumT), y + lid, dst_offset));\n"
    "#ifdef SUM_SQUARE\n"
    "int dst_sq_index = mad24(dst_sq_step, LOCAL_SUM_SIZE * gid, "
    "mad24((int)sizeof(sumSQT), y + lid, dst_sq_offset));\n"
    "#endif\n"
    "lsum_index = LOCAL_SUM_STRIDE * lid;\n"
...
struct cv::ocl::internal::ProgramEntry laplacian3_oclsrc = {
    moduleName, "laplacian3",
    "#define DIG(a) a,\n"
    "__constant float kx[] = { KERNEL_MATRIX };\n"
    "#define OP(delta, x) (convert_float16(arr[delta + x]) * kx[x])\n"
    "__kernel void laplacian3_8UC1_cols16_rows2(__global const uint* src, int "
    "src_step,\n"
...
struct cv::ocl::internal::ProgramEntry laplacian5_oclsrc = {
    moduleName, "laplacian5",
    "#define noconvert\n"
    "#ifdef ONLY_SUM_CONVERT\n"
    "__kernel void sumConvert(__global const uchar * src1ptr, int src1_step, "
    "int src1_offset,\n"
    "__global const uchar * src2ptr, int src2_step, int src2_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "if (y < dst_rows && x < dst_cols)\n"
    "{\n"
    "int src1_index = mad24(y, src1_step, mad24(x, (int)sizeof(srcT), "
    "src1_offset));\n"
    "int src2_index = mad24(y, src2_step, mad24(x, (int)sizeof(srcT), "
    "src2_offset));\n"
    "int dst_index = mad24(y, dst_step, mad24(x, (int)sizeof(dstT), "
    "dst_offset));\n"
    "__global const srcT * src1 = (__global const srcT *)(src1ptr + "
    "src1_index);\n"
    "__global const srcT * src2 = (__global const srcT *)(src2ptr + "
    "src2_index);\n"
    "__global dstT * dst = (__global dstT *)(dstptr + dst_index);\n"
    "#if wdepth <= 4\n"
    "dst[0] = convertToDT( mad24((WT)(scale), convertToWT(src1[0]) + "
    "convertToWT(src2[0]), (WT)(delta)) );\n"
    "#else\n"
    "dst[0] = convertToDT( mad((WT)(scale), convertToWT(src1[0]) + "
    "convertToWT(src2[0]), (WT)(delta)) );\n"
    "#endif\n"
    "}\n"
    "}\n"
    ...
    "__constant WT1 mat_kernelX[] = { KERNEL_MATRIX_X };\n"
    "__constant WT1 mat_kernelY[] = { KERNEL_MATRIX_Y };\n"
    "__kernel void laplacian(__global uchar* Src, int src_step, int "
    "srcOffsetX, int srcOffsetY, int height, int width,\n"
    "__global uchar* Dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "WT1 scale, WT1 delta)\n"
    ...
    "storepix(convertToDT(sum), Dst + mad24(y + liy, dst_step, mad24(x, "
    "DSTSIZE, dst_offset)));\n"
    "}\n"
    "for (int i = liy * BLK_X + lix; i < (RADIUS*2) * (BLK_X+(RADIUS*2)); i += "
    "BLK_X * BLK_Y)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry linearPolar_oclsrc = {
    moduleName, "linearPolar",
    "#define CV_2PI 6.283185307179586476925286766559\n"
    "#ifdef ForwardMap\n"
    "__kernel void computeAngleRadius(__global float2* cp_sp, __global float* "
    "r, float maxRadius_width,  float PI2_height,  unsigned width, unsigned "
    "height)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void linearPolar(__global float* mx, __global float* my, "
    "__global float2* cp_sp,  __global float* r, float cx, float cy, unsigned "
    ...
    "__kernel void linearPolar(__global float* mx, __global float* my, float "
    "ascale, float pscale, float cx, float cy, int angle_border, unsigned "
    "width, unsigned height)\n"
    "{\n"
    "const int x = get_global_id(0);\n"
    "const int y = get_global_id(1);\n"
...
struct cv::ocl::internal::ProgramEntry logPolar_oclsrc = {
    moduleName, "logPolar",
    "#define CV_2PI 6.283185307179586476925286766559\n"
    "#ifdef ForwardMap\n"
    "__kernel void computeAngleRadius(__global float2* cp_sp, __global float* "
    "r, float m, float PI2_height, unsigned width, unsigned height)\n"
    "{\n"
    "unsigned gid = get_global_id(0);\n"
    "if (gid < height)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void logPolar(__global float* mx, __global float* my, __global "
    "float2* cp_sp, __global float* r, float cx, float cy, unsigned width, "
    "unsigned height)\n"
    "{\n"
    "__local float l_r[MEM_SIZE];\n"
    "__local float2 l_double[MEM_SIZE];\n"
    ...
    "}\n"
    "#elif defined (InverseMap)\n"
    "__kernel void logPolar(__global float* mx, __global float* my, float "
    "ascale, float m, float cx, float cy, int angle_border, unsigned width, "
    "unsigned height)\n"
    "{\n"
    "const int x = get_global_id(0);\n"
    "const int y = get_global_id(1);\n"
...
struct cv::ocl::internal::ProgramEntry match_template_oclsrc = {
    ...
    "#endif\n"
    "#define SQSUMS_PTR(ox, oy) mad24(y + oy, src_sqsums_step, mad24(x + ox, "
    "cn, src_sqsums_offset))\n"
    "#define SUMS_PTR(ox, oy) mad24(y + oy, src_sums_step, mad24(x + ox, cn, "
    "src_sums_offset))\n"
    "#define SUMS(ox, oy)    mad24(y+oy, src_sums_step, mad24(x+ox, "
    "(int)sizeof(T1)*cn, src_sums_offset))\n"
    "#define SQ_SUMS(ox, oy) mad24(y+oy, src_sqsums_step, mad24(x+ox, "
    "(int)sizeof(T1)*cn, src_sqsums_offset))\n"
    ...
    "#endif\n"
    "#ifdef CALC_SUM\n"
    "__kernel void calcSum(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    ...
    "for ( ; id < total; id += WGS)\n"
    "{\n"
    "int src_index = mad24(id / cols, src_step, mad24(id % cols, TSIZE, "
    "src_offset));\n"
    ...
    "__kernel void extractFirstChannel( const __global uchar* img, int "
    "img_step, int img_offset,\n"
    ...
    "{\n"
    "T1 image = *(__global const T1*)(img + mad24(y, img_step, mad24(x, "
    "(int)sizeof(T1)*cn, img_offset)));;\n"
    "int res_idx = mad24(y, res_step, mad24(x, (int)sizeof(float), "
    "res_offset));\n"
    ...
    "__kernel void matchTemplate_Naive_CCORR(__global const uchar * srcptr, "
    "int src_step, int src_offset,\n"
    "__global const uchar * templateptr, int template_step, int "
    "template_offset, int template_rows, int template_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols)\n"
    "{\n"
    ...
    "__global const T1 * template = (__global const T1*)(templateptr + "
    "template_offset);\n"
    ...
    "{\n"
    "T temp = (T)(template[j]);\n"
    "T src = vload4(0, (__global const T1*)(srcptr + ind + "
    "j*(int)sizeof(T1)));\n"
    "sum = mad(convertToWT(src), convertToWT(temp), sum);\n"
    "}\n"
    "ind += src_step;\n"
    "template = (__global const T1 *)((__global const uchar *)template + "
    "template_step);\n"
    "}\n"
    "T temp = (T)(template[0]);\n"
    "int dst_idx = mad24(y, dst_step, mad24(x0, (int)sizeof(float), "
    "dst_offset));\n"
    ...
    "__global const T1 * src = (__global const T1 *)(srcptr + mad24(y, "
    "src_step, mad24(x0, (int)sizeof(T1), src_offset)));\n"
    "__global const T1 * template = (__global const T1 *)(templateptr + "
    "template_offset);\n"
    "for (int i = 0; i < template_rows; ++i)\n"
    "{\n"
    ...
    "{\n"
    "sum[cx] = mad(convertToWT1(src[j+cx]), convertToWT1(template[j]), "
    "sum[cx]);\n"
    "}\n"
    "}\n"
    ...
    "for (int j = 0; j < template_cols; ++j)\n"
    "{\n"
    "T src      = loadpix(srcptr      + mad24(y+i, src_step,    mad24(x+j, "
    "TSIZE, src_offset)));\n"
    "T template = loadpix(templateptr + mad24(i, template_step, mad24(j, "
    "TSIZE, template_offset)));\n"
    "sum = mad(convertToWT(src), convertToWT(template), sum);\n"
    "}\n"
    "}\n"
    "int dst_idx = mad24(y, dst_step, mad24(x, (int)sizeof(float), "
    "dst_offset));\n"
    ...
    "__kernel void matchTemplate_CCORR_NORMED(__global const uchar * "
    "src_sqsums, int src_sqsums_step, int src_sqsums_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "int template_rows, int template_cols, __global const float * "
    "template_sqsum)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y = get_global_id(1);\n"
    ...
    "float image_sqsum_ = (float)(sqsum[SQSUMS_PTR(template_cols, "
    "template_rows)] - sqsum[SQSUMS_PTR(template_cols, 0)] -\n"
    ...
    "}\n"
    "#elif defined SQDIFF\n"
    "__kernel void matchTemplate_Naive_SQDIFF(__global const uchar * srcptr, "
    "int src_step, int src_offset,\n"
    "__global const uchar * templateptr, int template_step, int "
    "template_offset, int template_rows, int template_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    ...
    "__kernel void matchTemplate_Prepared_SQDIFF(__global const uchar * "
    "src_sqsums, int src_sqsums_step, int src_sqsums_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "int template_rows, int template_cols, __global const float * "
    ...
    "(sqsum[SQSUMS_PTR(template_cols, template_rows)] - "
    "sqsum[SQSUMS_PTR(template_cols, 0)]) -\n"
    "(sqsum[SQSUMS_PTR(0, template_rows)] - sqsum[SQSUMS_PTR(0, 0)]));\n"
    ...
    "__kernel void matchTemplate_SQDIFF_NORMED(__global const uchar * "
    "src_sqsums, int src_sqsums_step, int src_sqsums_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "int template_rows, int template_cols, __global const float * "
    ...
    "dst_offset));\n"
    "__global float * dstult = (__global float *)(dst + dst_idx);\n"
    "*dstult = normAcc_SQDIFF(image_sqsum_ - 2.0f * dstult[0] + "
    "template_sqsum_value, sqrt(image_sqsum_ * template_sqsum_value));\n"
    "}\n"
    "}\n"
    "#elif defined CCOEFF\n"
    "#if cn == 1\n"
    "__kernel void matchTemplate_Prepared_CCOEFF(__global const uchar * "
    "src_sums, int src_sums_step, int src_sums_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "if (x < dst_cols && y < dst_rows)\n"
    "{\n"
    "__global const T* sum = (__global const T*)(src_sums + mad24(y, "
    "src_sums_step, mad24(x, (int)sizeof(T), src_sums_offset)));\n"
    "int step = src_sums_step/(int)sizeof(T);\n"
    "T image_sum = (T)(0), value;\n"
    "value = (T)(sum[mad24(template_rows, step, template_cols)] - "
    "sum[mad24(template_rows, step, 0)] - sum[template_cols] + sum[0]);\n"
    ...
    "value  = vload3(0, (__global const T1 *)(src_sums + SUMS(template_cols, "
    "template_rows)));\n"
    "value -= vload3(0, (__global const T1 *)(src_sums + SUMS(0, "
    "template_rows)));\n"
    "value -= vload3(0, (__global const T1 *)(src_sums + SUMS(template_cols, "
    "0)));\n"
    ...
    "#endif\n"
    "value = (sum[mad24(template_rows, step, template_cols)] - "
    "sum[mad24(template_rows, step, 0)] - sum[template_cols] + sum[0]);\n"
    ...
    "#if cn == 1\n"
    "__kernel void matchTemplate_CCOEFF_NORMED(__global const uchar * "
    "src_sums, int src_sums_step, int src_sums_offset,\n"
    "__global const uchar * src_sqsums, int src_sqsums_step, int "
    "src_sqsums_offset,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "int t_rows, int t_cols, float weight, float template_sum, float "
    "template_sqsum)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y = get_global_id(1);\n"
    ...
    "__global const T* sum   = (__global const T*)(src_sums + mad24(y, "
    "src_sums_step,     mad24(x, (int)sizeof(T), src_sums_offset)));\n"
    "__global const T* sqsum = (__global const T*)(src_sqsums + mad24(y, "
    "src_sqsums_step, mad24(x, (int)sizeof(T), src_sqsums_offset)));\n"
    "T value_sum   = sum[mad24(t_rows, step, t_cols)] - sum[mad24(t_rows, "
    "step, 0)] - sum[t_cols] + sum[0];\n"
    "T value_sqsum = sqsum[mad24(t_rows, step, t_cols)] - sqsum[mad24(t_rows, "
    "step, 0)] - sqsum[t_cols] + sqsum[0];\n"
    ...
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "int t_rows, int t_cols, float weight, float4 template_sum, float "
    "template_sqsum)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y = get_global_id(1);\n"
    ...
    "temp_sum.y = template_sum.y;\n"
    "temp_sum.z = template_sum.z;\n"
    "value_sum  = vload3(0, (__global const T1 *)(src_sums + SUMS(t_cols, "
    "t_rows)));\n"
    "value_sum -= vload3(0, (__global const T1 *)(src_sums + SUMS(0, "
    "t_rows)));\n"
    "value_sum -= vload3(0, (__global const T1 *)(src_sums + SUMS(t_cols, "
    "0)));\n"
    "value_sum += vload3(0, (__global const T1 *)(src_sums + SUMS(0, 0)));\n"
    "value_sqsum  = vload3(0, (__global const T1 *)(src_sqsums + "
    "SQ_SUMS(t_cols, t_rows)));\n"
    "value_sqsum -= vload3(0, (__global const T1 *)(src_sqsums + SQ_SUMS(0, "
    "t_rows)));\n"
    "value_sqsum -= vload3(0, (__global const T1 *)(src_sqsums + "
    "SQ_SUMS(t_cols, 0)));\n"
    "value_sqsum += vload3(0, (__global const T1 *)(src_sqsums + SQ_SUMS(0, "
...
struct cv::ocl::internal::ProgramEntry medianFilter_oclsrc = {
    ...
    "#ifdef USE_4OPT\n"
    "#if cn == 1\n"
    "#define LOAD4(val, offs) (val) = vload4(0, (__global T1 *)(srcptr + "
    "src_index + (offs)))\n"
    "#define STORE4(val, offs) vstore4((val), 0, (__global T1 *)(dstptr + "
    "(offs)))\n"
    "#define SHUFFLE4_3(src0, src1, src2, dst0, dst1, dst2) { dst1 = src1; \\\n"
    "dst0 = (T4)(src0, dst1.xyz); \\\n"
    "dst2 = (T4)(dst1.yzw, src2); }\n"
    "#define SHUFFLE4_5(src0, src1, src2, src3, src4, dst0, dst1, dst2, dst3, "
    "dst4) { dst2 = src2; \\\n"
    ...
    "#elif cn == 2\n"
    "#define LOAD4(val, offs) (val) = vload8(0, (__global T1 *)(srcptr + "
    "src_index + (offs)))\n"
    "#define STORE4(val, offs) vstore8((val), 0, (__global T1 *)(dstptr + "
    "(offs)))\n"
    "#define SHUFFLE4_3(src0, src1, src2, dst0, dst1, dst2) { dst1 = src1; \\\n"
    ...
    "#elif cn == 4\n"
    "#define LOAD4(val, offs) (val) = vload16(0, (__global T1 *)(srcptr + "
    "src_index + (offs)))\n"
    "#define STORE4(val, offs) vstore16((val), 0, (__global T1 *)(dstptr + "
    "(offs)))\n"
    "#define SHUFFLE4_3(src0, src1, src2, dst0, dst1, dst2) { dst1 = src1; \\\n"
    ...
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "T p0 = data[y][x], p1 = data[y][x+1], p2 = data[y][x+2], p3 = "
    "data[y][x+3], p4 = data[y][x+4];\n"
    "T p5 = data[y+1][x], p6 = data[y+1][x+1], p7 = data[y+1][x+2], p8 = "
    "data[y+1][x+3], p9 = data[y+1][x+4];\n"
    "T p10 = data[y+2][x], p11 = data[y+2][x+1], p12 = data[y+2][x+2], p13 = "
    "data[y+2][x+3], p14 = data[y+2][x+4];\n"
    "T p15 = data[y+3][x], p16 = data[y+3][x+1], p17 = data[y+3][x+2], p18 = "
    "data[y+3][x+3], p19 = data[y+3][x+4];\n"
    "T p20 = data[y+4][x], p21 = data[y+4][x+1], p22 = data[y+4][x+2], p23 = "
    "data[y+4][x+3], p24 = data[y+4][x+4];\n"
    "T mid;\n"
    "OP(p1, p2); OP(p0, p1); OP(p1, p2); OP(p4, p5); OP(p3, p4);\n"
    "OP(p4, p5); OP(p0, p3); OP(p2, p5); OP(p2, p3); OP(p1, p4);\n"
    "OP(p1, p2); OP(p3, p4); OP(p7, p8); OP(p6, p7); OP(p7, p8);\n"
...
struct cv::ocl::internal::ProgramEntry moments_oclsrc = {
    moduleName, "moments",
    "#if TILE_SIZE != 32\n"
    "#error \"TILE SIZE should be 32\"\n"
    "#endif\n"
    "__kernel void moments(__global const uchar* src, int src_step, int "
    "src_offset,\n"
    ...
    "(int4)(p.s2, p.s2 * 2, p.s2 * 4, p.s2 * 8) + (int4)(p.s3, p.s3 * 3, p.s3 "
    "* 9, p.s3 * 27);\n"
    "if (x_max >= 8)\n"
    "{\n"
    ...
    "p = min(p, 1);\n"
    "#endif\n"
    "S += (int4)(p.s0, p.s0 * 4, p.s0 * 16, p.s0 * 64) + (int4)(p.s1, p.s1 * "
    "5, p.s1 * 25, p.s1 * 125) +\n"
    "(int4)(p.s2, p.s2 * 6, p.s2 * 36, p.s2 * 216) + (int4)(p.s3, p.s3 * 7, "
    "p.s3 * 49, p.s3 * 343);\n"
    "if (x_max >= 12)\n"
    "{\n"
    ...
    "p = min(p, 1);\n"
    "#endif\n"
    "S += (int4)(p.s0, p.s0 * 8, p.s0 * 64, p.s0 * 512) + (int4)(p.s1, p.s1 * "
    "9, p.s1 * 81, p.s1 * 729) +\n"
    "(int4)(p.s2, p.s2 * 10, p.s2 * 100, p.s2 * 1000) + (int4)(p.s3, p.s3 * "
    "11, p.s3 * 121, p.s3 * 1331);\n"
    "if (x_max >= 16)\n"
    "{\n"
    ...
    "p = min(p, 1);\n"
    "#endif\n"
    "S += (int4)(p.s0, p.s0 * 12, p.s0 * 144, p.s0 * 1728) + (int4)(p.s1, p.s1 "
    "* 13, p.s1 * 169, p.s1 * 2197) +\n"
    "(int4)(p.s2, p.s2 * 14, p.s2 * 196, p.s2 * 2744) + (int4)(p.s3, p.s3 * "
    "15, p.s3 * 225, p.s3 * 3375);\n"
    "}\n"
    "}\n"
    "}\n"
    "}\n"
    ...
    "p = min(p, 1);\n"
    "#endif\n"
    "S += (int4)(p.s0, p.s0 * 16, p.s0 * 256, p.s0 * 4096) + (int4)(p.s1, p.s1 "
    "* 17, p.s1 * 289, p.s1 * 4913) +\n"
    "(int4)(p.s2, p.s2 * 18, p.s2 * 324, p.s2 * 5832) + (int4)(p.s3, p.s3 * "
    "19, p.s3 * 361, p.s3 * 6859);\n"
    "if (x_max >= 24)\n"
    "{\n"
    ...
    "p = min(p, 1);\n"
    "#endif\n"
    "S += (int4)(p.s0, p.s0 * 20, p.s0 * 400, p.s0 * 8000) + (int4)(p.s1, p.s1 "
    "* 21, p.s1 * 441, p.s1 * 9261) +\n"
    "(int4)(p.s2, p.s2 * 22, p.s2 * 484, p.s2 * 10648) + (int4)(p.s3, p.s3 * "
    "23, p.s3 * 529, p.s3 * 12167);\n"
    "if (x_max >= 28)\n"
    "{\n"
    ...
    "p = min(p, 1);\n"
    "#endif\n"
    "S += (int4)(p.s0, p.s0 * 24, p.s0 * 576, p.s0 * 13824) + (int4)(p.s1, "
    "p.s1 * 25, p.s1 * 625, p.s1 * 15625) +\n"
    "(int4)(p.s2, p.s2 * 26, p.s2 * 676, p.s2 * 17576) + (int4)(p.s3, p.s3 * "
    "27, p.s3 * 729, p.s3 * 19683);\n"
    "if (x_max >= 32)\n"
    "{\n"
    ...
    "p = min(p, 1);\n"
    "#endif\n"
    "S += (int4)(p.s0, p.s0 * 28, p.s0 * 784, p.s0 * 21952) + (int4)(p.s1, "
    "p.s1 * 29, p.s1 * 841, p.s1 * 24389) +\n"
    "(int4)(p.s2, p.s2 * 30, p.s2 * 900, p.s2 * 27000) + (int4)(p.s3, p.s3 * "
    "31, p.s3 * 961, p.s3 * 29791);\n"
    "}\n"
    "}\n"
    "}\n"
    "}\n"
...
struct cv::ocl::internal::ProgramEntry morph_oclsrc = {
    moduleName, "morph",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#define ELEM(i, l_edge, r_edge, elem1, elem2) (i) < (l_edge) | (i) >= "
    "(r_edge) ? (elem1) : (elem2)\n"
    "#if defined OP_GRADIENT || defined OP_TOPHAT || defined OP_BLACKHAT\n"
    "#define EXTRA_PARAMS , __global const uchar * matptr, int mat_step, int "
    "mat_offset\n"
    ...
    "#define EXTRA_PARAMS\n"
    "#endif\n"
    "__kernel void morph(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    ...
    "start_addr2 = start_addr2 < end_addr && start_addr2 > 0 ? start_addr2 : "
    "0;\n"
    ...
    "#ifdef OP_GRADIENT\n"
    "storepix(convertToT(convertToWT(res) - convertToWT(value)), dstptr + "
    "dst_index);\n"
    "#elif defined OP_TOPHAT\n"
    "storepix(convertToT(convertToWT(value) - convertToWT(res)), dstptr + "
    "dst_index);\n"
    "#elif defined OP_BLACKHAT\n"
    "storepix(convertToT(convertToWT(res) - convertToWT(value)), dstptr + "
    "dst_index);\n"
...
struct cv::ocl::internal::ProgramEntry morph3x3_oclsrc = {
    moduleName, "morph3x3",
    "#ifdef OP_ERODE\n"
    ...
    "PROCESS_ELEM_\n"
    "__kernel void morph3x3_8UC1_cols16_rows2(__global const uint* src, int "
    "src_step,\n"
    "__global uint* dst, int dst_step,\n"
    "int rows, int cols\n"
    "EXTRA_PARAMS)\n"
    ...
    "uchar16 line_out[2];\n"
    "int src_index = block_x * 4 * ssx + (y - 1) * (src_step / 4);\n"
    "line[0] = (y == 0) ? (uchar16)VAL: as_uchar16(vload4(0, src + "
    "src_index));\n"
    "line[1] = as_uchar16(vload4(0, src + src_index + (src_step / 4)));\n"
    "line[2] = as_uchar16(vload4(0, src + src_index + 2 * (src_step / 4)));\n"
    "line[3] = (y == (rows - 2)) ? (uchar16)VAL: as_uchar16(vload4(0, src + "
    "src_index + 3 * (src_step / 4)));\n"
    "__global uchar *src_p = (__global uchar *)src;\n"
    "bool line_end = ((block_x + 1) * 16 == cols);\n"
    "src_index = block_x * 16 * ssx + (y - 1) * src_step;\n"
    ...
    "j = (block_x == 0 || y == (rows - 2)) ? VAL : src_p[src_index + 3 * "
    "src_step - 1];\n"
    "k = line[3];\n"
    "l = (line_end || y == (rows - 2)) ? VAL : src_p[src_index + 3 * src_step "
    "+ 16];\n"
    "uchar16 arr[12];\n"
    ...
    "#ifdef OP_GRADIENT\n"
    "line_out[0] = convert_uchar16_sat(convert_int16(line_out[0]) - "
    "convert_int16(val0));\n"
    "line_out[1] = convert_uchar16_sat(convert_int16(line_out[1]) - "
    "convert_int16(val1));\n"
    "vstore4(as_uint4(line_out[0]), 0, dst + dst_index);\n"
    "vstore4(as_uint4(line_out[1]), 0, dst + dst_index + (dst_step / 4));\n"
    "#elif defined OP_TOPHAT\n"
    "line_out[0] = convert_uchar16_sat(convert_int16(val0) - "
    "convert_int16(line_out[0]));\n"
    "line_out[1] = convert_uchar16_sat(convert_int16(val1) - "
    "convert_int16(line_out[1]));\n"
    "vstore4(as_uint4(line_out[0]), 0, dst + dst_index);\n"
    "vstore4(as_uint4(line_out[1]), 0, dst + dst_index + (dst_step / 4));\n"
    "#elif defined OP_BLACKHAT\n"
    "line_out[0] = convert_uchar16_sat(convert_int16(line_out[0]) - "
...
struct cv::ocl::internal::ProgramEntry precornerdetect_oclsrc = {
    moduleName, "precornerdetect",
    "__kernel void preCornerDetect(__global const uchar * Dxptr, int dx_step, "
    "int dx_offset,\n"
    "__global const uchar * Dyptr, int dy_step, int dy_offset,\n"
...
struct cv::ocl::internal::ProgramEntry pyr_down_oclsrc = {
    moduleName, "pyr_down",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#elif defined BORDER_REFLECT\n"
    "#define EXTRAPOLATE(x, maxV) clamp(min(((maxV)-1)*2-(x)+1, "
    "max((x),-(x)-1) ), 0, (maxV)-1)\n"
    "#elif defined BORDER_REFLECT_101 || defined BORDER_REFLECT101\n"
    "#define EXTRAPOLATE(x, maxV) clamp(min(((maxV)-1)*2-(x), max((x),-(x)) ), "
    "0, (maxV)-1)\n"
    "#else\n"
    "#error No extrapolation method\n"
    "#endif\n"
    ...
    "#endif\n"
    "#define SRC(_x,_y) convertToFT(loadpix(srcData + mad24(_y, src_step, "
    "PIXSIZE * _x)))\n"
    "#if kercn == 4\n"
    "#define SRC4(_x,_y) convert_float4(vload4(0, srcData + mad24(_y, "
    "src_step, PIXSIZE * _x)))\n"
    "#endif\n"
    "#ifdef INTEL_DEVICE\n"
    ...
    "  \\\n"
    "sum41 = MAD(co2, SRC4(col_gl,  EXTRAPOLATE_(src_y + 3, src_rows)), "
    "sum41);  \\\n"
    "sum41 = MAD(co3, SRC4(col_gl,  EXTRAPOLATE_(src_y + 4, src_rows)), "
    "sum41);  \\\n"
    "vstore4(sum41, col_lcl, (__local float*) &smem[1][2]);\n"
    "#endif\n"
    "#define noconvert\n"
    "__kernel void pyrDown(__global const uchar * src, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols)\n"
    "{\n"
    ...
    "if (get_local_id(0) < 2)\n"
    "{\n"
    "col = EXTRAPOLATE((int)(get_group_id(0)*LOCAL_SIZE + get_local_id(0) - "
    "2), src_cols);\n"
    "LOAD_LOCAL(col, get_local_id(0))\n"
    "}\n"
    "else if (get_local_id(0) < 4)\n"
    "{\n"
    "col = EXTRAPOLATE((int)((get_group_id(0)+1)*LOCAL_SIZE + get_local_id(0) "
    "- 2), src_cols);\n"
    ...
    "#if cn == 1\n"
    "#if fdepth <= 5\n"
    "FT sum = dot(vload4(0, (__local float*) (&smem) + tid2 + (yin - y) * "
    "(LOCAL_SIZE + 4)), (float4)(co3, co2, co1, co2));\n"
    "#else\n"
    "FT sum = dot(vload4(0, (__local double*) (&smem) + tid2 + (yin - y) * "
    "(LOCAL_SIZE + 4)), (double4)(co3, co2, co1, co2));\n"
    "#endif\n"
    "#else\n"
    ...
    "sum = MAD(co2, smem[yin - y][2 + tid4 + 1], sum);\n"
    "storepix(convertToT(sum), dstData + mad24(yin, dst_step, dst_x * "
    "PIXSIZE));\n"
    "dst_x ++;\n"
...
struct cv::ocl::internal::ProgramEntry pyr_up_oclsrc = {
    moduleName, "pyr_up",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#define noconvert\n"
    "__kernel void pyrUp(__global const uchar * src, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols)\n"
    "{\n"
    ...
    "{\n"
    "int srcx = EXTRAPOLATE(mad24((int)get_group_id(0), LOCAL_SIZE/2, tidx) - "
    "1, src_cols);\n"
    "int srcy = EXTRAPOLATE(mad24((int)get_group_id(1), LOCAL_SIZE/2, tidy) - "
    "1, src_rows);\n"
    "s_srcPatch[tidy][tidx] = convertToFT(loadpix(srcData + srcy * src_step + "
    "srcx * PIXSIZE));\n"
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "FT sum = 0.f;\n"
    ...
    "}\n"
    "__kernel void pyrUp_unrolled(__global const uchar * src, int src_step, "
    ...
    "s_srcPatch[ly][lx] = convertToFT(loadpix(srcData + srcy1 * src_step + "
    "srcx1 * PIXSIZE));\n"
    "s_srcPatch[ly+1][lx] = convertToFT(loadpix(srcData + srcy2 * src_step + "
    "srcx1 * PIXSIZE));\n"
    "s_srcPatch[ly][lx+1] = convertToFT(loadpix(srcData + srcy1 * src_step + "
    "srcx2 * PIXSIZE));\n"
    "s_srcPatch[ly+1][lx+1] = convertToFT(loadpix(srcData + srcy2 * src_step + "
    "srcx2 * PIXSIZE));\n"
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "FT sum;\n"
    ...
    "storepix(convertToT(sum), dstData + dst_y * dst_step + (dst_x+1) * "
    "PIXSIZE);\n"
    "sum =       co2 * s_dstPatch[1 + get_local_id(1)    ][lx];\n"
    "sum = mad(co2, s_dstPatch[1 + get_local_id(1) + 1][lx], sum);\n"
    "storepix(convertToT(sum), dstData + (dst_y+1) * dst_step + dst_x * "
    "PIXSIZE);\n"
    "sum =       co2 * s_dstPatch[1 + get_local_id(1)    ][lx+1];\n"
    "sum = mad(co2, s_dstPatch[1 + get_local_id(1) + 1][lx+1], sum);\n"
    "storepix(convertToT(sum), dstData + (dst_y+1) * dst_step + (dst_x+1) * "
    "PIXSIZE);\n"
    "}\n"
    "}\n",
    "e48abb0036bd5e090ad06600b018eec9", NULL};
struct cv::ocl::internal::ProgramEntry pyramid_up_oclsrc = {
    moduleName, "pyramid_up",
    "__constant float kx[] = { 0.125, 0.5, 0.75, 0.5, 0.125 };\n"
    "__constant float ky[] = { 0.125, 0.5, 0.75, 0.5, 0.125 };\n"
    "#define OP(delta, y, x) (convert_float4(arr[(y + delta) * 5 + x]) * ky[y] "
    "* kx[x])\n"
    "__kernel void pyrUp_cols2(__global const uchar * src, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols)\n"
    "{\n"
    ...
    "float4 sum[2];\n"
    "sum[0] = OP(0, 0, 0) + OP(0, 0, 1) + OP(0, 0, 2) + OP(0, 0, 3) + OP(0, 0, "
    "4) +\n"
    ...
    "sum[1] = OP(1, 0, 0) + OP(1, 0, 1) + OP(1, 0, 2) + OP(1, 0, 3) + OP(1, 0, "
    "4) +\n"
...
struct cv::ocl::internal::ProgramEntry remap_oclsrc = {
    moduleName, "remap",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "v = convertToWT(loadpix((__global const T*)(srcptr + mad24(v2.y, "
    "src_step, v2.x * TSIZE + src_offset)))); \\\n"
    "}\n"
    "#elif defined BORDER_WRAP\n"
    "#define EXTRAPOLATE(v2, v) \\\n"
    "{ \\\n"
    ...
    "#error No extrapolation method\n"
    "#endif\n"
    "#define NEED_EXTRAPOLATION(gx, gy) (gx >= src_cols || gy >= src_rows || "
    "gx < 0 || gy < 0)\n"
    "#ifdef INTER_NEAREST\n"
    "__kernel void remap_2_32FC1(__global const uchar * srcptr, int src_step, "
    "int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "int map1_index = mad24(y, map1_step, mad24(x, (int)sizeof(float), "
    "map1_offset));\n"
    "int map2_index = mad24(y, map2_step, mad24(x, (int)sizeof(float), "
    "map2_offset));\n"
    "int dst_index = mad24(y, dst_step, mad24(x, TSIZE, dst_offset));\n"
    "#pragma unroll\n"
    ...
    "if (y < dst_rows)\n"
    "{\n"
    "__global const float * map1 = (__global const float *)(map1ptr + "
    "map1_index);\n"
    "__global const float * map2 = (__global const float *)(map2ptr + "
    "map2_index);\n"
    "__global T * dst = (__global T *)(dstptr + dst_index);\n"
    ...
    "}\n"
    "}\n"
    "__kernel void remap_32FC2(__global const uchar * srcptr, int src_step, "
    "int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "T scalar = convertScalar(nVal);\n"
    "int dst_index = mad24(y, dst_step, mad24(x, TSIZE, dst_offset));\n"
    "int map_index = mad24(y, map_step, mad24(x, (int)sizeof(float2), "
    "map_offset));\n"
    "#pragma unroll\n"
    "for (int i = 0; i < rowsPerWI; ++i, ++y,\n"
    ...
    "if (y < dst_rows)\n"
    "{\n"
    "__global const float2 * map = (__global const float2 *)(mapptr + "
    "map_index);\n"
    "__global T * dst = (__global T *)(dstptr + dst_index);\n"
    ...
    "}\n"
    "}\n"
    "__kernel void remap_16SC2(__global const uchar * srcptr, int src_step, "
    "int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__global const uchar * mapptr, int map_step, int map_offset,\n"
    ...
    "T scalar = convertScalar(nVal);\n"
    "int dst_index = mad24(y, dst_step, mad24(x, TSIZE, dst_offset));\n"
    "int map_index = mad24(y, map_step, mad24(x, (int)sizeof(short2), "
    "map_offset));\n"
    "#pragma unroll\n"
    "for (int i = 0; i < rowsPerWI; ++i, ++y,\n"
    "map_index += map_step, dst_index += dst_step)\n"
    "if (y < dst_rows)\n"
    "{\n"
    "__global const short2 * map = (__global const short2 *)(mapptr + "
    "map_index);\n"
    "__global T * dst = (__global T *)(dstptr + dst_index);\n"
    ...
    "}\n"
    "}\n"
    "__kernel void remap_16SC2_16UC1(__global const uchar * srcptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__global const uchar * map1ptr, int map1_step, int map1_offset,\n"
    ...
    "T scalar = convertScalar(nVal);\n"
    "int dst_index = mad24(y, dst_step, mad24(x, TSIZE, dst_offset));\n"
    "int map1_index = mad24(y, map1_step, mad24(x, (int)sizeof(short2), "
    "map1_offset));\n"
    "int map2_index = mad24(y, map2_step, mad24(x, (int)sizeof(ushort), "
    "map2_offset));\n"
    "#pragma unroll\n"
    "for (int i = 0; i < rowsPerWI; ++i, ++y,\n"
    "map1_index += map1_step, map2_index += map2_step, dst_index += dst_step)\n"
    "if (y < dst_rows)\n"
    "{\n"
    "__global const short2 * map1 = (__global const short2 *)(map1ptr + "
    "map1_index);\n"
    "__global const ushort * map2 = (__global const ushort *)(map2ptr + "
    "map2_index);\n"
    "__global T * dst = (__global T *)(dstptr + dst_index);\n"
    "int map2Value = convert_int(map2[0]) & (INTER_TAB_SIZE2 - 1);\n"
    "int dx = (map2Value & (INTER_TAB_SIZE - 1)) < (INTER_TAB_SIZE >> 1) ? 1 : "
    "0;\n"
    ...
    "{ 1.000000f, 0.000000f, 0.968750f, 0.031250f, 0.937500f, 0.062500f, "
    "0.906250f, 0.093750f, 0.875000f, 0.125000f, 0.843750f, 0.156250f,\n"
    "0.812500f, 0.187500f, 0.781250f, 0.218750f, 0.750000f, 0.250000f, "
    "0.718750f, 0.281250f, 0.687500f, 0.312500f, 0.656250f, 0.343750f,\n"
    "0.625000f, 0.375000f, 0.593750f, 0.406250f, 0.562500f, 0.437500f, "
    "0.531250f, 0.468750f, 0.500000f, 0.500000f, 0.468750f, 0.531250f,\n"
    "0.437500f, 0.562500f, 0.406250f, 0.593750f, 0.375000f, 0.625000f, "
    "0.343750f, 0.656250f, 0.312500f, 0.687500f, 0.281250f, 0.718750f,\n"
    "0.250000f, 0.750000f, 0.218750f, 0.781250f, 0.187500f, 0.812500f, "
    "0.156250f, 0.843750f, 0.125000f, 0.875000f, 0.093750f, 0.906250f,\n"
    ...
    "WT2 u = (WT2)(map2Value & (INTER_TAB_SIZE - 1), map2Value >> INTER_BITS) "
    "/ (WT2)(INTER_TAB_SIZE);\n"
    "WT a = scalar, b = scalar, c = scalar, d = scalar;\n"
    "if (!NEED_EXTRAPOLATION(map_dataA.x, map_dataA.y))\n"
    "a = convertToWT(loadpix((__global const T *)(srcptr + mad24(map_dataA.y, "
    "src_step, map_dataA.x * TSIZE + src_offset))));\n"
    "else\n"
    "EXTRAPOLATE(map_dataA, a);\n"
    "if (!NEED_EXTRAPOLATION(map_dataB.x, map_dataB.y))\n"
    "b = convertToWT(loadpix((__global const T *)(srcptr + mad24(map_dataB.y, "
    "src_step, map_dataB.x * TSIZE + src_offset))));\n"
    "else\n"
    "EXTRAPOLATE(map_dataB, b);\n"
    "if (!NEED_EXTRAPOLATION(map_dataC.x, map_dataC.y))\n"
    "c = convertToWT(loadpix((__global const T *)(srcptr + mad24(map_dataC.y, "
    "src_step, map_dataC.x * TSIZE + src_offset))));\n"
    "else\n"
    "EXTRAPOLATE(map_dataC, c);\n"
    "if (!NEED_EXTRAPOLATION(map_dataD.x, map_dataD.y))\n"
    "d = convertToWT(loadpix((__global const T *)(srcptr + mad24(map_dataD.y, "
    "src_step, map_dataD.x * TSIZE + src_offset))));\n"
    "else\n"
    ...
    "int sx = convert_int_sat_rtz(mad(xf, (float)INTER_TAB_SIZE, 0.5f)) >> "
    "INTER_BITS;\n"
    "int sy = convert_int_sat_rtz(mad(yf, (float)INTER_TAB_SIZE, 0.5f)) >> "
    "INTER_BITS;\n"
    "__constant float * coeffs_x = coeffs + ((convert_int_rte(xf * "
    "INTER_TAB_SIZE) & (INTER_TAB_SIZE - 1)) << 1);\n"
    "__constant float * coeffs_y = coeffs + ((convert_int_rte(yf * "
    "INTER_TAB_SIZE) & (INTER_TAB_SIZE - 1)) << 1);\n"
    "WT sum = (WT)(0), xsum;\n"
    ...
    "xsum = fma(convertToWT(loadpix(srcptr + mad24(xp, TSIZE, src_index))), "
    "coeffs_x[xp], xsum);\n"
    "#endif\n"
    "}\n"
    "else\n"
    ...
    "convertToWT(loadpix(srcptr + mad24(xp, TSIZE, src_index))) : scalar, "
    "coeffs_x[xp], xsum);\n"
    "}\n"
    ...
    "WT2 u = convertToWT2(convert_int2_rte(convertToWT2(_u) * "
    "(WT2)INTER_TAB_SIZE)) / (WT2)INTER_TAB_SIZE;\n"
    "WT scalar = convertToWT(convertScalar(nVal));\n"
    "WT a = scalar, b = scalar, c = scalar, d = scalar;\n"
    "if (!NEED_EXTRAPOLATION(map_dataA.x, map_dataA.y))\n"
    "a = convertToWT(loadpix((__global const T *)(srcptr + mad24(map_dataA.y, "
...
struct cv::ocl::internal::ProgramEntry resize_oclsrc = {
    moduleName, "resize",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "INTERMEDIATE_TYPE intermediate = READ_IMAGE(srcImage, sampler, "
    "(float2)(sx, sy));\n"
    "#if depth <= 4\n"
    ...
    "__kernel void resizeLN(__global const uchar * srcptr, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "{\n"
    "__global const int * xofs = (__global const int *)(buffer), * yofs = xofs "
    "+ dst_cols;\n"
    "__global const short * ialpha = (__global const short *)(yofs + "
    "dst_rows);\n"
    ...
    "WT data0 = convertToWT(loadpix(srcptr + mad24(y, src_step, mad24(x, "
    "TSIZE, src_offset))));\n"
    "WT data1 = convertToWT(loadpix(srcptr + mad24(y, src_step, mad24(x_, "
    "TSIZE, src_offset))));\n"
    "WT data2 = convertToWT(loadpix(srcptr + mad24(y_, src_step, mad24(x, "
    "TSIZE, src_offset))));\n"
    "WT data3 = convertToWT(loadpix(srcptr + mad24(y_, src_step, mad24(x_, "
    "TSIZE, src_offset))));\n"
    ...
    "WT data3 = convertToWT(loadpix(srcptr + mad24(y_, src_step, mad24(x_, "
    "TSIZE, src_offset))));\n"
    "T uval = u1 * v1 * data0 + u * v1 * data1 + u1 * v *data2 + u * v "
    "*data3;\n"
    "#endif\n"
    "storepix(uval, dstptr + mad24(dy, dst_step, mad24(dx, TSIZE, "
    "dst_offset)));\n"
    "}\n"
    "}\n"
    "#elif defined INTER_NEAREST\n"
    "__kernel void resizeNN(__global const uchar * srcptr, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "float ifx, float ify)\n"
    ...
    "storepix(loadpix(srcptr + mad24(sy, src_step, mad24(sx, TSIZE, "
    "src_offset))),\n"
    "dstptr + mad24(dy, dst_step, mad24(dx, TSIZE, dst_offset)));\n"
    "}\n"
    "}\n"
    ...
    "__kernel void resizeAREA_FAST(__global const uchar * src, int src_step, "
    "int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "storepix(convertToT(convertToWT2V(sum) * (WT2V)(SCALE)), dst + mad24(dx, "
    "TSIZE, dst_index));\n"
    "}\n"
    "}\n"
    "#else\n"
    "__kernel void resizeAREA(__global const uchar * src, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "__global const int * ymap_tab = (__global const int *)(map_tab + "
    "(src_cols << 1));\n"
    "__global const float * xalpha_tab = alpha_tab;\n"
    "__global const float * yalpha_tab = (__global const float *)(alpha_tab + "
    "(src_cols << 1));\n"
    "__global const int * xofs_tab = ofs_tab;\n"
    "__global const int * yofs_tab = (__global const int *)(ofs_tab + dst_cols "
    "+ 1);\n"
    ...
    "for (int sy = sy0, yk = yk0; sy <= sy1; ++sy, src_index += src_step, "
    "++yk)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry sepFilter3x3_oclsrc = {
    moduleName, "sepFilter3x3",
    "#define DIG(a) a,\n"
    "__constant float kx[] = { KERNEL_MATRIX_X };\n"
    "__constant float ky[] = { KERNEL_MATRIX_Y };\n"
    "#define OP(delta, y, x) (convert_float16(arr[(y + delta) * 3 + x]) * "
    "ky[y] * kx[x])\n"
    "__kernel void sepFilter3x3_8UC1_cols16_rows2(__global const uint* src, "
    "int src_step,\n"
    "__global uint* dst, int dst_step,\n"
    "int rows, int cols, float delta)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry threshold_oclsrc = {
    ...
    "#endif\n"
    "#endif\n"
    "__kernel void threshold(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int rows, int "
    "cols,\n"
    ...
    "if (gx < cols)\n"
    "{\n"
    "int src_index = mad24(gy, src_step, mad24(gx, (int)sizeof(T), "
    "src_offset));\n"
    "int dst_index = mad24(gy, dst_step, mad24(gx, (int)sizeof(T), "
    "dst_offset));\n"
    "#pragma unroll\n"
...
struct cv::ocl::internal::ProgramEntry warp_affine_oclsrc = {
    moduleName, "warp_affine",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "#endif\n"
    "#ifdef INTER_NEAREST\n"
    "__kernel void warpAffine(__global const uchar * srcptr, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "short sx = convert_short_sat(X0 >> INTER_BITS), sy = convert_short_sat(Y0 "
    ">> INTER_BITS);\n"
    "short ax = convert_short(X0 & (INTER_TAB_SIZE-1)), ay = convert_short(Y0 "
    "& (INTER_TAB_SIZE-1));\n"
    ...
    "{\n"
    "if (sy >= 0 && sy < src_rows)\n"
    "v0 = convertToWT(loadpix(srcptr + mad24(sy, src_step, mad24(sx, pixsize, "
    "src_offset))));\n"
    "if (sy+1 >= 0 && sy+1 < src_rows)\n"
    "v2 = convertToWT(loadpix(srcptr + mad24(sy+1, src_step, mad24(sx, "
    "pixsize, src_offset))));\n"
    "}\n"
    "if (sx+1 >= 0 && sx+1 < src_cols)\n"
    "{\n"
    "if (sy >= 0 && sy < src_rows)\n"
    "v1 = convertToWT(loadpix(srcptr + mad24(sy, src_step, mad24(sx+1, "
    "pixsize, src_offset))));\n"
    "if (sy+1 >= 0 && sy+1 < src_rows)\n"
    "v3 = convertToWT(loadpix(srcptr + mad24(sy+1, src_step, mad24(sx+1, "
    "pixsize, src_offset))));\n"
    "}\n"
    ...
    "#if depth <= 4\n"
    "int itab0 = convert_short_sat_rte( (1.0f-taby)*(1.0f-tabx) * "
    "INTER_REMAP_COEF_SCALE );\n"
    "int itab1 = convert_short_sat_rte( (1.0f-taby)*tabx * "
    "INTER_REMAP_COEF_SCALE );\n"
    "int itab2 = convert_short_sat_rte( taby*(1.0f-tabx) * "
    "INTER_REMAP_COEF_SCALE );\n"
    "int itab3 = convert_short_sat_rte( taby*tabx * INTER_REMAP_COEF_SCALE );\n"
    "WT val = mad24(v0, itab0, mad24(v1, itab1, mad24(v2, itab2, v3 * "
    "itab3)));\n"
    "storepix(convertToT((val + (1 << (INTER_REMAP_COEF_BITS-1))) >> "
    "INTER_REMAP_COEF_BITS), dstptr + dst_index);\n"
    "#else\n"
    "float tabx2 = 1.0f - tabx, taby2 = 1.0f - taby;\n"
    "WT val = fma(tabx2, fma(v0, taby2, v2 * taby), tabx * fma(v1, taby2, v3 * "
    "taby));\n"
    "storepix(convertToT(val), dstptr + dst_index);\n"
    "#endif\n"
    "#else\n"
    "__constant float * coeffs_y = coeffs + (ay << 1), * coeffs_x = coeffs + "
    "(ax << 1);\n"
    "int src_index0 = mad24(sy, src_step, mad24(sx, pixsize, src_offset)), "
    "src_index;\n"
    "int dst_index = mad24(dy, dst_step, mad24(dx, pixsize, dst_offset));\n"
    "WT sum = (WT)(0), xsum;\n"
    "#pragma unroll\n"
    ...
    "#pragma unroll\n"
    "for (int x = 0; x < 2; x++)\n"
    "xsum = fma(convertToWT(loadpix(srcptr + mad24(x, pixsize, src_index))), "
    "coeffs_x[x], xsum);\n"
    "#endif\n"
    "}\n"
    "else\n"
    ...
    "convertToWT(loadpix(srcptr + mad24(x, pixsize, src_index))) : scalar, "
    "coeffs_x[x], xsum);\n"
    "}\n"
    ...
    "{\n"
    "const float A = -0.75f;\n"
    "coeffs[0] = fma(fma(fma(A, (x + 1.f), - 5.0f*A), (x + 1.f), 8.0f*A), x + "
    "1.f, - 4.0f*A);\n"
    "coeffs[1] = fma(fma(A + 2.f, x, - (A + 3.f)), x*x, 1.f);\n"
    "coeffs[2] = fma(fma(A + 2.f, 1.f - x, - (A + 3.f)), (1.f - x)*(1.f - x), "
    "1.f);\n"
    "coeffs[3] = 1.f - coeffs[0] - coeffs[1] - coeffs[2];\n"
    "}\n"
    "#else\n"
    "__constant float coeffs[128] =\n"
    "{ 0.000000f, 1.000000f, 0.000000f, 0.000000f, -0.021996f, 0.997841f, "
    "0.024864f, -0.000710f, -0.041199f, 0.991516f, 0.052429f, -0.002747f,\n"
    "-0.057747f, 0.981255f, 0.082466f, -0.005974f, -0.071777f, 0.967285f, "
    "0.114746f, -0.010254f, -0.083427f, 0.949837f, 0.149040f, -0.015450f,\n"
    "-0.092834f, 0.929138f, 0.185120f, -0.021423f, -0.100136f, 0.905418f, "
    "0.222755f, -0.028038f, -0.105469f, 0.878906f, 0.261719f, -0.035156f,\n"
    "-0.108971f, 0.849831f, 0.301781f, -0.042641f, -0.110779f, 0.818420f, "
    "0.342712f, -0.050354f, -0.111031f, 0.784904f, 0.384285f, -0.058159f,\n"
    "-0.109863f, 0.749512f, 0.426270f, -0.065918f, -0.107414f, 0.712471f, "
    "0.468437f, -0.073494f, -0.103821f, 0.674011f, 0.510559f, -0.080750f,\n"
    "-0.099220f, 0.634361f, 0.552406f, -0.087547f, -0.093750f, 0.593750f, "
    "0.593750f, -0.093750f, -0.087547f, 0.552406f, 0.634361f, -0.099220f,\n"
    "-0.080750f, 0.510559f, 0.674011f, -0.103821f, -0.073494f, 0.468437f, "
    "0.712471f, -0.107414f, -0.065918f, 0.426270f, 0.749512f, -0.109863f,\n"
    "-0.058159f, 0.384285f, 0.784904f, -0.111031f, -0.050354f, 0.342712f, "
    "0.818420f, -0.110779f, -0.042641f, 0.301781f, 0.849831f, -0.108971f,\n"
    "-0.035156f, 0.261719f, 0.878906f, -0.105469f, -0.028038f, 0.222755f, "
    "0.905418f, -0.100136f, -0.021423f, 0.185120f, 0.929138f, -0.092834f,\n"
    "-0.015450f, 0.149040f, 0.949837f, -0.083427f, -0.010254f, 0.114746f, "
    "0.967285f, -0.071777f, -0.005974f, 0.082466f, 0.981255f, -0.057747f,\n"
    "-0.002747f, 0.052429f, 0.991516f, -0.041199f, -0.000710f, 0.024864f, "
    "0.997841f, -0.021996f };\n"
    "#endif\n"
    "__kernel void warpAffine(__global const uchar * srcptr, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    ...
    "{\n"
    "int tmp = (dx << AB_BITS);\n"
    "int X0 = rint(M[0] * tmp) + rint(fma(M[1], (CT)dy, M[2]) * AB_SCALE) + "
    "ROUND_DELTA;\n"
    "int Y0 = rint(M[3] * tmp) + rint(fma(M[4], (CT)dy, M[5]) * AB_SCALE) + "
    "ROUND_DELTA;\n"
    "X0 = X0 >> (AB_BITS - INTER_BITS);\n"
    "Y0 = Y0 >> (AB_BITS - INTER_BITS);\n"
    "int sx = (short)(X0 >> INTER_BITS) - 1, sy = (short)(Y0 >> INTER_BITS) - "
    "1;\n"
    "int ay = (short)(Y0 & (INTER_TAB_SIZE - 1)), ax = (short)(X0 & "
    "(INTER_TAB_SIZE - 1));\n"
    "#ifdef AMD_DEVICE\n"
    "WT v[16];\n"
    "#pragma unroll\n"
    ...
    "convertToWT(loadpix(srcptr + mad24(sy+y, src_step, mad24(sx+x, pixsize, "
    "src_offset)))) : scalar;\n"
    "}\n"
    "else\n"
    "{\n"
    ...
    "storepix(convertToT( (sum + (1 << (INTER_REMAP_COEF_BITS-1))) >> "
    "INTER_REMAP_COEF_BITS ), dstptr + dst_index);\n"
    "#else\n"
    "#pragma unroll\n"
    "for (int i = 0; i < 16; i++)\n"
    ...
    "#endif\n"
    "#else\n"
    "__constant float * coeffs_y = coeffs + (ay << 2), * coeffs_x = coeffs + "
    "(ax << 2);\n"
    "int src_index0 = mad24(sy, src_step, mad24(sx, pixsize, src_offset)), "
    "src_index;\n"
    "int dst_index = mad24(dy, dst_step, mad24(dx, pixsize, dst_offset));\n"
    ...
    "xsum = dot(convert_float4(value), (float4)(coeffs_x[0], coeffs_x[1], "
    "coeffs_x[2], coeffs_x[3]));\n"
    "#else\n"
    "#pragma unroll\n"
    "for (int x = 0; x < 4; x++)\n"
    "xsum = fma(convertToWT(loadpix(srcptr + mad24(x, pixsize, src_index))), "
...
struct cv::ocl::internal::ProgramEntry warp_perspective_oclsrc = {
    moduleName, "warp_perspective",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "#endif\n"
    "#ifdef INTER_NEAREST\n"
    "__kernel void warpPerspective(__global const uchar * srcptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__constant CT * M, ST scalar_)\n"
    ...
    "convertToWT(loadpix(srcptr + mad24(sy, src_step, src_offset + sx * "
    "pixsize))) : scalar;\n"
    "WT v1 = (sx+1 >= 0 && sx+1 < src_cols && sy >= 0 && sy < src_rows) ?\n"
    "convertToWT(loadpix(srcptr + mad24(sy, src_step, src_offset + (sx+1) * "
    "pixsize))) : scalar;\n"
    "WT v2 = (sx >= 0 && sx < src_cols && sy+1 >= 0 && sy+1 < src_rows) ?\n"
    "convertToWT(loadpix(srcptr + mad24(sy+1, src_step, src_offset + sx * "
    "pixsize))) : scalar;\n"
    "WT v3 = (sx+1 >= 0 && sx+1 < src_cols && sy+1 >= 0 && sy+1 < src_rows) ?\n"
    "convertToWT(loadpix(srcptr + mad24(sy+1, src_step, src_offset + (sx+1) * "
    "pixsize))) : scalar;\n"
    "float taby = 1.f/INTER_TAB_SIZE*ay;\n"
    "float tabx = 1.f/INTER_TAB_SIZE*ax;\n"
    ...
    "#else\n"
    "float tabx2 = 1.0f - tabx, taby2 = 1.0f - taby;\n"
    "WT val = v0 * tabx2 * taby2 +  v1 * tabx * taby2 + v2 * tabx2 * taby + v3 "
    "* tabx * taby;\n"
    "storepix(convertToT(val), dstptr + dst_index);\n"
    "#endif\n"
    "}\n"
    ...
    "{\n"
    "const float A = -0.75f;\n"
    "coeffs[0] = ((A*(x + 1.f) - 5.0f*A)*(x + 1.f) + 8.0f*A)*(x + 1.f) - "
    "4.0f*A;\n"
    ...
    "#pragma unroll\n"
    "for (int x = 0; x < 4; x++)\n"
    "v[mad24(y, 4, x)] = (sx+x >= 0 && sx+x < src_cols && sy+y >= 0 && sy+y < "
    "src_rows) ?\n"
    "convertToWT(loadpix(srcptr + mad24(sy+y, src_step, src_offset + (sx+x) * "
    "pixsize))) : scalar;\n"
    "float tab1y[4], tab1x[4];\n"
    "float ayy = INTER_SCALE * ay;\n"
    "float axx = INTER_SCALE * ax;\n"
...
struct cv::ocl::internal::ProgramEntry warp_transform_oclsrc = {
    moduleName, "warp_transform",
    "__constant short4 vec_offset = (short4)(0, 1, 2, 3);\n"
    "#define GET_VAL(x, y) ((x) < 0 || (x) >= src_cols || (y) < 0 || (y) >= "
    "src_rows) ? scalar : src[src_offset + y * src_step + x]\n"
    "__kernel void warpAffine_nearest_8u(__global const uchar * src, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    ...
    "short4 new_x, new_y;\n"
    "new_x = convert_short4_sat_rte(M[0] * convert_float4(vec_offset + "
    "(short4)(x)) +\n"
    "M[1] * convert_float4((short4)y) + M[2]);\n"
    "new_y = convert_short4_sat_rte(M[3] * convert_float4(vec_offset + "
    "(short4)(x)) +\n"
    ...
    "return (uchar4)(pt, pb);\n"
    "}\n"
    "__kernel void warpAffine_linear_8u(__global const uchar * src, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__constant float * M, ST scalar_)\n"
    ...
    "if (x >= dst_cols || y >= dst_rows) return;\n"
    "float4 nx, ny;\n"
    "nx = M[0] * convert_float4((vec_offset + (short4)x)) + M[1] * "
    "convert_float4((short4)y) + M[2];\n"
    "ny = M[3] * convert_float4((vec_offset + (short4)x)) + M[4] * "
    "convert_float4((short4)y) + M[5];\n"
    "float4 s, t;\n"
    ...
    "uchar4 pix[4];\n"
    "pix[0] = read_pixels(src, tx.s0, ty.s0, src_offset, src_step, src_cols, "
    "src_rows, scalar);\n"
    "pix[1] = read_pixels(src, tx.s1, ty.s1, src_offset, src_step, src_cols, "
    "src_rows, scalar);\n"
    "pix[2] = read_pixels(src, tx.s2, ty.s2, src_offset, src_step, src_cols, "
    "src_rows, scalar);\n"
    "pix[3] = read_pixels(src, tx.s3, ty.s3, src_offset, src_step, src_cols, "
    "src_rows, scalar);\n"
    "float4 tl, tr, bl, br;\n"
    "tl = convert_float4((uchar4)(pix[0].s0, pix[1].s0, pix[2].s0, "
    "pix[3].s0));\n"
    "tr = convert_float4((uchar4)(pix[0].s1, pix[1].s1, pix[2].s1, "
    "pix[3].s1));\n"
    "bl = convert_float4((uchar4)(pix[0].s2, pix[1].s2, pix[2].s2, "
    "pix[3].s2));\n"
    "br = convert_float4((uchar4)(pix[0].s3, pix[1].s3, pix[2].s3, "
    "pix[3].s3));\n"
    "float4 pixel;\n"
    "pixel = tl * (1 - s) * (1 - t) + tr * s * (1 - t) + bl * (1 - s) * t + br "
    "* s * t;\n"
    "int dst_index = x + y * dst_step + dst_offset;\n"
    ...
    "return pix;\n"
    "}\n"
    "__kernel void warpAffine_cubic_8u(__global const uchar * src, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__constant float * M, ST scalar_)\n"
    ...
    "for (int i = 0; i < 4; i++)\n"
    "{\n"
    "pix = read_pixels_cubic(src, tx.s0, ty.s0 + i, src_offset, src_step, "
    "src_cols, src_rows, scalar);\n"
    "xsum = dot(convert_float4(pix), (float4)(coeffs_x[0], coeffs_x[1], "
    "coeffs_x[2], coeffs_x[3]));\n"
    ...
    "for (int i = 0; i < 4; i++)\n"
    "{\n"
    "pix = read_pixels_cubic(src, tx.s1, ty.s1 + i, src_offset, src_step, "
    "src_cols, src_rows, scalar);\n"
    "xsum = dot(convert_float4(pix), (float4)(coeffs_x[0], coeffs_x[1], "
    "coeffs_x[2], coeffs_x[3]));\n"
    ...
    "for (int i = 0; i < 4; i++)\n"
    "{\n"
    "pix = read_pixels_cubic(src, tx.s2, ty.s2 + i, src_offset, src_step, "
    "src_cols, src_rows, scalar);\n"
    "xsum = dot(convert_float4(pix), (float4)(coeffs_x[0], coeffs_x[1], "
    "coeffs_x[2], coeffs_x[3]));\n"
    ...
    "for (int i = 0; i < 4; i++)\n"
    "{\n"
    "pix = read_pixels_cubic(src, tx.s3, ty.s3 + i, src_offset, src_step, "
    "src_cols, src_rows, scalar);\n"
    "xsum = dot(convert_float4(pix), (float4)(coeffs_x[0], coeffs_x[1], "
    "coeffs_x[2], coeffs_x[3]));\n"
    ...
    "}\n"
    "__kernel void warpPerspective_nearest_8u(__global const uchar * src, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__constant float * M, ST scalar_)\n"
    ...
    "vstore4(pix, 0,  dst + dst_index);\n"
    "}\n"
    "__kernel void warpPerspective_linear_8u(__global const uchar * src, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__constant float * M, ST scalar_)\n"
    ...
    "ny = M[3] * convert_float4(vec_offset + (short4)(x)) + M[4] * "
    "convert_float4((short4)y) + M[5];\n"
    "nz = M[6] * convert_float4(vec_offset + (short4)(x)) + M[7] * "
    "convert_float4((short4)y) + M[8];\n"
    "float4 fz = select((float4)(0.0f), (float4)(1.0f / nz), nz != 0.0f);\n"
    "nx = nx * fz;\n"
    "ny = ny * fz;\n"
    "float4 s, t;\n"
    ...
    "vstore4(convert_uchar4_sat_rte(pixel), 0,  dst + dst_index);\n"
    "}\n"
    "__kernel void warpPerspective_cubic_8u(__global const uchar * src, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dst, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__constant float * M, ST scalar_)\n"
