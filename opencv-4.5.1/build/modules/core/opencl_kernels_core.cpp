...
struct cv::ocl::internal::ProgramEntry arithm_oclsrc = {
    moduleName, "arithm",
    ...
    "#endif\n"
    "#if defined(depth)\n"
    "#error \"Kernel configuration error: ambiguous 'depth' value is defined, "
    "use 'DEPTH_dst' instead\"\n"
    "#endif\n"
    "#if DEPTH_dst < 5 \n"
    ...
    "#else\n"
    "#define storedst(val) vstore3(val, 0, (__global dstT_C1 *)(dstptr + "
    "dst_index))\n"
    "#define storedst2(val) vstore3(val, 0, (__global dstT_C1 *)(dstptr2 + "
    "dst_index2))\n"
    "#endif\n"
    "#define noconvert\n"
    "#ifndef workT\n"
    ...
    "#else\n"
    "#define srcelem1 convertToWT1(vload3(0, (__global srcT1_C1 *)(srcptr1 + "
    "src1_index)))\n"
    "#define srcelem2 convertToWT2(vload3(0, (__global srcT2_C1 *)(srcptr2 + "
    "src2_index)))\n"
    "#endif\n"
    "#endif\n"
    "#ifndef workST\n"
    ...
    "#define PROCESS_ELEM \\\n"
    "workT e2 = srcelem2, zero = (workT)(0); \\\n"
    "storedst(convertToDT(e2 == zero ? zero : (srcelem1 * (workT)(scale) / "
    "e2)))\n"
    "#else\n"
    "#define PROCESS_ELEM \\\n"
    "workT e2 = srcelem2; \\\n"
    ...
    "storedst(convertToDT(e1 == zero ? zero : (srcelem2 * (workT)(scale) / "
    "e1)))\n"
    "#else\n"
    "#define PROCESS_ELEM \\\n"
    "workT e1 = srcelem1; \\\n"
    ...
    "#if wdepth <= 4\n"
    "#define PROCESS_ELEM storedst(convertToDT(mad24(srcelem1, alpha, "
    "mad24(srcelem2, beta, gamma))))\n"
    "#else\n"
    "#define PROCESS_ELEM storedst(convertToDT(fma(srcelem1, alpha, "
    "fma(srcelem2, beta, gamma))))\n"
    "#endif\n"
    "#elif defined OP_MAG\n"
    ...
    "#if wdepth <= 4\n"
    "#define PROCESS_ELEM storedst(convertToDT(mad24(srcelem1, (workT)(alpha), "
    "srcelem2)))\n"
    "#else\n"
    "#define PROCESS_ELEM storedst(convertToDT(fma(srcelem1, (workT)(alpha), "
    "srcelem2)))\n"
    "#endif\n"
    ...
    "dstT cartToPolar = y2 <= x2 ? x * y / mad((dstT)(0.28f), y2, x2 + "
    "CV_EPSILON) + tmp : (tmp1 - x * y / mad((dstT)(0.28f), x2, y2 + "
    "CV_EPSILON)); \\\n"
    "TO_DEGREE \\\n"
    "storedst(magnitude); \\\n"
    ...
    "#endif\n"
    "#if defined OP_CTP_AD || defined OP_CTP_AR || defined OP_PTC_AD || "
    "defined OP_PTC_AR\n"
    "#undef EXTRA_PARAMS\n"
    "#define EXTRA_PARAMS , __global uchar* dstptr2, int dststep2, int "
    "dstoffset2\n"
    "#undef EXTRA_INDEX\n"
    "#define EXTRA_INDEX int dst_index2 = mad24(y0, dststep2, mad24(x, "
    "(int)sizeof(dstT_C1) * cn, dstoffset2))\n"
    "#undef EXTRA_INDEX_ADD\n"
    "#define EXTRA_INDEX_ADD dst_index2 += dststep2\n"
    "#endif\n"
    "#if defined UNARY_OP || defined MASK_UNARY_OP\n"
    "#if defined OP_AND || defined OP_OR || defined OP_XOR || defined OP_ADD "
    "|| defined OP_SAT_ADD || \\\n"
    "defined OP_SUB || defined OP_SAT_SUB || defined OP_RSUB || defined "
    "OP_SAT_RSUB || \\\n"
    "defined OP_ABSDIFF || defined OP_CMP || defined OP_MIN || defined OP_MAX "
    "|| defined OP_POW || \\\n"
    "defined OP_MUL || defined OP_DIV || defined OP_POWN || defined OP_POWR || "
    "defined OP_ROOTN\n"
    "#undef EXTRA_PARAMS\n"
    ...
    "__kernel void KF(__global const uchar * srcptr1, int srcstep1, int "
    "srcoffset1,\n"
    ...
    "if (x < cols)\n"
    "{\n"
    "int src1_index = mad24(y0, srcstep1, mad24(x, (int)sizeof(srcT1_C1) * cn, "
    "srcoffset1));\n"
    "#if !(defined(OP_RECIP_SCALE) || defined(OP_NOT))\n"
    "int src2_index = mad24(y0, srcstep2, mad24(x, (int)sizeof(srcT2_C1) * cn, "
    "srcoffset2));\n"
    "#endif\n"
    "int dst_index  = mad24(y0, dststep, mad24(x, (int)sizeof(dstT_C1) * cn, "
    "dstoffset));\n"
    "EXTRA_INDEX;\n"
    "for (int y = y0, y1 = min(rows, y0 + rowsPerWI); y < y1; ++y, src1_index "
    "+= srcstep1, dst_index += dststep)\n"
    "{\n"
    "PROCESS_ELEM;\n"
    "#if !(defined(OP_RECIP_SCALE) || defined(OP_NOT))\n"
    "src2_index += srcstep2;\n"
    ...
    "dstoffset));\n"
    "for (int y = y0, y1 = min(rows, y0 + rowsPerWI); y < y1; ++y, src1_index "
    "+= srcstep1, mask_index += maskstep, dst_index += dststep)\n"
    "if (mask[mask_index])\n"
    "{\n"
    "PROCESS_ELEM;\n"
    "}\n"
...
struct cv::ocl::internal::ProgramEntry convert_oclsrc = {
    moduleName, "convert",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#endif\n"
    "#define noconvert\n"
    "__kernel void convertTo(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    ...
    "if (x < dst_cols)\n"
    "{\n"
    "int src_index = mad24(y0, src_step, mad24(x, (int)sizeof(srcT), "
    "src_offset));\n"
    "int dst_index = mad24(y0, dst_step, mad24(x, (int)sizeof(dstT), "
    "dst_offset));\n"
    "for (int y = y0, y1 = min(dst_rows, y0 + rowsPerWI); y < y1; ++y, "
    "src_index += src_step, dst_index += dst_step)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry copymakeborder_oclsrc = {
    moduleName, "copymakeborder",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "__kernel void copyMakeBorder(__global const uchar * srcptr, int src_step, "
    ...
    "{\n"
    "#ifdef BORDER_CONSTANT\n"
    "for (int y = y0, y1 = min(y0 + rowsPerWI, dst_rows); y < y1; ++y, "
    "dst_index += dst_step)\n"
...
struct cv::ocl::internal::ProgramEntry copyset_oclsrc = {
    moduleName, "copyset",
    "#ifdef COPY_TO_MASK\n"
    "#define DEFINE_DATA \\\n"
    "int src_index = mad24(y, src_step, mad24(x, (int)sizeof(T1) * scn, "
    "src_offset)); \\\n"
    "int dst_index = mad24(y, dst_step, mad24(x, (int)sizeof(T1) * scn, "
    "dst_offset)); \\\n"
    "\\\n"
    "__global const T1 * src = (__global const T1 *)(srcptr + src_index); \\\n"
    "__global T1 * dst = (__global T1 *)(dstptr + dst_index)\n"
    "__kernel void copyToMask(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    ...
    "#define storedst(val) vstore3(val, 0, (__global dstT1 *)(dstptr + "
    "dst_index))\n"
    "#endif\n"
    "__kernel void setMask(__global const uchar* mask, int maskstep, int "
    "maskoffset,\n"
    "__global uchar* dstptr, int dststep, int dstoffset,\n"
    ...
    "{\n"
    "int mask_index = mad24(y0, maskstep, x + maskoffset);\n"
    "int dst_index  = mad24(y0, dststep, mad24(x, (int)sizeof(dstT1) * cn, "
    "dstoffset));\n"
    ...
    "int dst_index  = mad24(y0, dststep, mad24(x, (int)sizeof(dstT1) * cn, "
    "dstoffset));\n"
    "for (int y = y0, y1 = min(rows, y0 + rowsPerWI); y < y1; ++y, dst_index "
    "+= dststep)\n"
    "storedst(value);\n"
    "}\n"
    "}\n"
...
struct cv::ocl::internal::ProgramEntry cvtclr_dx_oclsrc = {
    moduleName, "cvtclr_dx",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined cl_khr_fp64\n"
    ...
    "static const __constant float d1                = BT601_BLACK_RANGE / "
    "CV_8U_MAX;\n"
    "static const __constant float d2                = CV_8U_HALF / "
    "CV_8U_MAX;\n"
    "#define NCHANNELS 3\n"
    "__kernel\n"
    ...
    "if (y + 1 < rows)\n"
    "{\n"
    "__global uchar* pDstRow1 = pBGR + mad24(y, bgrStep, mad24(x, NCHANNELS, "
    "0));\n"
    ...
    "if (y < rows)\n"
    "{\n"
    "__global const uchar* pSrcRow1 = pBGR + mad24(y, bgrStep, mad24(x, "
    "NCHANNELS, 0));\n"
    "__global const uchar* pSrcRow2 = pSrcRow1 + bgrStep;\n"
    "float4 src_pix1 = convert_float4(vload4(0, pSrcRow1 + 0*NCHANNELS)) * "
    "CV_8U_SCALE;\n"
    "float4 src_pix2 = convert_float4(vload4(0, pSrcRow1 + 1*NCHANNELS)) * "
    "CV_8U_SCALE;\n"
    "float4 src_pix3 = convert_float4(vload4(0, pSrcRow2 + 0*NCHANNELS)) * "
    "CV_8U_SCALE;\n"
    "float4 src_pix4 = convert_float4(vload4(0, pSrcRow2 + 1*NCHANNELS)) * "
    "CV_8U_SCALE;\n"
    "__constant float* coeffs = c_RGB2YUVCoeffs_420;\n"
    "float Y1 = fma(coeffs[0], src_pix1.z, fma(coeffs[1], src_pix1.y, "
    "fma(coeffs[2], src_pix1.x, d1)));\n"
    "float Y2 = fma(coeffs[0], src_pix2.z, fma(coeffs[1], src_pix2.y, "
    "fma(coeffs[2], src_pix2.x, d1)));\n"
    "float Y3 = fma(coeffs[0], src_pix3.z, fma(coeffs[1], src_pix3.y, "
    "fma(coeffs[2], src_pix3.x, d1)));\n"
    "float Y4 = fma(coeffs[0], src_pix4.z, fma(coeffs[1], src_pix4.y, "
    "fma(coeffs[2], src_pix4.x, d1)));\n"
    "float4 UV;\n"
    "UV.x = fma(coeffs[3], src_pix1.z, fma(coeffs[4], src_pix1.y, "
    "fma(coeffs[5], src_pix1.x, d2)));\n"
    "UV.y = fma(coeffs[5], src_pix1.z, fma(coeffs[6], src_pix1.y, "
    "fma(coeffs[7], src_pix1.x, d2)));\n"
...
struct cv::ocl::internal::ProgramEntry fft_oclsrc = {
    moduleName, "fft",
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void butterfly2(CT a0, CT a1, __local CT* smem, __global const CT* "
    "twiddles,\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void butterfly4(CT a0, CT a1, CT a2, CT a3, __local CT* smem, __global "
    "const CT* twiddles,\n"
    "const int x, const int block_size)\n"
    "{\n"
    "const int k = x & (block_size - 1);\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void butterfly3(CT a0, CT a1, CT a2, __local CT* smem, __global const CT* "
    "twiddles,\n"
    "const int x, const int block_size)\n"
    "{\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void butterfly5(CT a0, CT a1, CT a2, CT a3, CT a4, __local CT* smem, "
    "__global const CT* twiddles,\n"
    "const int x, const int block_size)\n"
    "{\n"
    "const int k = x % block_size;\n"
    "a1 = mul_complex(twiddles[k], a1);\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix2(__local CT* smem, __global const CT* twiddles, const int "
    "x, const int block_size, const int t)\n"
    "{\n"
    "CT a0, a1;\n"
    "if (x < t)\n"
    "{\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix2_B2(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int x2 = x1 + t/2;\n"
    "CT a0, a1, a2, a3;\n"
    "if (x1 < t/2)\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix2_B3(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int x2 = x1 + t/3;\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix2_B4(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix2_B5(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix4(__local CT* smem, __global const CT* twiddles, const int "
    "x, const int block_size, const int t)\n"
    "{\n"
    "CT a0, a1, a2, a3;\n"
    "if (x < t)\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix4_B2(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int x2 = x1 + t/2;\n"
    "CT a0, a1, a2, a3, a4, a5, a6, a7;\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix4_B3(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int x2 = x1 + t/3;\n"
    "const int x3 = x2 + t/3;\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix8(__local CT* smem, __global const CT* twiddles, const int "
    "x, const int block_size, const int t)\n"
    "{\n"
    "const int k = x % block_size;\n"
    "CT a0, a1, a2, a3, a4, a5, a6, a7;\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix3(__local CT* smem, __global const CT* twiddles, const int "
    "x, const int block_size, const int t)\n"
    "{\n"
    "CT a0, a1, a2;\n"
    "if (x < t)\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix3_B2(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int x2 = x1 + t/2;\n"
    "CT a0, a1, a2, a3, a4, a5;\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix3_B3(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int x2 = x1 + t/3;\n"
    "const int x3 = x2 + t/3;\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix3_B4(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int thread_block = t/4;\n"
    "const int x2 = x1 + thread_block;\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix5(__local CT* smem, __global const CT* twiddles, const int "
    "x, const int block_size, const int t)\n"
    "{\n"
    "const int k = x % block_size;\n"
    "CT a0, a1, a2, a3, a4;\n"
    "if (x < t)\n"
    "{\n"
    "a0 = smem[x]; a1 = smem[x + t]; a2 = smem[x+2*t]; a3 = smem[x+3*t]; a4 = "
    "smem[x+4*t];\n"
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "if (x < t)\n"
    ...
    "}\n"
    "__attribute__((always_inline))\n"
    "void fft_radix5_B2(__local CT* smem, __global const CT* twiddles, const "
    "int x1, const int block_size, const int t)\n"
    "{\n"
    "const int x2 = x1+t/2;\n"
    "CT a0, a1, a2, a3, a4, a5, a6, a7, a8, a9;\n"
    ...
    "a0 = smem[x1]; a1 = smem[x1 + t]; a2 = smem[x1+2*t]; a3 = smem[x1+3*t]; "
    "a4 = smem[x1+4*t];\n"
    "a5 = smem[x2]; a6 = smem[x2 + t]; a7 = smem[x2+2*t]; a8 = smem[x2+3*t]; "
    "a9 = smem[x2+4*t];\n"
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "if (x1 < t/2)\n"
    ...
    "#endif\n"
    "__kernel void fft_multi_radix_rows(__global const uchar* src_ptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar* dst_ptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__global CT* twiddles_ptr, int twiddles_step, int twiddles_offset, const "
    "int t, const int nz)\n"
    "{\n"
    ...
    "if (y < nz)\n"
    "{\n"
    "__global const CT* twiddles = (__global const CT*)(twiddles_ptr + "
    "twiddles_offset);\n"
    "const int ind = x;\n"
    "#ifdef IS_1D\n"
    ...
    "#endif\n"
    "#ifdef COMPLEX_INPUT\n"
    "__global const CT* src = (__global const CT*)(src_ptr + mad24(y, "
    "src_step, mad24(x, (int)(sizeof(CT)), src_offset)));\n"
    "#pragma unroll\n"
    "for (int i=0; i<kercn; i++)\n"
    "smem[x+i*block_size] = src[i*block_size];\n"
    "#else\n"
    "__global const FT* src = (__global const FT*)(src_ptr + mad24(y, "
    "src_step, mad24(x, (int)sizeof(FT), src_offset)));\n"
    "#pragma unroll\n"
    "for (int i=0; i<kercn; i++)\n"
    ...
    "#endif\n"
    "__global CT* dst = (__global CT*)(dst_ptr + mad24(y, dst_step, "
    "dst_offset));\n"
    "#pragma unroll\n"
    ...
    "{\n"
    "dst[dst_cols-i] = (CT)(SCALE_VAL(smem[i].x, scale), SCALE_VAL(-smem[i].y, "
    "scale));\n"
    "}\n"
    "#endif\n"
    "#endif\n"
    ...
    "__global FT* dst = (__global FT*)(dst_ptr + mad24(y, dst_step, "
    "dst_offset));\n"
    ...
    "}\n"
    "}\n"
    "__kernel void fft_multi_radix_cols(__global const uchar* src_ptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar* dst_ptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__global CT* twiddles_ptr, int twiddles_step, int twiddles_offset, const "
    ...
    "if (x < nz)\n"
    "{\n"
    "__global const uchar* src = src_ptr + mad24(y, src_step, mad24(x, "
    "(int)(sizeof(CT)), src_offset));\n"
    "__global const CT* twiddles = (__global const CT*)(twiddles_ptr + "
    "twiddles_offset);\n"
    "const int ind = y;\n"
    "const int block_size = LOCAL_SIZE/kercn;\n"
    ...
    "#pragma unroll\n"
    "for (int i=0; i<kercn; i++)\n"
    "smem[y+i*block_size] = *((__global const CT*)(src + "
    "i*block_size*src_step));\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "RADIX_PROCESS;\n"
    "#ifdef COMPLEX_OUTPUT\n"
    "__global uchar* dst = dst_ptr + mad24(y, dst_step, mad24(x, "
    "(int)(sizeof(CT)), dst_offset));\n"
    "#pragma unroll\n"
    "for (int i=0; i<kercn; i++)\n"
    "*((__global CT*)(dst + i*block_size*dst_step)) = SCALE_VAL(smem[y + "
    "i*block_size], scale);\n"
    "#else\n"
    "if (x == 0)\n"
    "{\n"
    ...
    "if (y == 0)\n"
    "*((__global FT*) (dst_ptr + dst_offset)) = SCALE_VAL(smem_1cn[0], "
    "scale);\n"
    "}\n"
    "else if (x == (dst_cols+1)/2)\n"
    "{\n"
    "__local FT* smem_1cn = (__local FT*) smem;\n"
    "__global uchar* dst = dst_ptr + mad24(dst_cols-1, (int)sizeof(FT), "
    "mad24(y+1, dst_step, dst_offset));\n"
    "for (int i=y; i<dst_rows-1; i+=block_size, dst+=dst_step*block_size)\n"
    "*((__global FT*) dst) = SCALE_VAL(smem_1cn[i+2], scale);\n"
    "if (y == 0)\n"
    "*((__global FT*) (dst_ptr + mad24(dst_cols-1, (int)sizeof(FT), "
    "dst_offset))) = SCALE_VAL(smem_1cn[0], scale);\n"
    "}\n"
    "else\n"
    "{\n"
    "__global uchar* dst = dst_ptr + mad24(x, (int)sizeof(FT)*2, mad24(y, "
    "dst_step, dst_offset - (int)sizeof(FT)));\n"
    "#pragma unroll\n"
    ...
    "}\n"
    "}\n"
    "__kernel void ifft_multi_radix_rows(__global const uchar* src_ptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar* dst_ptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__global CT* twiddles_ptr, int twiddles_step, int twiddles_offset, const "
    ...
    "__global const CT* src = (__global const CT*)(src_ptr + mad24(y, "
    "src_step, mad24(2, (int)sizeof(FT), src_offset)));\n"
    "#pragma unroll\n"
    ...
    "for (int i=x; i<(LOCAL_SIZE-1)/2; i+=block_size)\n"
    "{\n"
    "CT src = vload2(0, (__global const FT*)(src_ptr + mad24(y, src_step, "
    "mad24(2*i+1, (int)sizeof(FT), src_offset))));\n"
    "smem[i+1].x = src.x;\n"
    "smem[i+1].y = -src.y;\n"
    ...
    "if (x==0)\n"
    "{\n"
    "smem[0].x = *(__global const FT*)(src_ptr + mad24(y, src_step, "
    "src_offset));\n"
    "smem[0].y = 0.f;\n"
    "if(LOCAL_SIZE % 2 ==0)\n"
    "{\n"
    ...
    "#else\n"
    "smem[LOCAL_SIZE/2].x = *(__global const FT*)(src_ptr + mad24(y, src_step, "
    "mad24(LOCAL_SIZE-1, (int)sizeof(FT), src_offset)));\n"
    "#endif\n"
    ...
    "RADIX_PROCESS;\n"
    "#ifdef COMPLEX_OUTPUT\n"
    "__global CT* dst = (__global CT*)(dst_ptr + mad24(y, dst_step, mad24(x, "
    "(int)(sizeof(CT)), dst_offset)));\n"
    "#pragma unroll\n"
    "for (int i=0; i<kercn; i++)\n"
    "{\n"
    ...
    "}\n"
    "#else\n"
    "__global FT* dst = (__global FT*)(dst_ptr + mad24(y, dst_step, mad24(x, "
    "(int)(sizeof(FT)), dst_offset)));\n"
    "#pragma unroll\n"
    "for (int i=0; i<kercn; i++)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void ifft_multi_radix_cols(__global const uchar* src_ptr, int "
    "src_step, int src_offset, int src_rows, int src_cols,\n"
    "__global uchar* dst_ptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "__global CT* twiddles_ptr, int twiddles_step, int twiddles_offset, const "
    ...
    "#endif\n"
    "{\n"
    "__global const uchar* src = src_ptr + mad24(y, src_step, mad24(2*x-1, "
    "(int)sizeof(FT), src_offset));\n"
    "#pragma unroll\n"
    "for (int i=0; i<kercn; i++)\n"
    "{\n"
    ...
    "{\n"
    "int ind = x==0 ? 0: 2*x-1;\n"
    "__global const FT* src = (__global const FT*)(src_ptr + mad24(1, "
    "src_step, mad24(ind, (int)sizeof(FT), src_offset)));\n"
    ...
    "if (y==0)\n"
    "{\n"
    "smem[0].x = *(__global const FT*)(src_ptr + mad24(ind, (int)sizeof(FT), "
    "src_offset));\n"
    "smem[0].y = 0.f;\n"
    "if(LOCAL_SIZE % 2 ==0)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry flip_oclsrc = {
    moduleName, "flip",
    "#if kercn != 3\n"
    "#define loadpix(addr) *(__global const T *)(addr)\n"
    "#define storepix(val, addr)  *(__global T *)(addr) = val\n"
    ...
    "#define TSIZE ((int)sizeof(T1)*3)\n"
    "#endif\n"
    "__kernel void arithm_flip_rows(__global const uchar * srcptr, int "
    ...
    "int src_index1 = mad24(rows - y0 - 1, src_step, mad24(x, TSIZE, "
    "src_offset));\n"
    "int dst_index0 = mad24(y0, dst_step, mad24(x, TSIZE, dst_offset));\n"
    "int dst_index1 = mad24(rows - y0 - 1, dst_step, mad24(x, TSIZE, "
    "dst_offset));\n"
    "#pragma unroll\n"
    ...
    "}\n"
    "}\n"
    "__kernel void arithm_flip_rows_cols(__global const uchar * srcptr, int "
    "src_step, int src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols, int thread_rows, int thread_cols)\n"
    "{\n"
    ...
    "{\n"
    "int src_index0 = mad24(y0, src_step, mad24(x, TSIZE, src_offset));\n"
    "int src_index1 = mad24(rows - y0 - 1, src_step, mad24(cols - x - 1, "
    "TSIZE, src_offset));\n"
    "int dst_index0 = mad24(y0, dst_step, mad24(x, TSIZE, dst_offset));\n"
    "int dst_index1 = mad24(rows - y0 - 1, dst_step, mad24(cols - x - 1, "
    "TSIZE, dst_offset));\n"
    "#pragma unroll\n"
    "for (int y = y0, y1 = min(thread_rows, y0 + PIX_PER_WI_Y); y < y1; ++y)\n"
    "{\n"
    ...
    "}\n"
    "}\n"
    "__kernel void arithm_flip_cols(__global const uchar * srcptr, int "
    "src_step, int src_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    "int rows, int cols, int thread_rows, int thread_cols)\n"
    "{\n"
    ...
    "{\n"
    "int src_index0 = mad24(y0, src_step, mad24(x, TSIZE, src_offset));\n"
    "int src_index1 = mad24(y0, src_step, mad24(cols - x - 1, TSIZE, "
    "src_offset));\n"
    "int dst_index0 = mad24(y0, dst_step, mad24(x, TSIZE, dst_offset));\n"
    "int dst_index1 = mad24(y0, dst_step, mad24(cols - x - 1, TSIZE, "
    "dst_offset));\n"
    "#pragma unroll\n"
...
struct cv::ocl::internal::ProgramEntry gemm_oclsrc = {
    moduleName, "gemm",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#endif\n"
    "__kernel void gemm(__global const uchar * A_ptr, int A_step, int "
    "A_offset,\n"
    "__global const uchar * B_ptr, int B_step, int B_offset,\n"
    "__global uchar * D_ptr, int D_step, int D_offset, int D_rows, int "
    "D_cols,\n"
    ...
    "a_local[mad24(lidy, LOCAL_SIZE_ODD, lidx)] = A[mad24(p, LOCAL_SIZE, "
    "lidx)];\n"
    "if (p * LOCAL_SIZE + lidy < n && x < D_cols)\n"
    "b_local[mad24(lidy, LOCAL_SIZE_ODD, lidx)] = B[mad24(p, LOCAL_SIZE, "
    "lidy)*STEP_B];\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "if (x < D_cols && y < D_rows)\n"
    "{\n"
    ...
    "#endif\n"
    "MUL(a_local[mad24(lidy, LOCAL_SIZE_ODD, i)], b_local[mad24(i, "
    "LOCAL_SIZE_ODD, lidx)]);\n"
    "}\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "}\n"
    ...
    "{\n"
    "#endif\n"
    "__global WT* D = (__global WT*)(D_ptr + mad24(y, D_step, mad24(x, WTSIZE, "
    "D_offset)));\n"
    "#if HAVE_C\n"
...
struct cv::ocl::internal::ProgramEntry halfconvert_oclsrc = {
    moduleName, "halfconvert",
    "#ifdef HALF_SUPPORT\n"
    "#ifdef cl_khr_fp16\n"
...
struct cv::ocl::internal::ProgramEntry inrange_oclsrc = {
    moduleName, "inrange",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#endif\n"
    "#endif\n"
    "__kernel void inrange(__global const uchar * src1ptr, int src1_step, int "
    "src1_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols,\n"
    "#ifdef HAVE_SCALAR\n"
    ...
    "if (x < dst_cols)\n"
    "{\n"
    "int src1_index = mad24(y0, src1_step, mad24(x, (int)sizeof(srcT1) * "
    "kercn, src1_offset));\n"
    "int dst_index = mad24(y0, dst_step, mad24(x, colsPerWI, dst_offset));\n"
    "#ifndef HAVE_SCALAR\n"
    "int src2_index = mad24(y0, src2_step, mad24(x, (int)sizeof(srcT1) * "
    "kercn, src2_offset));\n"
    "int src3_index = mad24(y0, src3_step, mad24(x, (int)sizeof(srcT1) * "
    "kercn, src3_offset));\n"
    "#endif\n"
    "for (int y = y0, y1 = min(dst_rows, y0 + rowsPerWI); y < y1; ++y, "
    "src1_index += src1_step, dst_index += dst_step)\n"
    "{\n"
    ...
    "#endif\n"
    "#else\n"
    "__global const srcT1 * src1 = (__global const srcT1 *)(src1ptr + "
    "src1_index);\n"
    "__global uchar * dst = dstptr + dst_index;\n"
    "#ifndef HAVE_SCALAR\n"
    "__global const srcT1 * src2 = (__global const srcT1 *)(src2ptr + "
    "src2_index);\n"
    "__global const srcT1 * src3 = (__global const srcT1 *)(src3ptr + "
    "src3_index);\n"
    "#endif\n"
    "#pragma unroll\n"
...
struct cv::ocl::internal::ProgramEntry intel_gemm_oclsrc = {
    moduleName, "intel_gemm",
    ...
    "__global float *dst_write0 = dst + local_x * VEC_SIZE + ( group_x * "
    "TILE_N ) + ( group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M) * ldC + "
    "offd;\n"
    "const __global float *src0_read = src0 + local_x * ( TILE_K / 8 ) + ( "
    "group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M ) * ldA + start_index + "
    "off0;\n"
    "const __global float *src1_read0 = src1 + local_x * VEC_SIZE + ( group_x "
    "* TILE_N ) + start_index * ldB + off1;\n"
    "float4 dot00 = (start_index != 0) ? vload4(0, dst_write0)           : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0          )) : "
    "(float4)(0.0));\n"
    "float4 dot01 = (start_index != 0) ? vload4(0, dst_write0 + 1 * ldC) : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0 + 1 * ldC)) : "
    "(float4)(0.0));\n"
    "float4 dot02 = (start_index != 0) ? vload4(0, dst_write0 + 2 * ldC) : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0 + 2 * ldC)) : "
    "(float4)(0.0));\n"
    "float4 dot03 = (start_index != 0) ? vload4(0, dst_write0 + 3 * ldC) : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0 + 3 * ldC)) : "
    "(float4)(0.0));\n"
    "float4 dot04 = (start_index != 0) ? vload4(0, dst_write0 + 4 * ldC) : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0 + 4 * ldC)) : "
    "(float4)(0.0));\n"
    "float4 dot05 = (start_index != 0) ? vload4(0, dst_write0 + 5 * ldC) : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0 + 5 * ldC)) : "
    "(float4)(0.0));\n"
    "float4 dot06 = (start_index != 0) ? vload4(0, dst_write0 + 6 * ldC) : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0 + 6 * ldC)) : "
    "(float4)(0.0));\n"
    "float4 dot07 = (start_index != 0) ? vload4(0, dst_write0 + 7 * ldC) : "
    "((beta != 0.0) ? ((float)beta * vload4(0, dst_write0 + 7 * ldC)) : "
    "(float4)(0.0));\n"
    ...
    "dot00 = "
    "mad((float4)(intel_sub_group_shuffle(arow0.s##suffix,index)),brow,dot00); "
    "\\\n"
    "dot01 = "
    "mad((float4)(intel_sub_group_shuffle(arow1.s##suffix,index)),brow,dot01); "
    "\\\n"
    "dot02 = "
    "mad((float4)(intel_sub_group_shuffle(arow2.s##suffix,index)),brow,dot02); "
    "\\\n"
    "dot03 = "
    "mad((float4)(intel_sub_group_shuffle(arow3.s##suffix,index)),brow,dot03); "
    "\\\n"
    "dot04 = "
    "mad((float4)(intel_sub_group_shuffle(arow4.s##suffix,index)),brow,dot04); "
    "\\\n"
    "dot05 = "
    "mad((float4)(intel_sub_group_shuffle(arow5.s##suffix,index)),brow,dot05); "
    "\\\n"
    "dot06 = "
    "mad((float4)(intel_sub_group_shuffle(arow6.s##suffix,index)),brow,dot06); "
    "\\\n"
    "dot07 = "
    "mad((float4)(intel_sub_group_shuffle(arow7.s##suffix,index)),brow,dot07);"
    "\n"
    "MM_DOT_PRODUCT(0,0);\n"
    "MM_DOT_PRODUCT(0,1);\n"
    "MM_DOT_PRODUCT(1,0);\n"
    ...
    "}\n"
    "if(w < end_index) {\n"
    "arow0.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row0 * "
    "ldA)[0] : 0.0f;\n"
    "arow0.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row0 "
    "* ldA)[1] : 0.0f;\n"
    "arow1.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row1 * "
    "ldA)[0] : 0.0f;\n"
    "arow1.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row1 "
    "* ldA)[1] : 0.0f;\n"
    "arow2.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row2 * "
    "ldA)[0] : 0.0f;\n"
    "arow2.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row2 "
    "* ldA)[1] : 0.0f;\n"
    "arow3.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row3 * "
    "ldA)[0] : 0.0f;\n"
    "arow3.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row3 "
    "* ldA)[1] : 0.0f;\n"
    "arow4.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row4 * "
    "ldA)[0] : 0.0f;\n"
    "arow4.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row4 "
    "* ldA)[1] : 0.0f;\n"
    "arow5.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row5 * "
    "ldA)[0] : 0.0f;\n"
    "arow5.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row5 "
    "* ldA)[1] : 0.0f;\n"
    "arow6.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row6 * "
    "ldA)[0] : 0.0f;\n"
    "arow6.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row6 "
    "* ldA)[1] : 0.0f;\n"
    "arow7.x = ((w + local_x * 2) < K) ? (float)alpha * (src0_read + row7 * "
    "ldA)[0] : 0.0f;\n"
    "arow7.y = ((w + local_x * 2 + 1) < K) ? (float)alpha * (src0_read + row7 "
    "* ldA)[1] : 0.0f;\n"
    "#define MM_DOT_PRODUCT(index,suffix)   \\\n"
    "brow = (w < K) ? vload4(0, src1_read0) : (float)0.0f;  src1_read0 += ldB; "
    "w++; \\\n"
    "dot00 = mad((float4)(intel_sub_group_shuffle( arow0.s##suffix, index "
    ")),brow,dot00 ); \\\n"
    "dot01 = mad((float4)(intel_sub_group_shuffle( arow1.s##suffix, index "
    ")),brow,dot01 ); \\\n"
    "dot02 = mad((float4)(intel_sub_group_shuffle( arow2.s##suffix, index "
    ")),brow,dot02 ); \\\n"
    "dot03 = mad((float4)(intel_sub_group_shuffle( arow3.s##suffix, index "
    ")),brow,dot03 ); \\\n"
    "dot04 = mad((float4)(intel_sub_group_shuffle( arow4.s##suffix, index "
    ")),brow,dot04 ); \\\n"
    "dot05 = mad((float4)(intel_sub_group_shuffle( arow5.s##suffix, index "
    ")),brow,dot05 ); \\\n"
    "dot06 = mad((float4)(intel_sub_group_shuffle( arow6.s##suffix, index "
    ")),brow,dot06 ); \\\n"
    "dot07 = mad((float4)(intel_sub_group_shuffle( arow7.s##suffix, index "
    ")),brow,dot07 );\n"
    "MM_DOT_PRODUCT(0,0);\n"
    "MM_DOT_PRODUCT(0,1);\n"
    "MM_DOT_PRODUCT(1,0);\n"
    ...
    "vstore4(dot00, 0, dst_write0); dst_write0 += ldC;\n"
    "if(mad24(global_y, 8, 1) < M) { vstore4(dot01, 0, dst_write0); dst_write0 "
    "+= ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 2) < M) { vstore4(dot02, 0, dst_write0); dst_write0 "
    "+= ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 3) < M) { vstore4(dot03, 0, dst_write0); dst_write0 "
    "+= ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 4) < M) { vstore4(dot04, 0, dst_write0); dst_write0 "
    "+= ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 5) < M) { vstore4(dot05, 0, dst_write0); dst_write0 "
    "+= ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 6) < M) { vstore4(dot06, 0, dst_write0); dst_write0 "
    "+= ldC; }\n"
    "else return;\n"
    ...
    "if(mad24(global_y, 8, 1) < M) { vstore2(dot01.xy, 0, dst_write0); "
    "dst_write0 += ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 2) < M) { vstore2(dot02.xy, 0, dst_write0); "
    "dst_write0 += ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 3) < M) { vstore2(dot03.xy, 0, dst_write0); "
    "dst_write0 += ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 4) < M) { vstore2(dot04.xy, 0, dst_write0); "
    "dst_write0 += ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 5) < M) { vstore2(dot05.xy, 0, dst_write0); "
    "dst_write0 += ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 6) < M) { vstore2(dot06.xy, 0, dst_write0); "
    "dst_write0 += ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 7) < M) { vstore2(dot07.xy, 0, dst_write0); }\n"
    "} else {\n"
    "dst_write0[0] = dot00.x; dst_write0 += ldC;\n"
    "if(mad24(global_y, 8, 1) < M) { dst_write0[0] = dot01.x; dst_write0 += "
    "ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 2) < M) { dst_write0[0] = dot02.x; dst_write0 += "
    "ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 3) < M) { dst_write0[0] = dot03.x; dst_write0 += "
    "ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 4) < M) { dst_write0[0] = dot04.x; dst_write0 += "
    "ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 5) < M) { dst_write0[0] = dot05.x; dst_write0 += "
    "ldC; }\n"
    "else return;\n"
    "if(mad24(global_y, 8, 6) < M) { dst_write0[0] = dot06.x; dst_write0 += "
    "ldC; }\n"
    "else return;\n"
    ...
    "offd;\n"
    "const __global float *src0_read = src0 + local_x * ( TILE_K / 8 ) + ( "
    "group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M ) * ldA + off0;\n"
    "const __global float *src1_read0 = src1 + ( group_x * TILE_N ) * ldB + "
    "off1;\n"
    ...
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    "vstore4(vload4(0, src1_read0 + mad24(0, ldB, local_index)), 0, slm_brow + "
    "mad24(0, SLM_BLOCK, local_index));\n"
    "vstore4(vload4(0, src1_read0 + mad24(1, ldB, local_index)), 0, slm_brow + "
    "mad24(1, SLM_BLOCK, local_index));\n"
    "vstore4(vload4(0, src1_read0 + mad24(2, ldB, local_index)), 0, slm_brow + "
    "mad24(2, SLM_BLOCK, local_index));\n"
    "vstore4(vload4(0, src1_read0 + mad24(3, ldB, local_index)), 0, slm_brow + "
    "mad24(3, SLM_BLOCK, local_index));\n"
    "vstore4(vload4(0, src1_read0 + mad24(4, ldB, local_index)), 0, slm_brow + "
    "mad24(4, SLM_BLOCK, local_index));\n"
    "vstore4(vload4(0, src1_read0 + mad24(5, ldB, local_index)), 0, slm_brow + "
    "mad24(5, SLM_BLOCK, local_index));\n"
    "vstore4(vload4(0, src1_read0 + mad24(6, ldB, local_index)), 0, slm_brow + "
    "mad24(6, SLM_BLOCK, local_index));\n"
    "vstore4(vload4(0, src1_read0 + mad24(7, ldB, local_index)), 0, slm_brow + "
    "mad24(7, SLM_BLOCK, local_index));\n"
    "barrier(CLK_LOCAL_MEM_FENCE);\n"
    ...
    "_dot = mad( (float8)(arow.x), (float8)(brow0.x, brow1.x, brow2.x, "
    "brow3.x, brow4.x, brow5.x, brow6.x, brow7.x), _dot ); \\\n"
    "_dot = mad( (float8)(arow.y), (float8)(brow0.y, brow1.y, brow2.y, "
    "brow3.y, brow4.y, brow5.y, brow6.y, brow7.y), _dot ); \\\n"
    "_dot = mad( (float8)(arow.z), (float8)(brow0.z, brow1.z, brow2.z, "
    "brow3.z, brow4.z, brow5.z, brow6.z, brow7.z), _dot ); \\\n"
    "_dot = mad( (float8)(arow.w), (float8)(brow0.w, brow1.w, brow2.w, "
    "brow3.w, brow4.w, brow5.w, brow6.w, brow7.w), _dot );\n"
    ...
    "}\n"
    "#define REDUCE(_dot) \\\n"
    "_dot.s0 = intel_sub_group_shuffle(_dot.s0, 0) + "
    "intel_sub_group_shuffle(_dot.s0, 1) + intel_sub_group_shuffle(_dot.s0, 2) "
    "+ intel_sub_group_shuffle(_dot.s0, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s0, 4) + intel_sub_group_shuffle(_dot.s0, 5) "
    "+ intel_sub_group_shuffle(_dot.s0, 6) + intel_sub_group_shuffle(_dot.s0, "
    "7); \\\n"
    "_dot.s1 = intel_sub_group_shuffle(_dot.s1, 0) + "
    "intel_sub_group_shuffle(_dot.s1, 1) + intel_sub_group_shuffle(_dot.s1, 2) "
    "+ intel_sub_group_shuffle(_dot.s1, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s1, 4) + intel_sub_group_shuffle(_dot.s1, 5) "
    "+ intel_sub_group_shuffle(_dot.s1, 6) + intel_sub_group_shuffle(_dot.s1, "
    "7); \\\n"
    "_dot.s2 = intel_sub_group_shuffle(_dot.s2, 0) + "
    "intel_sub_group_shuffle(_dot.s2, 1) + intel_sub_group_shuffle(_dot.s2, 2) "
    "+ intel_sub_group_shuffle(_dot.s2, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s2, 4) + intel_sub_group_shuffle(_dot.s2, 5) "
    "+ intel_sub_group_shuffle(_dot.s2, 6) + intel_sub_group_shuffle(_dot.s2, "
    "7); \\\n"
    "_dot.s3 = intel_sub_group_shuffle(_dot.s3, 0) + "
    "intel_sub_group_shuffle(_dot.s3, 1) + intel_sub_group_shuffle(_dot.s3, 2) "
    "+ intel_sub_group_shuffle(_dot.s3, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s3, 4) + intel_sub_group_shuffle(_dot.s3, 5) "
    "+ intel_sub_group_shuffle(_dot.s3, 6) + intel_sub_group_shuffle(_dot.s3, "
    "7); \\\n"
    "_dot.s4 = intel_sub_group_shuffle(_dot.s4, 0) + "
    "intel_sub_group_shuffle(_dot.s4, 1) + intel_sub_group_shuffle(_dot.s4, 2) "
    "+ intel_sub_group_shuffle(_dot.s4, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s4, 4) + intel_sub_group_shuffle(_dot.s4, 5) "
    "+ intel_sub_group_shuffle(_dot.s4, 6) + intel_sub_group_shuffle(_dot.s4, "
    "7); \\\n"
    "_dot.s5 = intel_sub_group_shuffle(_dot.s5, 0) + "
    "intel_sub_group_shuffle(_dot.s5, 1) + intel_sub_group_shuffle(_dot.s5, 2) "
    "+ intel_sub_group_shuffle(_dot.s5, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s5, 4) + intel_sub_group_shuffle(_dot.s5, 5) "
    "+ intel_sub_group_shuffle(_dot.s5, 6) + intel_sub_group_shuffle(_dot.s5, "
    "7); \\\n"
    "_dot.s6 = intel_sub_group_shuffle(_dot.s6, 0) + "
    "intel_sub_group_shuffle(_dot.s6, 1) + intel_sub_group_shuffle(_dot.s6, 2) "
    "+ intel_sub_group_shuffle(_dot.s6, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s6, 4) + intel_sub_group_shuffle(_dot.s6, 5) "
    "+ intel_sub_group_shuffle(_dot.s6, 6) + intel_sub_group_shuffle(_dot.s6, "
    "7); \\\n"
    "_dot.s7 = intel_sub_group_shuffle(_dot.s7, 0) + "
    "intel_sub_group_shuffle(_dot.s7, 1) + intel_sub_group_shuffle(_dot.s7, 2) "
    "+ intel_sub_group_shuffle(_dot.s7, 3) +  \\\n"
    "intel_sub_group_shuffle(_dot.s7, 4) + intel_sub_group_shuffle(_dot.s7, 5) "
    "+ intel_sub_group_shuffle(_dot.s7, 6) + intel_sub_group_shuffle(_dot.s7, "
    "7);\n"
    "REDUCE(dot00);\n"
    "REDUCE(dot01);\n"
    "REDUCE(dot02);\n"
    ...
    "offd;\n"
    "const __global float *src0_read = src0 + (local_x * ( TILE_K / 8 ) + "
    "start_index) * ldA + group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M + "
    "off0;\n"
    "const __global float *src1_read0 = src1 + local_x * VEC_SIZE + ( group_x "
    "* TILE_N ) + start_index * ldB + off1;\n"
    "float4 dot00 = (start_index != 0) ? vload4(0, dst_write0)           : "
    ...
    "brow = vload4(0, src1_read0);  src1_read0 += ldB; \\\n"
    "dot00 = mad( (float4)(intel_sub_group_shuffle(_arow.s0, index)), brow, "
    "dot00 ); \\\n"
    "dot01 = mad( (float4)(intel_sub_group_shuffle(_arow.s1, index)), brow, "
    "dot01 ); \\\n"
    "dot02 = mad( (float4)(intel_sub_group_shuffle(_arow.s2, index)), brow, "
    "dot02 ); \\\n"
    "dot03 = mad( (float4)(intel_sub_group_shuffle(_arow.s3, index)), brow, "
    "dot03 ); \\\n"
    "dot04 = mad( (float4)(intel_sub_group_shuffle(_arow.s4, index)), brow, "
    "dot04 ); \\\n"
    "dot05 = mad( (float4)(intel_sub_group_shuffle(_arow.s5, index)), brow, "
    "dot05 ); \\\n"
    "dot06 = mad( (float4)(intel_sub_group_shuffle(_arow.s6, index)), brow, "
    "dot06 ); \\\n"
    "dot07 = mad( (float4)(intel_sub_group_shuffle(_arow.s7, index)), brow, "
    "dot07 );\n"
    ...
    "float8 arow0 = ((start_index + local_x * 2) < K) ? ((float)alpha * "
    "vload8(0, src0_read)) : (float)0.0f;\n"
    "float8 arow1 = ((start_index + local_x * 2 + 1) < K) ? ((float)alpha * "
    "vload8(0, src0_read + ldA)) : (float)0.0f;\n"
    "#define MM_DOT_PRODUCT(_arow,index) \\\n"
    "brow = (start_index < K) ? vload4(0, src1_read0) : (float)0.0f;  "
    "src1_read0 += ldB; start_index++; \\\n"
    "dot00 = mad( (float4)(intel_sub_group_shuffle(_arow.s0, index)), brow, "
    "dot00 ); \\\n"
    "dot01 = mad( (float4)(intel_sub_group_shuffle(_arow.s1, index)), brow, "
    "dot01 ); \\\n"
    ...
    "off0;\n"
    "const __global float *src1_read0 = src1 + (local_x * VEC_SIZE + ( group_x "
    "* TILE_N )) * ldB + start_index + off1;\n"
    "float4 dot00 = (start_index != 0) ? vload4(0, dst_write0)           : "
    "((beta != 0.0)? ((float)beta * vload4(0, dst_write0          )) : "
    "(float4)(0.0));\n"
    "float4 dot01 = (start_index != 0) ? vload4(0, dst_write0 + ldC)     : "
    "((beta != 0.0)? ((float)beta * vload4(0, dst_write0 + ldC    )) : "
    "(float4)(0.0));\n"
    "float4 dot02 = (start_index != 0) ? vload4(0, dst_write0 + 2 * ldC) : "
    "((beta != 0.0)? ((float)beta * vload4(0, dst_write0 + 2 * ldC)) : "
    "(float4)(0.0));\n"
    ...
    "float8 arow1 = (float)alpha * vload8(0, src0_read + ldA);\n"
    "#define DOT_PRODUCT( _dot, _arow, index, _brow) \\\n"
    "_dot.s0 = (w < K) ? mad( intel_sub_group_shuffle( _arow.s0, index ), "
    "_brow, _dot.s0 ) : _dot.s0; \\\n"
    "_dot.s1 = (w < K) ? mad( intel_sub_group_shuffle( _arow.s1, index ), "
    "_brow, _dot.s1 ) : _dot.s1; \\\n"
    "_dot.s2 = (w < K) ? mad( intel_sub_group_shuffle( _arow.s2, index ), "
    "_brow, _dot.s2 ) : _dot.s2; \\\n"
    "_dot.s3 = (w < K) ? mad( intel_sub_group_shuffle( _arow.s3, index ), "
    "_brow, _dot.s3 ) : _dot.s3; \\\n"
    "_dot.s4 = (w < K) ? mad( intel_sub_group_shuffle( _arow.s4, index ), "
    "_brow, _dot.s4 ) : _dot.s4; \\\n"
    "_dot.s5 = (w < K) ? mad( intel_sub_group_shuffle( _arow.s5, index ), "
    "_brow, _dot.s5 ) : _dot.s5; \\\n"
    "_dot.s6 = (w < K) ? mad( intel_sub_group_shuffle( _arow.s6, index ), "
    "_brow, _dot.s6 ) : _dot.s6; \\\n"
    "_dot.s7 = (w++ < K) ? mad( intel_sub_group_shuffle( _arow.s7, index ), "
    "_brow, _dot.s7 ) : _dot.s7;\n"
    "#define MM_DOT_PRODUCT( _brow, _dot) \\\n"
    "DOT_PRODUCT(_dot, arow0, 0, _brow.s0); \\\n"
    "DOT_PRODUCT(_dot, arow1, 0, _brow.s1); \\\n"
    "DOT_PRODUCT(_dot, arow0, 1, _brow.s2); \\\n"
...
struct cv::ocl::internal::ProgramEntry lut_oclsrc = {
    moduleName, "lut",
    "#if lcn == 1\n"
    "#if dcn == 4\n"
    "#define LUT_OP  \\\n"
    ...
    "#if dcn == 4\n"
    "#define LUT_OP \\\n"
    "__global const uchar4 * src_pixel = (__global const uchar4 *)(srcptr + "
    "src_index); \\\n"
    "int4 idx = mad24(convert_int4(src_pixel[0]), (int4)(lcn), (int4)(0, 1, 2, "
    "3)); \\\n"
    "dst = (__global dstT *)(dstptr + dst_index); \\\n"
    "dst[0] = lut_l[idx.x]; \\\n"
    "dst[1] = lut_l[idx.y]; \\\n"
    ...
    "#elif dcn == 2\n"
    "#define LUT_OP \\\n"
    "__global const uchar2 * src_pixel = (__global const uchar2 *)(srcptr + "
    "src_index); \\\n"
    ...
    "#endif\n"
    "#endif\n"
    "__kernel void LUT(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    ...
    "__global const dstT * lut = (__global const dstT *)(lutptr + "
    "lut_offset);\n"
    "for (int i = mad24((int)get_local_id(1), (int)get_local_size(0), "
    "(int)get_local_id(0)),\n"
    ...
    "if (x < cols && y < rows)\n"
    "{\n"
    "int src_index = mad24(y, src_step, mad24(x, (int)sizeof(srcT) * dcn, "
    "src_offset));\n"
    "int dst_index = mad24(y, dst_step, mad24(x, (int)sizeof(dstT) * dcn, "
    "dst_offset));\n"
    "__global dstT * dst;\n"
    "LUT_OP;\n"
    "if (y < rows - 1)\n"
...
struct cv::ocl::internal::ProgramEntry meanstddev_oclsrc = {
    moduleName, "meanstddev",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "#define storesqpix(val, addr) vstore3(val, 0, (__global sqdstT1 "
    "*)(addr))\n"
    "#define srcTSIZE ((int)sizeof(srcT1)*3)\n"
    "#define dstTSIZE ((int)sizeof(dstT1)*3)\n"
    "#define sqdstTSIZE ((int)sizeof(sqdstT1)*3)\n"
    "#endif\n"
    "__kernel void meanStdDev(__global const uchar * srcptr, int src_step, int "
    "src_offset, int cols,\n"
    ...
    "storesqpix(localMemSqSum[0], dstptr + mad24(dstTSIZE, groups, sqdstTSIZE "
    "* gid));\n"
    "#ifdef HAVE_MASK\n"
    "*(__global int *)(dstptr + mad24(dstTSIZE + sqdstTSIZE, groups, "
    "(int)sizeof(int) * gid)) = localMemNonZero[0];\n"
    "#endif\n"
    "}\n"
    "}\n",
    "1284edd21da32ce135cd26c0c897bd08", NULL};
struct cv::ocl::internal::ProgramEntry minmaxloc_oclsrc = {
    moduleName, "minmaxloc",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    "#elif defined (cl_khr_fp64)\n"
    ...
    "{\n"
    "return (pos + (MINMAX_STRUCT_ALIGNMENT - 1)) & (~(MINMAX_STRUCT_ALIGNMENT "
    "- 1));\n"
    "}\n"
    "#ifdef DEPTH_0\n"
    "#define MIN_VAL 0\n"
    ...
    "CALC_MAX(p, inc) \\\n"
    "CALC_MAX2(p)\n"
    "__kernel void minmaxloc(__global const uchar * srcptr, int src_step, int "
    "src_offset, int cols,\n"
    ...
    "int pos = 0;\n"
    "#ifdef NEED_MINVAL\n"
    "*(__global dstT1 *)(dstptr + mad24(gid, (int)sizeof(dstT1), pos)) = "
    "localmem_min[0];\n"
    ...
    "#endif\n"
    "#ifdef NEED_MINLOC\n"
    "*(__global uint *)(dstptr + mad24(gid, (int)sizeof(uint), pos)) = "
    "localmem_minloc[0];\n"
...
struct cv::ocl::internal::ProgramEntry mixchannels_oclsrc = {
    moduleName, "mixchannels",
    "#define DECLARE_INPUT_MAT(i) \\\n"
    "__global const uchar * src##i##ptr, int src##i##_step, int "
    "src##i##_offset,\n"
    "#define DECLARE_OUTPUT_MAT(i) \\\n"
    "__global uchar * dst##i##ptr, int dst##i##_step, int dst##i##_offset,\n"
    "#define DECLARE_INDEX(i) \\\n"
    "int src##i##_index = mad24(src##i##_step, y0, mad24(x, (int)sizeof(T) * "
    "scn##i, src##i##_offset)); \\\n"
    "int dst##i##_index = mad24(dst##i##_step, y0, mad24(x, (int)sizeof(T) * "
    "dcn##i, dst##i##_offset));\n"
    "#define PROCESS_ELEM(i) \\\n"
    "__global const T * src##i = (__global const T *)(src##i##ptr + "
    "src##i##_index); \\\n"
    "__global T * dst##i = (__global T *)(dst##i##ptr + dst##i##_index); \\\n"
    "dst##i[0] = src##i[0]; \\\n"
    "src##i##_index += src##i##_step; \\\n"
    "dst##i##_index += dst##i##_step;\n"
    "__kernel void mixChannels(DECLARE_INPUT_MAT_N DECLARE_OUTPUT_MAT_N int "
    "rows, int cols, int rowsPerWI)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y0 = get_global_id(1) * rowsPerWI;\n"
    "if (x < cols)\n"
...
struct cv::ocl::internal::ProgramEntry mulspectrums_oclsrc = {
    moduleName, "mulspectrums",
    ...
    "}\n"
    "__kernel void mulAndScaleSpectrums(__global const uchar * src1ptr, int "
    "src1_step, int src1_offset,\n"
    "__global const uchar * src2ptr, int src2_step, int src2_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset,\n"
    ...
    "if (x < dst_cols)\n"
    "{\n"
    "int src1_index = mad24(y0, src1_step, mad24(x, (int)sizeof(float2), "
    "src1_offset));\n"
    "int src2_index = mad24(y0, src2_step, mad24(x, (int)sizeof(float2), "
    "src2_offset));\n"
    "int dst_index = mad24(y0, dst_step, mad24(x, (int)sizeof(float2), "
    "dst_offset));\n"
    "for (int y = y0, y1 = min(dst_rows, y0 + rowsPerWI); y < y1; ++y,\n"
...
struct cv::ocl::internal::ProgramEntry normalize_oclsrc = {
    ...
    "#define dstTSIZE ((int)sizeof(dstT1)*3)\n"
    "#endif\n"
    "__kernel void normalizek(__global const uchar * srcptr, int src_step, int "
    "src_offset,\n"
    "__global const uchar * mask, int mask_step, int mask_offset,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset, int dst_rows, int "
    "dst_cols\n"
    ...
    "++y, src_index += src_step, dst_index += dst_step, mask_index += "
    "mask_step)\n"
    "{\n"
    "if (mask[mask_index])\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry reduce_oclsrc = {
    moduleName, "reduce",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#else\n"
    "#define MASK_INDEX int mask_index = mad24(id / cols, mask_step, "
    "mask_offset + (id % cols))\n"
    "#endif\n"
    "#if cn != 3\n"
    "#define loadpix(addr) *(__global const srcT *)(addr)\n"
    ...
    "#ifdef HAVE_MASK\n"
    "#ifdef HAVE_SRC2\n"
    "#define EXTRA_PARAMS , __global const uchar * mask, int mask_step, int "
    "mask_offset, __global const uchar * src2ptr, int src2_step, int "
    "src2_offset\n"
    "#else\n"
    "#define EXTRA_PARAMS , __global const uchar * mask, int mask_step, int "
    "mask_offset\n"
    ...
    "#else\n"
    "#ifdef HAVE_SRC2\n"
    "#define EXTRA_PARAMS , __global const uchar * src2ptr, int src2_step, int "
    "src2_offset\n"
    "#else\n"
    "#define EXTRA_PARAMS\n"
    "#endif\n"
    "#endif\n"
    "#if defined OP_SUM || defined OP_SUM_ABS || defined OP_SUM_SQR || defined "
    "OP_DOT\n"
    "#ifdef OP_DOT\n"
    "#if ddepth <= 4\n"
    ...
    "#else\n"
    "#define SRC2_INDEX int src2_index = mad24(id / cols, src2_step, mad24(id "
    "% cols, srcTSIZE, src2_offset))\n"
    "#endif\n"
    "#if kercn == 1\n"
    "#define REDUCE_GLOBAL \\\n"
    "SRC2_INDEX; \\\n"
    "dstTK temp = convertToDT(loadpix(srcptr + src_index)), temp2 = "
    "convertToDT(loadpix(src2ptr + src2_index)); \\\n"
    ...
    "storepix(localmem2[0], dstptr + mad24(groupnum, dstTSIZE, dstTSIZE * "
    "gid))\n"
    "#else\n"
    "#define SET_LOCAL_1 \\\n"
    "localmem[lid] = accumulator\n"
    ...
    "#ifdef OP_DOT\n"
    "#undef EXTRA_PARAMS\n"
    "#define EXTRA_PARAMS , __global uchar * src2ptr, int src2_step, int "
    "src2_offset\n"
    "#endif\n"
    "__kernel void reduce(__global const uchar * srcptr, int src_step, int "
    "src_offset, int cols,\n"
    ...
    "#else\n"
    "int src2_index = mad24(id / cols, src2_step, mul24(id % cols, "
    "srcTSIZE));\n"
    "#endif\n"
    "#endif\n"
    "REDUCE_GLOBAL;\n"
...
struct cv::ocl::internal::ProgramEntry reduce2_oclsrc = {
    moduleName, "reduce2",
    "#ifdef DOUBLE_SUPPORT\n"
    "#ifdef cl_amd_fp64\n"
    "#pragma OPENCL EXTENSION cl_amd_fp64:enable\n"
    ...
    "#endif\n"
    "#ifdef OP_REDUCE_PRE\n"
    "__kernel void reduce_horz_opt(__global const uchar * srcptr, int "
    ...
    "{\n"
    "int src_index = mad24(y, src_step, mad24(x, (int)sizeof(srcT) * cn, "
    "src_offset));\n"
    "__global const srcT * src = (__global const srcT *)(srcptr + src_index);\n"
    "bufT tmp[cn];\n"
    "#pragma unroll\n"
...
struct cv::ocl::internal::ProgramEntry repeat_oclsrc = {
    moduleName, "repeat",
    "#if cn != 3\n"
    "#define loadpix(addr) *(__global const T *)(addr)\n"
    "#define storepix(val, addr)  *(__global T *)(addr) = val\n"
    ...
    "#define TSIZE ((int)sizeof(T1)*3)\n"
    "#endif\n"
    "__kernel void repeat(__global const uchar * srcptr, int src_step, int "
    ...
    "if (x < src_cols)\n"
    "{\n"
    "int src_index = mad24(y0, src_step, mad24(x, (int)sizeof(T), "
    "src_offset));\n"
    "int dst_index0 = mad24(y0, dst_step, mad24(x, (int)sizeof(T), "
    "dst_offset));\n"
    "for (int y = y0, y1 = min(src_rows, y0 + rowsPerWI); y < y1; ++y, "
    "src_index += src_step, dst_index0 += dst_step)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry set_identity_oclsrc = {
    moduleName, "set_identity",
    "#if kercn != 3\n"
    "#define storepix(val, addr)  *(__global T *)(addr) = val\n"
    "#define TSIZE (int)sizeof(T)\n"
    ...
    "#endif\n"
    "__kernel void setIdentity(__global uchar * srcptr, int src_step, int "
    "src_offset, int rows, int cols,\n"
    "ST scalar_)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    ...
    "#if kercn == cn\n"
    "#pragma unroll\n"
    "for (int y = y0, i = 0, y1 = min(rows, y0 + rowsPerWI); i < rowsPerWI; "
    "++y, ++i, src_index += src_step)\n"
    "if (y < y1)\n"
    "storepix(x == y ? scalar : (T)(0), srcptr + src_index);\n"
    "#elif kercn == 4 && cn == 1\n"
    "if (y0 < rows)\n"
    "{\n"
    "storepix(x == y0 >> 2 ? (T)(scalar, 0, 0, 0) : (T)(0), srcptr + "
    "src_index);\n"
    "if (++y0 < rows)\n"
    "{\n"
    "src_index += src_step;\n"
    "storepix(x == y0 >> 2 ? (T)(0, scalar, 0, 0) : (T)(0), srcptr + "
    "src_index);\n"
    "if (++y0 < rows)\n"
    "{\n"
    "src_index += src_step;\n"
    "storepix(x == y0 >> 2 ? (T)(0, 0, scalar, 0) : (T)(0), srcptr + "
    "src_index);\n"
    "if (++y0 < rows)\n"
    "{\n"
    "src_index += src_step;\n"
    "storepix(x == y0 >> 2 ? (T)(0, 0, 0, scalar) : (T)(0), srcptr + "
    "src_index);\n"
    "}\n"
    "}\n"
    "}\n"
...
struct cv::ocl::internal::ProgramEntry split_merge_oclsrc = {
    moduleName, "split_merge",
    "#ifdef OP_MERGE\n"
    "#define DECLARE_SRC_PARAM(index) __global const uchar * src##index##ptr, "
    "int src##index##_step, int src##index##_offset,\n"
    "#define DECLARE_INDEX(index) int src##index##_index = "
    "mad24(src##index##_step, y0, mad24(x, (int)sizeof(T) * scn##index, "
    "src##index##_offset));\n"
    "#define PROCESS_ELEM(index) \\\n"
    "__global const T * src##index = (__global const T *)(src##index##ptr + "
    "src##index##_index); \\\n"
    ...
    "{\n"
    "DECLARE_INDEX_N\n"
    "int dst_index = mad24(dst_step, y0, mad24(x, (int)sizeof(T) * cn, "
    "dst_offset));\n"
    "for (int y = y0, y1 = min(rows, y0 + rowsPerWI); y < y1; ++y, dst_index "
    "+= dst_step)\n"
    "{\n"
    ...
    "#define DECLARE_DST_PARAM(index) , __global uchar * dst##index##ptr, int "
    "dst##index##_step, int dst##index##_offset\n"
    "#define DECLARE_INDEX(index) int dst##index##_index = mad24(y0, "
    "dst##index##_step, mad24(x, (int)sizeof(T), dst##index##_offset));\n"
    "#define PROCESS_ELEM(index) \\\n"
    "__global T * dst##index = (__global T *)(dst##index##ptr + "
    "dst##index##_index); \\\n"
    "dst##index[0] = src[index]; \\\n"
    "dst##index##_index += dst##index##_step;\n"
    "__kernel void split(__global uchar* srcptr, int src_step, int src_offset, "
    "int rows, int cols DECLARE_DST_PARAMS, int rowsPerWI)\n"
    "{\n"
    "int x = get_global_id(0);\n"
    "int y0 = get_global_id(1) * rowsPerWI;\n"
    "if (x < cols)\n"
    "{\n"
    "DECLARE_INDEX_N\n"
    "int src_index = mad24(y0, src_step, mad24(x, cn * (int)sizeof(T), "
    "src_offset));\n"
    "for (int y = y0, y1 = min(rows, y0 + rowsPerWI); y < y1; ++y, src_index "
    "+= src_step)\n"
    "{\n"
...
struct cv::ocl::internal::ProgramEntry transpose_oclsrc = {
    ...
    "__kernel void transpose(__global const uchar * srcptr, int src_step, int "
    "src_offset, int src_rows, int src_cols,\n"
    "__global uchar * dstptr, int dst_step, int dst_offset)\n"
    "{\n"
    ...
    "{\n"
    "int index_dst = mad24(y_index, dst_step, mad24(x_index, TSIZE, "
    "dst_offset));\n"
    "#pragma unroll\n"
    "for (int i = 0; i < TILE_DIM; i += BLOCK_ROWS)\n"
    ...
    "}\n"
    "#else\n"
    "__kernel void transpose_inplace(__global uchar * srcptr, int src_step, "
    ...
    "T tmp;\n"
    "#pragma unroll\n"
    "for (int i = 0; i < rowsPerWI; ++i, ++y, src_index += src_step, dst_index "
    "+= TSIZE)\n"
    "if (y < src_rows && x < y)\n"
    "{\n"
