...
struct cv::ocl::internal::ProgramEntry akaze_oclsrc = {
    moduleName, "akaze",
    "__kernel void\n"
    "AKAZE_pm_g2(__global const float* lx, __global const float* ly, __global "
    "float* dst,\n"
    "float k, int size)\n"
    "{\n"
    ...
    "}\n"
    "__kernel void\n"
    "AKAZE_nld_step_scalar(__global const float* lt, int lt_step, int "
    "lt_offset, int rows, int cols,\n"
    ...
    "}\n"
    "__kernel void\n"
    "AKAZE_compute_determinant(__global const float* lxx, __global const "
    "float* lxy, __global const float* lyy,\n"
...
struct cv::ocl::internal::ProgramEntry brute_force_match_oclsrc = {
    moduleName, "brute_force_match",
    ...
    "#    define SHARED_MEM_SZ      (kercn * (BLOCK_SIZE * MAX_DESC_LEN + "
    ...
    "#           define DIST(x, y) {value_type d = fabs((x) - (y)); result += "
    "d.s0 + d.s1 + d.s2 + d.s3 + d.s4 + d.s5 + d.s6 + d.s7;}\n"
    "#       elif (4 == kercn)\n"
    ...
    "#           define DIST(x, y) {value_type d = abs((x) - (y)); result += "
    "d.s0 + d.s1 + d.s2 + d.s3 + d.s4 + d.s5 + d.s6 + d.s7;}\n"
    "#       elif (4 == kercn)\n"
    ...
    "#   if (8 == kercn)\n"
    "typedef float8 value_type;\n"
    "#       define DIST(x, y)   {value_type d = ((x) - (y)); result += "
    "dot(d.s0123, d.s0123) + dot(d.s4567, d.s4567);}\n"
    "#   elif (4 == kercn)\n"
    "typedef float4      value_type;\n"
    "#       define DIST(x, y)   {value_type d = ((x) - (y)); result += dot(d, "
    "d);}\n"
    "#   else\n"
    "typedef float       value_type;\n"
    "#       define DIST(x, y)   {value_type d = ((x) - (y)); result = mad(d, "
    "d, result);}\n"
    "#   endif\n"
    ...
    "{\n"
    "DIST(s_query[lidy * BLOCK_SIZE_ODD + j], s_train[j * BLOCK_SIZE_ODD + "
    "lidx]);\n"
    "}\n"
    ...
    "for (int j = 0 ; j < BLOCK_SIZE ; j++)\n"
    "{\n"
    "DIST(s_query[lidy * MAX_DESC_LEN + block_index * BLOCK_SIZE + j], "
    ...
    "#if 0 < MAX_DESC_LEN\n"
    "__local value_type *s_train = (__local value_type *)sharebuffer + "
    "BLOCK_SIZE * MAX_DESC_LEN;\n"
    "#pragma unroll\n"
    "for (int i = 0; i < MAX_DESC_LEN / BLOCK_SIZE; i++)\n"
    "{\n"
    "const int loadx = mad24(BLOCK_SIZE, i, lidx);\n"
    "s_query[mad24(MAX_DESC_LEN, lidy, loadx)] = loadx < query_cols ? "
    "query_vec[loadx] : 0;\n"
    "}\n"
    "#else\n"
    "__local value_type *s_train = (__local value_type *)sharebuffer + "
    "BLOCK_SIZE_ODD * BLOCK_SIZE;\n"
    ...
    "{\n"
    "result_type result = 0;\n"
    "const int trainOffset = min(mad24(BLOCK_SIZE, t, lidy), train_rows - 1) * "
    "step;\n"
    ...
    "{\n"
    "const int loadx = mad24(BLOCK_SIZE, i, lidx);\n"
    "s_train[mad24(BLOCK_SIZE, lidx, lidy)] = loadx < train_cols ? "
    "train_vec[loadx] : 0;\n"
    ...
    "}\n"
    "#else\n"
    "for (int i = 0, endq = (query_cols + BLOCK_SIZE - 1) / BLOCK_SIZE; i < "
    "endq; i++)\n"
    "{\n"
    ...
    "if (queryIdx < query_rows && trainIdx < train_rows && result < "
    "myBestDistance )\n"
    "{\n"
    ...
    "__local int *s_trainIdx = (__local int *)(sharebuffer + BLOCK_SIZE_ODD * "
    "BLOCK_SIZE);\n"
    ...
    "const int trainOffset = min(mad24(BLOCK_SIZE, groupidx, lidy), train_rows "
    "- 1) * step;\n"
    "__global TN *train_vec = (__global TN *)(train + trainOffset);\n"
    "query_cols /= kercn;\n"
    "__local float sharebuffer[SHARED_MEM_SZ];\n"
    ...
    "{\n"
    "result_type result = 0;\n"
    "int trainOffset = min(mad24(BLOCK_SIZE, t, lidy), train_rows - 1) * "
    "step;\n"
    "__global TN *train_vec = (__global TN *)(train + trainOffset);\n"
    "#if 0 < MAX_DESC_LEN\n"
    "#pragma unroll\n"
...
struct cv::ocl::internal::ProgramEntry fast_oclsrc = {
    moduleName, "fast",
    ...
    "tofs = ofs; d[idx] = (short)(v - img[tofs]); d[idx+8] = (short)(v - "
    "img[-tofs])\n"
    "LOAD2(0, 3);\n"
    "LOAD2(1, -step+3);\n"
    "LOAD2(2, -step*2+2);\n"
...
struct cv::ocl::internal::ProgramEntry orb_oclsrc = {
    moduleName, "orb",
    ...
    "#ifdef ORB_RESPONSES\n"
    "__kernel void\n"
    "ORB_HarrisResponses(__global const uchar* imgbuf, int imgstep, int "
    "imgoffset0,\n"
    ...
    "(kpt[KEYPOINT_Y] - blockSize/2)*imgstep + (kpt[KEYPOINT_X] - "
    "blockSize/2);\n"
    "int i, j;\n"
    "int a = 0, b = 0, c = 0;\n"
    ...
    "{\n"
    "int Ix = (img[1] - img[-1])*2 + img[-imgstep+1] - img[-imgstep-1] + "
    "img[imgstep+1] - img[imgstep-1];\n"
    "int Iy = (img[imgstep] - img[-imgstep])*2 + img[imgstep-1] - "
    "img[-imgstep-1] + img[imgstep+1] - img[-imgstep+1];\n"
    "a += Ix*Ix;\n"
    "b += Iy*Iy;\n"
    "c += Ix*Iy;\n"
    "}\n"
    "}\n"
    "responses[idx] = ((float)a * b - (float)c * c - HARRIS_K * (float)(a + b) "
    "* (a + b))*scale_sq_sq;\n"
    "}\n"
    "}\n"
    "#endif\n"
    ...
    "__kernel void\n"
    "ORB_computeDescriptor(__global const uchar* imgbuf, int imgstep, int "
    "imgoffset0,\n"
    "__global const int* layerinfo, __global const int* keypoints,\n"
    ...
    "center[mad24(convert_int_rte(pattern[(idx)*2] * sina + pattern[(idx)*2+1] "
    "* cosa), imgstep, \\\n"
