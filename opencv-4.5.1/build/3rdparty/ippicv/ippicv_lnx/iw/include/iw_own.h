...
#define OWN_SAFE_COPY(SRC, DST, SIZE)                                          \
...
#endif
#if defined IW_ENABLE_DATA_TYPE_32U || defined IW_ENABLE_DATA_TYPE_32S ||      \
    defined IW_ENABLE_DATA_TYPE_32F
#define IW_ENABLE_DATA_DEPTH_32 1
#endif
#if defined IW_ENABLE_DATA_TYPE_64U || defined IW_ENABLE_DATA_TYPE_64S ||      \
    defined IW_ENABLE_DATA_TYPE_64F
...
static IW_INLINE Ipp8u *ownCastArray_64f8u(const Ipp64f *pVal, Ipp64f *pBuffer,
                                           IwSize len) {
  OWN_ARRAY_SATURATED_CAST(Ipp8u, 8U)
  return (Ipp8u *)pBuffer;
}
static IW_INLINE Ipp8s *ownCastArray_64f8s(const Ipp64f *pVal, Ipp64f *pBuffer,
                                           IwSize len) {
  OWN_ARRAY_SATURATED_CAST(Ipp8s, 8S)
  return (Ipp8s *)pBuffer;
}
static IW_INLINE Ipp16u *ownCastArray_64f16u(const Ipp64f *pVal,
                                             Ipp64f *pBuffer, IwSize len) {
  OWN_ARRAY_SATURATED_CAST(Ipp16u, 16U)
  return (Ipp16u *)pBuffer;
}
static IW_INLINE Ipp16s *ownCastArray_64f16s(const Ipp64f *pVal,
                                             Ipp64f *pBuffer, IwSize len) {
  OWN_ARRAY_SATURATED_CAST(Ipp16s, 16S)
  return (Ipp16s *)pBuffer;
}
static IW_INLINE Ipp32u *ownCastArray_64f32u(const Ipp64f *pVal,
                                             Ipp64f *pBuffer, IwSize len) {
  OWN_ARRAY_SATURATED_CAST(Ipp32u, 32U)
  return (Ipp32u *)pBuffer;
}
static IW_INLINE Ipp32s *ownCastArray_64f32s(const Ipp64f *pVal,
                                             Ipp64f *pBuffer, IwSize len) {
  OWN_ARRAY_SATURATED_CAST(Ipp32s, 32S)
  return (Ipp32s *)pBuffer;
}
static IW_INLINE Ipp32f *ownCastArray_64f32f(const Ipp64f *pVal,
                                             Ipp64f *pBuffer, IwSize len) {
  ippsConvert_64f32f(pVal, (Ipp32f *)pBuffer, (int)len);
  return (Ipp32f *)pBuffer;
}
static IW_INLINE Ipp64f *ownCastArray_64f64f(const Ipp64f *pVal,
                                             Ipp64f *pBuffer, IwSize len) {
  ippsCopy_64f(pVal, pBuffer, (int)len);
  return pBuffer;
}
static void ownCastArray_64f(const Ipp64f *pVal, Ipp64f *pBuffer,
                             IppDataType dataType, IwSize len) {
  switch (dataType) {
  case ipp8u:
    ownCastArray_64f8u(pVal, pBuffer, len);
    return;
  case ipp8s:
    ownCastArray_64f8s(pVal, pBuffer, len);
    return;
  case ipp16u:
    ownCastArray_64f16u(pVal, pBuffer, len);
    return;
  case ipp16s:
    ownCastArray_64f16s(pVal, pBuffer, len);
    return;
  case ipp32u:
    ownCastArray_64f32u(pVal, pBuffer, len);
    return;
  case ipp32s:
    ownCastArray_64f32s(pVal, pBuffer, len);
    return;
  case ipp32f:
    ownCastArray_64f32f(pVal, pBuffer, len);
    return;
  case ipp64f:
    ownCastArray_64f64f(pVal, pBuffer, len);
    return;
  default:
    return;
  }
...
static IW_INLINE Ipp32f ownCast_64f32f(Ipp64f val) {
  if (val > IPP_MAXABS_32F)
    val = IPP_MAXABS_32F;
  else if (val < -IPP_MAXABS_32F)
    val = -IPP_MAXABS_32F;
  return (Ipp32f)val;
}

typedef void *(*OwnCastArray_ptr)(const Ipp64f *pVal, Ipp64f *pBuffer,
                                  IwSize len);
...
IW_DECL(void)
ownVector_Init(OwnVector *pVector, size_t elemSize, size_t reserve);
