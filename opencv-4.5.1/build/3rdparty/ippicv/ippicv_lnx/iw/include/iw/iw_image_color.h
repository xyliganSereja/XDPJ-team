...
// some standard generally accepted way. In essence, a color model is a
// specification of a 3-D coordinate system and a subspace within that system
...
// Transferring color information from one industry to another requires
...
//      If color format is of planar type then returns number channels for the
//      specified plane. If color format is undefined or number of planes is
//      incorrect then returns 0.
IW_DECL(int)
iwiColorToChannels(IwiColorFmt color, // Color format
                   int planeNum // Number of plane for planar format, from 0.
                                // Ignored for interleaved formats
);
...
//      If color format is of planar type then returns size for the specified
//      plane. If color format is undefined or number of planes is incorrect
//      then returns {0,0}.
IW_DECL(IwiSize)
...
// Color conversions aggregation. Performs conversions between every supported
// color types Features support:
//      Inplace mode:            no
//      64-bit sizes:            no
...
IW_DECL(IppStatus)
iwiColorConvert(
    const IwiImage *const pSrcImage[], // [in]     Pointer to an array of
                                       // pointers to source images
    IwiColorFmt srcFormat,             // [in]     Source image color format
    IwiImage *const pDstImage[], // [in,out] Pointer to an array of pointers to
                                 // destination images
    IwiColorFmt dstFormat,       // [in]     Destination image color format
    Ipp64f alphaVal, // [in]     Value to set for alpha channel if new alpha
                     // channels is to be created
    const IwiColorConvertParams
        *pAuxParams, // [in]     Pointer to the auxiliary parameters structure.
                     // If NULL - default parameters will be used
    const IwiTile
        *pTile // [in,out] Pointer to IwiTile structure for tiling. If NULL -
