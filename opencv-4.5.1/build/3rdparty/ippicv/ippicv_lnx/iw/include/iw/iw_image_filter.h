...
typedef struct _IwiFilterParams {
  double divisor; // The value by which the computed result is divided for 16s
                  // kernel type. If divisor is 0 and the sum of the kernel
                  // elements is not 0 then result will be normalized by the sum
                  // of the kernel elements Only integer values are supported
  int offset; // An offset which will be added to the final signed result before
              // converting it to unsigned for 8u and 16u
...
//      ippStsNotEvenStepErr                step value is not divisible by size
//      of elements ippStsNotSupportedModeErr           selected function mode
//      is not supported ippStsNoMemErr                      failed to allocate
//      memory ippStsInplaceModeNotSupportedErr    doesn't support output into
//      the source buffer ippStsNullPtrErr                    unexpected NULL
//      pointer ippStsNoErr                         no errors
IW_DECL(IppStatus)
iwiFilter(
    const IwiImage *pSrcImage, // [in]     Pointer to the source image
    IwiImage *pDstImage,       // [in,out] Pointer to the destination image
    const IwiImage *pKernel,   // [in]     Pointer to the filter kernel image.
                               // Kernel must be continuous, [16s,32f], [C1]
    const IwiFilterParams
        *pAuxParams, // [in]     Pointer to the auxiliary parameters structure.
                     // If NULL - default parameters will be used
    IwiBorderType border, // [in]     Extrapolation algorithm for out of image
                          // pixels: ippBorderConst, ippBorderRepl,
                          // ippBorderMirror, ippBorderInMem
...
iwiFilterSobel(
    ...
    IwiImage *pDstImage,       // [in,out] Pointer to the destination image
    IwiDerivativeType
        opType, // [in]     Type of derivative from IwiDerivativeType
    IppiMaskSize kernelSize, // [in]     Size of the filter kernel:
                             // ippMskSize3x3, ippMskSize5x5
    const IwiFilterSobelParams
        *pAuxParams, // [in]     Pointer to the auxiliary parameters structure.
                     // If NULL - default parameters will be used
    IwiBorderType border, // [in]     Extrapolation algorithm for out of image
                          // pixels: ippBorderConst, ippBorderRepl,
...
iwiFilterLaplacian(
    ...
    IppiMaskSize kernelSize,   // [in]     Size of the filter kernel:
                               // ippMskSize3x3, ippMskSize5x5
    const IwiFilterLaplacianParams
        *pAuxParams, // [in]     Pointer to the auxiliary parameters structure.
                     // If NULL - default parameters will be used
    IwiBorderType border, // [in]     Extrapolation algorithm for out of image
                          // pixels: ippBorderConst, ippBorderRepl,
...
iwiFilterGaussian(
    const IwiImage *pSrcImage, // [in]     Pointer to the source image
    IwiImage *pDstImage,       // [in,out] Pointer to the destination image
    int kernelSize, // [in]     Size of the Gaussian kernel (odd, greater or
                    // equal to 3)
    double sigma,   // [in]     Standard deviation of the Gaussian kernel
    const IwiFilterGaussianParams
        *pAuxParams, // [in]     Pointer to the auxiliary parameters structure.
                     // If NULL - default parameters will be used
    IwiBorderType border, // [in]     Extrapolation algorithm for out of image
                          // pixels: ippBorderConst, ippBorderRepl,
...
iwiFilterGaussian_InitAlloc(
    IwiFilterGaussianSpec *
        *pSpec,   // [out]    Pointer to pointer to internal spec structure.
                  // Structure will be allocated here.
...
//      initialized or of invalid type ippStsNoMemErr failed to allocate memory
//      ippStsSizeErr                       size fields values are illegal
//      ippStsInplaceModeNotSupportedErr    doesn't support output into the
//      source buffer ippStsNullPtrErr                    unexpected NULL
//      pointer ippStsNoErr                         no errors
...
iwiFilterGaussian_Process(
    const IwiFilterGaussianSpec
        *pSpec,                // [in]     Pointer to internal spec structure
    const IwiImage *pSrcImage, // [in]     Pointer to the source image
    IwiImage *pDstImage,       // [in,out] Pointer to the destination image
    IwiBorderType
        border, // [in]     Extrapolation algorithm for out of image pixels:
                // ippBorderConst, ippBorderRepl, ippBorderInMem
    const Ipp64f *pBorderVal, // [in]     Pointer to array of border values for
                              // ippBorderConst. One element for each channel.
                              // Can be NULL for any other border
    const IwiTile
...
typedef struct _IwiFilterCannyParams {
  IppiDifferentialKernel
      kernel; // Type of differential kernel: ippFilterSobel, ippFilterScharr
  IppiMaskSize
      kernelSize;   // Size of the filter kernel: ippFilterSobel: ippMskSize3x3,
                    // ippMskSize5x5; ippFilterScharr: ippMskSize3x3
...
//      ippStsNumChannelsErr                channels value is illegal
//      ippStsNotEvenStepErr                step value is not divisible by size
//      of elements ippStsNoMemErr                      failed to allocate
//      memory ippStsSizeErr                       size fields values are
//      illegal ippStsNullPtrErr                    unexpected NULL pointer
//      ippStsNoErr                         no errors
IW_DECL(IppStatus)
iwiFilterCannyDeriv(
    const IwiImage
        *pSrcImageDx, // [in]     Pointer to X derivative of the source image
    const IwiImage
        *pSrcImageDy,    // [in]     Pointer to Y derivative of the source image
    IwiImage *pDstImage, // [in,out] Pointer to the destination image
    Ipp32f treshLow,     // [in]     Lower threshold for edges detection
    Ipp32f treshHigh,    // [in]     Upper threshold for edges detection
    const IwiFilterCannyDerivParams
        *pAuxParams // [in]     Pointer to the auxiliary parameters structure.
                    // If NULL - default parameters will be used
);
...
iwiFilterMorphology(
    ...
    const IwiImage
        *pMaskImage, // [in]     Pointer to morphology mask image. Mask must be
                     // continuous, 8-bit, 1 channel image
    const IwiFilterMorphologyParams
        *pAuxParams, // [in]     Pointer to the auxiliary parameters structure.
                     // If NULL - default parameters will be used
    IwiBorderType border, // [in]     Extrapolation algorithm for out of image
                          // pixels: ippBorderConst, ippBorderRepl,
...
// Returns:
//      ippStsNotSupportedModeErr           selected function mode is not
//      supported ippStsNullPtrErr                    unexpected NULL pointer
//      ippStsNoErr                         no errors
IW_DECL(IppStatus)
...
typedef struct _IwiFilterBilateralParams {
  IppiFilterBilateralType
      filter; // Type of bilateral filter: ippiFilterBilateralGauss
  IppiDistanceMethodType distMethod; // Method for definition of distance
                                     // between pixel intensity: ippDistNormL1
...
//      Inplace mode:            no
//      64-bit sizes:            yes
//      Internal threading:      yes (check IW_ENABLE_THREADING_LAYER
//      definition) Manual tiling:           yes IwiTile simple tiling:   yes
//      IwiTile pipeline tiling: yes
// Returns:
//      ippStsSizeErr                       size fields values are illegal
//      ippStsBadArgErr                     valSquareSigma or posSquareSigma is
//      less or equal 0 ippStsMaskSizeErr                   radius is less or
//      equal 0 ippStsBorderErr                     border value is illegal
//      ippStsDataTypeErr                   data type is illegal
//      ippStsNumChannelsErr                channels value is illegal
//      ippStsNotEvenStepErr                step value is not divisible by size
//      of elements ippStsNotSupportedModeErr           filter or distMethod is
//      not supported ippStsNoMemErr                      failed to allocate
//      memory ippStsInplaceModeNotSupportedErr    doesn't support output into
//      the source buffer ippStsNullPtrErr                    unexpected NULL
//      pointer ippStsNoErr                         no errors
IW_DECL(IppStatus)
iwiFilterBilateral(
    const IwiImage *pSrcImage, // [in]     Pointer to the source image
    IwiImage *pDstImage,       // [in,out] Pointer to the destination image
    int radius, // [in]     Radius of circular neighborhood what defines pixels
                // for calculation
    Ipp32f valSquareSigma, // [in]     Square of Sigma for factor function for
                           // pixel intensity
    Ipp32f posSquareSigma, // [in]     Square of Sigma for factor function for
                           // pixel position
    const IwiFilterBilateralParams
        *pAuxParams, // [in]     Pointer to the auxiliary parameters structure.
                     // If NULL - default parameters will be used
    IwiBorderType border, // [in]     Extrapolation algorithm for out of image
                          // pixels: ippBorderConst, ippBorderRepl,
