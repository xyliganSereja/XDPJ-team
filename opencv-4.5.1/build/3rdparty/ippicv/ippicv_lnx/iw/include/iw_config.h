...
/*
    These switches are used during IW library compilation to customize the
   library code and decrease memory footprint of the library.
*/
...
#define IW_ENABLE_THREADING_LAYER                                              \
  0 // Enables Intel IPP Threading Layer calls inside IW if possible (requires
    // OpenMP support)
#endif // Parallel version of functions will be used if:
       // 1. There is a parallel implementation for a particular function (see
       // function description in the header)
       // 2. If iwGetThreadsNum() function result is greater than 1 before
       // functions call or spec initialization call Note: tiling cannot be used
       // with internal threading. IwiTile parameter will be ignored if
       // conditions above are true before function call To disable threading on
       // run time: call iwSetThreadsNum(1) before a function call

#ifndef IW_ENABLE_TLS
#define IW_ENABLE_TLS                                                          \
  1 // Enables use of Thread Local Storage. This adds dependency on POSIX
...
// These switches can remove Intel IPP functions calls with some data types to
// reduce memory footprint. Functions which operates with several types and
// channels will be enabled if at least one of parameters has enabled type Note
// that some functionality can become completely disabled if some of these
// defines are switched off
