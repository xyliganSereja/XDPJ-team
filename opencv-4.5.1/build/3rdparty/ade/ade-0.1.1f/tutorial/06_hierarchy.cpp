...
using TGraph =
    ade::TypedGraph<Type, Operation, Wrap, ade::passes::TopologicalSortData>;
...
ade::NodeHandle wrap(TGraph &graph, std::shared_ptr<ade::Graph> g,
                     const std::string &op) {
...
int main(int argc, char *argv[]) {
  ...
  ade::NodeHandle foo = m::op(trms, "foo"), bar = m::op(trms, "bar"),
                  baz = m::op(trms, "baz");
  ...
  ade::NodeHandle pre = m::op(tsuper, "pre"), fbb = m::wrap(tsuper, rms, "fbb"),
