...
using TGraph =
    ade::TypedGraph<Dirty, Format, Type, Operation, Data, Const, Storage, Link,
                    Internals, Visited, ade::passes::TopologicalSortData>;

using CTGraph = ade::ConstTypedGraph<Dirty, Format, Type, Operation, Data,
                                     Const, Storage, Link, Internals, Visited,
...
void ExtractInternals(ade::passes::PassContext &ctx) {
  // This pass stores function nodes and data nodes in two different lists (for
  // easier access)
  ...
  for (auto nh : g.nodes()) {
    ...
    case X::Type::OP:
      ints.operations.push_back(nh);
      break;
    case X::Type::DATA:
      ints.data.push_back(nh);
      break;
    default:
      ADE_ASSERT(false);
    }
  }
  std::cout << " Done: " << ints.operations.size() << " operations, "
            << ints.data.size() << " data objects." << std::endl;
  g.metadata().set(ints);
}

void CheckIOData(ade::passes::PassContext &ctx) {
  // This pass checks if there's physical buffers assigned to graph
  // inputs/outputs
  ...
  for (auto nh : ints.data) {
    if (g.metadata(nh).get<X::Data>().acc == X::REAL) {
      if (nh->inNodes().size() == 0 && nh->outNodes().size() != 0) {
        in_found = true;
        if (!g.metadata(nh).contains<X::Format>() ||
            !g.metadata(nh).contains<X::Storage>())
          throw std::logic_error(
              "Format/storage information is not found for input object");
      }
      if (nh->inNodes().size() != 0 && nh->outNodes().size() == 0) {
        out_found = true;
        if (!g.metadata(nh).contains<X::Format>() ||
            !g.metadata(nh).contains<X::Storage>())
          throw std::logic_error(
              "Format/storage information is not found for output object");
      }
      if (nh->inNodes().size() != 0 && nh->outNodes().size() != 0)
        throw std::logic_error(
            "Write through real objects is not supported in this sample");
    }
  }
  if (!in_found)
...
void CheckPorts(ade::passes::PassContext &ctx) {
  ...
  for (auto nh : ints.operations) {
    ...
    for (auto e : nh->inEdges()) {
      ...
      if (writes > 1)
        throw std::logic_error(
            "Multiple data objects on the same operation input port");
    }
...
void ResolveMeta(ade::passes::PassContext &ctx) {
  // This pass checks asks every operation to check its input data format and
  // fill output data format It is important to iterate over operations in
  // topological order (to ensure validators are called with valid/resolved data
  // on inputs
  std::cout << "BEGIN Metadata resolution" << std::endl;

  const X::TGraph g(
      ctx.graph);          // constant typed graph (for invoking validators)
  ...
  for (auto nh : sorted.nodes()) {
    ...
    if (g.metadata(nh).get<X::Type>().t == X::Type::OP) {
      ...
      // Put input/output handles in an expected order. inputs/outputs are DATA
      // objects!
      ...
      for (auto in_e : nh->inEdges()) {
        inputs[g.metadata(in_e).get<X::Link>().port] =
            g.metadata(in_e->srcNode()).get<X::Format>().size;
      }
...
void RemoveDeadEnds(ade::passes::PassContext &ctx) {
  // This pass identifies which nodes don't contribute to generating REAL
  // outputs, thus can be removed safely. If there were dead ends removed, this
  // pass also sets "Dirty" flag, causing one more compilation round.
...
public:
  explicit Executable(const ade::Graph &gr) : m_gr(gr) {
    ...
    std::cout << "X::Executable: " << sizeof(float) * total
              << " bytes allocated" << std::endl;
  }

  virtual void run() override {
    // Invoke functional units in topological order, supplying neccessary
    // information
    ...
    for (auto nh : sorted.nodes()) {
      ...
      if (m_gr.metadata(nh).get<X::Type>().t == X::Type::OP) {
        ...
        // Here we prepare a invocation context for every functional unit we
        // vist, on every run. Sure it can be done only once and stored in Meta
        // in backend-specific format
        const int num_inputs = op_meta.op->inputs(),
        ...
        for (auto in_e : nh->inEdges()) {
          ...
          in_data[port] =
              m_gr.metadata(in_e->srcNode()).get<X::Data>().acc == VIRT
        ...
        for (auto out_e : nh->outEdges()) {
          ...
          out_data[port] =
              m_gr.metadata(out_e->dstNode()).get<X::Data>().acc == VIRT
                  ? m_memory[out_e->dstNode().get()].get()
                  : m_gr.metadata(out_e->dstNode()).get<X::Storage>().ptr;
          out_metas[port] =
              m_gr.metadata(out_e->dstNode()).get<X::Format>().size;
        }

        op_meta.op->apply(m_gr.metadata(nh), in_metas, in_data, out_metas,
                          out_data);
      }
    }
  }
...
struct Backend : public ade::ExecutionBackend {
  virtual void
  setupExecutionEngine(ade::ExecutionEngineSetupContext &ectx) override {
    // Add new "init" pass right before "validation" - we do put graph
    // transformations here which then needs to be verified with common routines
    ectx.addPass("init", "put_extra_buffers",
                 [](ade::passes::PassContext &ctx) {
                   // On the backend level, we know our operations don't support
                   // partial outputs - i.e., when there's operation with
                   // multiple outputs and only one of them is consumed. So put
                   // extra data virtual objects to make all outputs valid
                   X::TGraph g(ctx.graph);
                   auto ints = g.metadata().get<X::Internals>();
                   for (auto nh : ints.operations) {
                     auto op_meta = g.metadata(nh).get<X::Operation>();
                     ...
                     for (auto it : ade::util::indexed(out_edges)) {
                       ...
                       if (!oe.get()) {
                         ...
                         std::cout << "Put extra data for " << op_meta.label
                                   << " port " << port << std::endl;
                       }
                     }
                   }
  ...
  virtual std::unique_ptr<ade::BackendExecutable>
  createExecutable(const ade::Graph &gr) override {
...
struct UnaryCOp : public UnaryOp {
  virtual void validate(const TGraph::CMetadataT &node_meta,            // in
    ...
    // Check if this node meta also contains a constant value required for
    // operation
    if (!node_meta.contains<Const>())
      throw std::logic_error(
          "No constant data found for operation with Constant!");
...
template <typename F> void Apply(const ade::memory::DynMdSize &dims, F f) {
...
struct Add : public BinaryOp {
  virtual void apply(const CTGraph::CMetadataT &node_meta,                 // in
    ...
    Apply(out_metas[0],
          [&](const int p) { out_data[0][p] = in_data[0][p] + in_data[1][p]; });
  }
};
...
struct Sqrt : public UnaryOp {
  virtual void apply(const CTGraph::CMetadataT &node_meta,                 // in
    ...
    Apply(out_metas[0],
          [&](const int p) { out_data[0][p] = std::sqrt(in_data[0][p]); });
  }
};
...
struct AddC : public UnaryCOp {
  virtual void apply(const CTGraph::CMetadataT &node_meta,                 // in
    ...
    Apply(out_metas[0],
          [&](const int p) { out_data[0][p] = in_data[0][p] + cval; });
  }
};
...
public:
  explicit Builder(TGraph &g) : m_g(g) { m_g.metadata().set(Dirty{false}); }

  template <typename T> Builder &node(const char *name) {
  ...
  Builder &link(const char *src, int src_port, const char *dst, int dst_port) {
    ...
    ade::NodeHandle nh, srch = src_it->second, dsth = dst_it->second;
  ...
  Builder &bind(const char *name, float *data,
                const ade::memory::DynMdSize &dims) {
    ade::NodeHandle nh = data_handle(name);
...
int main(int argc, char *argv[]) {
  ...
  // In this example, size/buf_size/strides for input/output buffers area
  // actually equal, but still defined separately for modifications/testing
  // purposes (demo).
  ...
  builder.bind("in1", in_1.data(), sz_in)
  ...
  engine.addPass("validation", "extract_internals",
                 X::Passes::ExtractInternals);
  ...
  engine.addPass("validation", "topological_sort",
                 ade::passes::TopologicalSort());
