...
TEST(ChainRangeTest, SameElemTypes2) {
  ...
  for (auto &elem :
       ade::util::chain(ade::util::toRange(arr1), ade::util::toRange(arr2))) {
    static_assert(std::is_same<decltype(elem), int &>::value, "Invalid type");
    result.push_back(elem);
  }
...
TEST(ChainRangeTest, DifferentElemTypes3) {
  ...
  for (auto elem :
       ade::util::chain(ade::util::toRange(arr1), ade::util::toRange(arr2),
                        ade::util::toRange(arr3))) {
    static_assert(std::is_same<decltype(elem), long long>::value,
                  "Invalid type");
...
TEST(ChainRangeTest, Test4) {
  ...
  for (auto elem :
       ade::util::chain(ade::util::toRange(arr1), ade::util::toRange(arr2),
                        ade::util::toRange(arr3), ade::util::iota(9, 12))) {
    static_assert(std::is_same<decltype(elem), int>::value, "Invalid type");
    result.push_back(elem);
  }
...
TEST(ChainRangeTest, SizeAndOpIndex) {
  ...
  auto range =
      ade::util::chain(ade::util::memory_range(arr1.data(), arr1.size()),
