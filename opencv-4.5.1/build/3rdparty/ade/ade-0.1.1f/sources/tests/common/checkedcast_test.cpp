...
struct CheckedCastTestHandler final {
  template <typename T> void operator()(bool valid, T && /*value*/) const {
    if (!valid) {
...
template <typename I, typename J> I checked_cast_test(J value) {
...
TEST(CheckedCastTest, CheckValid) {
  ...
  EXPECT_EQ((unsigned)0xffff,
            checked_cast_test<unsigned>((unsigned short)0xffff));

  EXPECT_EQ((short)0x7fff, checked_cast_test<short>((int)0x7fff));
  EXPECT_EQ((unsigned short)0xffff,
            checked_cast_test<unsigned short>((unsigned)0xffff));
  ...
  EXPECT_EQ(static_cast<unsigned>(1000000.0f),
            checked_cast_test<unsigned>(1000000.0f));
  ...
  EXPECT_EQ(static_cast<float>(-1000000.0),
            checked_cast_test<float>(-1000000.0));

  for (int i = std::numeric_limits<char>::min();
       i <= std::numeric_limits<char>::max(); ++i) {
    EXPECT_EQ(static_cast<char>(static_cast<float>(i)),
              checked_cast_test<char>(static_cast<float>(i)));
  }
}
...
TEST(CheckedCastTest, CheckOverflows) {
  ...
  EXPECT_THROW(checked_cast_test<float>(std::numeric_limits<double>::max()),
               CheckedCastFailed);
  EXPECT_THROW(checked_cast_test<float>(std::numeric_limits<double>::lowest()),
               CheckedCastFailed);

  EXPECT_THROW(checked_cast_test<int>(std::numeric_limits<float>::infinity()),
               CheckedCastFailed);
  EXPECT_THROW(checked_cast_test<int>(-std::numeric_limits<float>::infinity()),
               CheckedCastFailed);
  EXPECT_THROW(checked_cast_test<int>(std::numeric_limits<float>::quiet_NaN()),
               CheckedCastFailed);

  EXPECT_THROW(
      checked_cast_test<unsigned>(std::numeric_limits<float>::infinity()),
      CheckedCastFailed);
  EXPECT_THROW(
      checked_cast_test<unsigned>(-std::numeric_limits<float>::infinity()),
      CheckedCastFailed);
  EXPECT_THROW(
      checked_cast_test<unsigned>(std::numeric_limits<float>::quiet_NaN()),
      CheckedCastFailed);
}
