...
TEST(TupleTest, FixRvals) {
  ...
  static_assert(
      std::is_same<std::tuple_element<0, decltype(t)>::type, int &>::value,
      "Invalid elements type");
  static_assert(std::is_same<std::tuple_element<1, decltype(t)>::type,
                             const int &>::value,
                "Invalid elements type");
  static_assert(
      std::is_same<std::tuple_element<2, decltype(t)>::type, int &>::value,
      "Invalid elements type");
  static_assert(
      std::is_same<std::tuple_element<3, decltype(t)>::type, int>::value,
      "Invalid elements type");
}
