...
TEST(ExecutionEngine, PassCallbacks) {
  ...
  engine.addPrePassCallback([&](const ExecutionEngine::PassDesc &desc,
  ...
  engine.addPostPassCallback([&](const ExecutionEngine::PassDesc &desc,
                                 const passes::PassContext &context) {
    EXPECT_EQ(&gr, &context.graph);
...
struct TestPass final {
  ...
  void operator()(passes::PassContext &) { ++state->testPassCalled; }
};
...
struct TestPassChecker final {
  ...
  bool nodeAboutToBeDestroyed(const Graph & /*graph*/,
  ...
  bool edgeAboutToBeDestroyed(const Graph & /*graph*/,
...
TEST(ExecutionEngine, LazyPass) {
  ...
  auto reset = [&]() { state = TestPassState{}; };

  {
    ...
    engine.addPass(
        "stage", "pass2", [](passes::PassContext &) {},
        std::array<std::string, 1>{{passName}});
  ...
  }
  {
    ...
    engine.addPass("stage", "pass1",
                   ...
                   {passName});
    engine.addPass("stage", "pass2",
                   [&](passes::PassContext &ctx) {
                     // Graph wasn't changed in previous pass so dependent pass
                     // shouldn't be called
