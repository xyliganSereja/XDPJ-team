...
public:
  ...
  std::function<void(const ade::EdgeHandle &, const NodeHandle &,
                     const NodeHandle &)>
      relinkEdge;

  virtual void nodeCreated(const Graph & /*graph*/,
                           const NodeHandle &node) override {
    createNode(node);
  }
  virtual void nodeAboutToBeDestroyed(const Graph & /*graph*/,
                                      const NodeHandle &node) override {
    destroyNode(node);
  }

  virtual void edgeCreated(const Graph & /*graph*/,
                           const EdgeHandle &edge) override {
    createEdge(edge);
  }
  virtual void edgeAboutToBeDestroyed(const Graph & /*graph*/,
                                      const EdgeHandle &edge) override {
    destroyEdge(edge);
  }
...
TEST(Graph, Listener) {
  ...
  listener.reset();
  listener.createNode = [&](const ade::NodeHandle &node) { tempNode = node; };
