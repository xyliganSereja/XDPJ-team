...
public:
  ...
  virtual void
  setupExecutionEngine(ExecutionEngineSetupContext & /*context*/) override {
    EXPECT_FALSE(setupCalled);
    setupCalled = true;
  }
  virtual std::unique_ptr<BackendExecutable>
  createExecutable(const Graph & /*graph*/) override {
    EXPECT_TRUE(setupCalled);
    EXPECT_EQ(nullptr, exec);
    std::unique_ptr<BackendExecutable> retval;
    if (!emptyExec) {
      retval.reset(createOpaq ? new TestBackendExecOpaq(this)
                              : new TestBackendExec(this));
    }
    return retval;
  }
};

TestBackendExec::TestBackendExec(TestBackend *e) : parent(e) {
...
template <typename T> inline std::unique_ptr<T> wrap_unique(T *ptr) {
...
TEST(ExecutionEngine, BackendsRunOpaq) {
  ...
  auto backend1 = new TestBackend;
  backend1->createOpaq = true;
  auto backend2 = new TestBackend;
  backend2->createOpaq = true;
  auto backend3 = new TestBackend;
  backend3->createOpaq = false;
