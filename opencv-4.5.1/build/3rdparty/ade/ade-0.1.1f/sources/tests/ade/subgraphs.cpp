...
TEST(Helpers, Subgraphs) {
  ...
  std::vector<ade::NodeHandle> nodes{
      gr.createNode(), gr.createNode(), gr.createNode(), gr.createNode(),
      gr.createNode(), gr.createNode(), gr.createNode(), gr.createNode(),
      gr.createNode(), gr.createNode()};

  std::vector<ade::NodeHandle> splitterNodes{gr.createNode(), gr.createNode(),
                                             gr.createNode(), gr.createNode()};
  ...
  const auto subgraphs =
      ade::splitSubgraphs(nodes, [&](const ade::EdgeHandle &edge) {
        return (splitterNodes.end() != std::find(splitterNodes.begin(),
                                                 splitterNodes.end(),
                                                 edge->srcNode())) ||
               (splitterNodes.end() != std::find(splitterNodes.begin(),
                                                 splitterNodes.end(),
                                                 edge->dstNode()));
      });
...
template <typename C> void sort_subgraphs(C &c) {
  std::sort(
      c.begin(), c.end(),
      [](const decltype(c.front()) &val1, const decltype(c.front()) &val2) {
        return val1.front().get() < val2.front().get();
      });
}

template <bool Value> struct Always {
  template <typename... T> bool operator()(T &&...) const { return Value; }
};
} // namespace
...
TEST(Helpers, AssembleSubgraphSimple) {
  ...
  ade::Graph gr;
  ade::NodeHandle nodes[] = {gr.createNode(), gr.createNode(), gr.createNode(),
                             gr.createNode(), gr.createNode(), gr.createNode()};
  ...
  for (auto &&node : nodes) {
    ...
    EXPECT_EQ((node_set(std::begin(nodes), std::end(nodes))),
              node_set(subgraph.begin(), subgraph.end()));
  }
}
  ...
  for (auto &&srcNode : acceptedNodes) {
    ...
    if (srcNode->outNodes().size() > 1) {
      for (auto &&dstNode : acceptedNodes) {
        ...
        if (srcNode != dstNode && dstNode->inNodes().size() > 1) {
...
TEST(Helpers, AssembleSubgraphCycles1) {
  ...
  auto mergeChecker = [&](const ade::EdgeHandle &edge,
  ...
  auto topoChecker = [&](const ade::subgraphs::NodesSet &acceptedNodes,
                         const ade::subgraphs::NodesSet &rejectedNodes) {
  ...
  ade::NodeHandle roots[] = {nodes[0], nodes[1], nodes[2],
                             nodes[4], nodes[6], nodes[7]};
  ...
  EXPECT_TRUE((exp0 == res0 && exp1 == res1) || (exp0 == res1 && exp1 == res0));
}
...
TEST(Helpers, AssembleSubgraphCycles2) {
  ...
  ade::NodeHandle roots[] = {nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
  ...
  auto res1 = node_set(subgraphs[1].begin(), subgraphs[1].end());
  auto exp0 =
      node_set{nodes[0], nodes[1], nodes[2], nodes[3], nodes[4], nodes[6]};
  auto exp1 =
      node_set{nodes[2], nodes[4], nodes[6], nodes[7], nodes[8], nodes[9]};
  EXPECT_TRUE((exp0 == res0 && exp1 == res1) || (exp0 == res1 && exp1 == res0));
}
...
TEST(Helpers, AssembleSubgraphCyclesStress2) {
  ...
  for (auto i : ade::util::iota(N)) {
    (void)i;
    ade::NodeHandle tempNodes[] = {gr.createNode(), gr.createNode(),
                                   gr.createNode(), gr.createNode(),
                                   gr.createNode()};
    if (nullptr != prevNode) {
      gr.link(prevNode, tempNodes[0]);
...
namespace {
std::size_t
getLongestSubgraph(const std::vector<std::vector<ade::NodeHandle>> &subgraphs) {
...
TEST(Helpers, SelectAllSubgraphs1) {
  ...
  auto subgraphs1 = ade::selectAllSubgraphs(
      nodes1,
      [&](const ade::EdgeHandle &edge, const ade::SubgraphMergeDirection dir) {
...
TEST(Helpers, SelectAllSubgraphs2) {
  ...
  auto subgraphs2 = ade::selectAllSubgraphs(
      ...
      [&](const ade::EdgeHandle & /*edge*/,
          const ade::SubgraphMergeDirection /*dir*/) { return true; },
      [&](const ade::subgraphs::NodesSet &acceptedNodes,
          const ade::subgraphs::NodesSet &rejectedNodes) {
        EXPECT_EQ(hasCycle(acceptedNodes, rejectedNodes),
                  cycleChecker(acceptedNodes, rejectedNodes));
  ...
  EXPECT_EQ(
      (node_set{nodes[0], nodes[2], nodes[4], nodes[3], nodes[1], nodes[6]}),
      node_set(subgraphs1[0].begin(), subgraphs1[0].end()));
...
TEST(Helpers, FindPaths1) {
...

  {
    ...
    EXPECT_TRUE((paths[0] == resPaths[0]) || (paths[0] == resPaths[1]));
  }

  {
    std::vector<std::vector<ade::NodeHandle>> paths;
    ade::findBiPaths(nodes[7], nodes[0],
                     [&](const std::vector<ade::NodeHandle> &path) {
                       paths.push_back(path);
                       return false;
                     });
...
TEST(Helpers, FindPaths2) {
  ...
  {
    std::vector<std::vector<ade::NodeHandle>> paths;
    ade::findBiPaths(nodes[8], nodes[0],
                     [&](const std::vector<ade::NodeHandle> &path) {
                       paths.push_back(path);
                       return false;
                     });
