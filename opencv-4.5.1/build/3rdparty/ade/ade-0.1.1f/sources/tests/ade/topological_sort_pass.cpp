...
template <typename T> static std::vector<NodeHandle> toVec(T range) {
...
static void checkOrder(const std::vector<NodeHandle> &nodes) {
  for (size_t i = 0; i < nodes.size(); ++i) {
    auto node = nodes[i];
    for (auto prev : util::map(node->inEdges(), [](const EdgeHandle &e) {
           return e->srcNode();
         })) {
...
TEST(TopologicalSortPass, TestNodeRemoval) {
  ...
  {
    const auto sorted =
        toVec(tgr.metadata().get<TopologicalSortData>().nodes());
