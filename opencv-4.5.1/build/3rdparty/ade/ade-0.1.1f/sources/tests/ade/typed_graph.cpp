...
const char *getMetadataName(ade::MetadataNameTag<Bar>) { return "Bar"; }
} // namespace Tst
} // namespace
...
TEST(TypedGraph, NotUniqueName) {
  ...
  EXPECT_THROW(TG gr(srcGr),
               std::logic_error); // Qux and Baz cannot be in one graph, because
                                  // have the same name
}
...
namespace {
template <typename T> void checkMeta3(T &gr) {
...
template <typename T> void checkMeta2(T &gr) {
  auto meta = gr.metadata();
  EXPECT_EQ(42, meta.template get<Tst::Foo>().val);
  EXPECT_EQ(77, meta.template get<Tst::Bar>().val);
}
