...
void Metadata::erase(const MetadataId &id) { m_data.erase(id); }

std::size_t Metadata::IdHash::operator()(const MetadataId &id) const {
  return std::hash<decltype(MetadataId::m_id)>()(id.m_id);
}

MetadataId::MetadataId(void *id) : m_id(id) { ADE_ASSERT(nullptr != m_id); }
...
bool MetadataId::isNull() const { return nullptr == m_id; }
