...
using CallbackCache =
    std::unordered_map<ade::NodeHandle, CallbackCacheEntry, NodeHasher>;
...
void visitProducer(Cache &cache, CallbackCache &callbackCache,
                   const ade::NodeHandle &commNode, const ade::NodeHandle &node,
  ...
  auto memDesc = findParentView(
      ctx.graph.metadata(node).get<ade::meta::DataObject>().dataRef.getView());
...
void visitConsumer(Cache &cache, CallbackCache &callbackCache,
                   const ade::NodeHandle &commNode, const ade::NodeHandle &node,
                   ade::passes::ConnectCommChannels::Context &ctx) {
  ADE_ASSERT(nullptr != node);
  ADE_ASSERT(ctx.graph.metadata(node).contains<ade::meta::DataObject>());
...
template <typename T> ade::ICommChannel::BufferDesc fillBufferDesc(T &elem) {
...
public:
  HostBufferImpl(std::size_t elementSize_, const ade::memory::DynMdSize &size_,
...
HostBufferImpl::HostBufferImpl(std::size_t elementSize_,
                               ...
                               const ade::memory::DynMdSize &alignment_)
    : m_size(size_), m_alignment(alignment_),
...
HostBufferImpl::HostBufferImpl(const ade::MemoryDescriptorRef &memRef_)
    : m_size(memRef_.span().size()), m_memRef(memRef_) {}

HostBufferImpl::~HostBufferImpl() { ADE_ASSERT(0 == m_accessCount); }

ade::IDataBuffer::MapId HostBufferImpl::map(const Span &span,
                                            Access /*access*/) {
...
void ade::passes::ConnectCommChannels::operator()(
    ade::passes::ConnectCommChannels::Context ctx) const {
  // Step 1:
  ...
  for (auto &elem : commObjects.dataObjects) {
    ...
    for (auto node : util::chain(util::toRange(elem.producers),
      ...
      ade::ICommChannel::BufferPrefs prefs =
          channel->getBufferPrefs(bufferDesc);
      ADE_ASSERT(prefs.preferredAlignment.dims_count() ==
                 summary.preferredAlignment.dims_count());
      for (auto i : util::iota(summary.preferredAlignment.dims_count())) {
        ...
        summary.preferredAlignment[i] = std::max(summary.preferredAlignment[i],
    ...
    for (auto node : util::chain(util::toRange(elem.producers),
      ...
      auto channel = meta.get<ade::meta::CommChannel>().channel;
      channel->setBuffer(
          ade::DataBufferView(buffer.get(), bufferDesc.memoryRef.span()),
          bufferDesc);
    }
  ...
  // Connect comm objects callbacks
  {
    ...
    std::unordered_map<ade::NodeHandle, std::vector<std::function<void()>>,
                       NodeHasher>
        producerCallbacks;
    for (auto &elem : commObjects.callbackObjects) {
      ...
      ade::CallbackConnector<> connector(
          util::checked_cast<int>(elem.producers.size()),
      ...
      for (auto &consumer : elem.consumers) {
        ...
        auto callback =
            std::move(meta.get<ade::meta::CommConsumerCallback>().callback);
      ...
      if (nullptr != resetter) {
        ...
        meta.get<ade::meta::Finalizers>().finalizers.emplace_back(
            std::move(resetter));
      }
    ...
    for (auto &elem : producerCallbacks) {
      ...
      if (1 == callbacks.size()) {
        ...
        meta.get<ade::meta::CommProducerCallback>().callback =
            Connector{std::move(callbacks)};
      }
    }
  }
}
