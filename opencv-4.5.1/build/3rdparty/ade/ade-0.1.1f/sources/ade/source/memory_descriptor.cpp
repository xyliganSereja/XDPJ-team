...
MemoryDescriptor::MemoryDescriptor(size_t element_size,
    ...
    : m_elementSize(element_size), m_dims(dims) {
  ADE_ASSERT(dims.dims_count() > 0);
  ADE_ASSERT(element_size > 0);
}
...
MemoryDescriptor::AccessHandle
MemoryDescriptor::access(const memory::DynMdSpan &span,
...
const memory::DynMdSize &MemoryDescriptor::dimensions() const { return m_dims; }

std::size_t MemoryDescriptor::elementSize() const { return m_elementSize; }

void MemoryDescriptor::setExternalView(const memory::DynMdView<void> &view) {
  ADE_ASSERT(view.elementSize() == m_elementSize);
  ADE_ASSERT(view.count() == m_dims.dims_count());
  m_externalView = view;
