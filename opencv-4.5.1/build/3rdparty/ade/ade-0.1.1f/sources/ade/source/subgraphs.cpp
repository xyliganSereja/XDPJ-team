...
std::vector<NodeHandle> assembleSubgraph(
    ...
    util::func_ref<bool(const EdgeHandle &, SubgraphMergeDirection)>
        mergeChecker,
  ...
  visitAdjacent(
      root, [&](const EdgeHandle &edge, SubgraphMergeDirection direction) {
        ...
        if (subgraph.nodeRejected(srcNode) || subgraph.nodeVisited(dstNode)) {
          return false;
        }
...
void findPaths(const NodeHandle &src, const NodeHandle &dst,
  ...
  visitPaths(pathVec, src,
             [&](const std::vector<NodeHandle> &path, const NodeHandle &prev,
...
void SubgraphSelfReferenceChecker::reset() { m_cache.clear(); }
