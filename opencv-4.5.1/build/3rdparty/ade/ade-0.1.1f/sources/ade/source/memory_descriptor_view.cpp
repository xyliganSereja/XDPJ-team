...
struct MemoryDescriptorView::Connector final {
  ...
  void addListener(MemoryDescriptorView *view,
                   IMemoryDescriptorViewListener *listener) {
  ...
  void removeListener(MemoryDescriptorView *view,
                      IMemoryDescriptorViewListener *listener) {
    ADE_ASSERT(nullptr != view);
    ADE_ASSERT(nullptr != listener);
    ADE_ASSERT(contains(view, listener));
  ...
  bool contains(const MemoryDescriptorView *view,
                const IMemoryDescriptorViewListener *listener) const {
    ADE_ASSERT(nullptr != view);
    ADE_ASSERT(nullptr != listener);
    auto it = util::find_if(listeners, OwnerComparator{view});
    if (listeners.end() == it) {
  ...
  ~Connector() { ADE_ASSERT(0 == listenersCount()); }
};
...
MemoryDescriptorView::MemoryDescriptorView(MemoryDescriptor &descriptor_,
    ...
    : m_parent(&descriptor_), m_span(span_), m_retargetable(retargetable_),
...
MemoryDescriptorView::MemoryDescriptorView(MemoryDescriptorView &parent_,
                                           const memory::DynMdSpan &span_)
    : m_parent_view(&parent_), m_span(span_),
...
MemoryDescriptorView::RetargetableState
MemoryDescriptorView::retargetableState() const {
...
void MemoryDescriptorView::addListener(
    IMemoryDescriptorViewListener *listener) {
  ADE_ASSERT(nullptr != listener);
  ADE_ASSERT(nullptr != m_connector);
...
void MemoryDescriptorView::removeListener(
    IMemoryDescriptorViewListener *listener) {
  ADE_ASSERT(nullptr != listener);
  ADE_ASSERT(nullptr != m_connector);
...
MemoryDescriptorView::AccessHandle
MemoryDescriptorView::access(const memory::DynMdSpan &span_,
