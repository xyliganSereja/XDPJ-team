...
Node::Node(Graph *parent) : m_parent(parent) {}
...
void Node::removeInEdge(Edge *edge) {
  ADE_ASSERT(nullptr != edge);
  // Nodes usually have only a small amount of connections so linear search
  // should be fine here
...
Graph *Node::getParent() const { return m_parent; }
