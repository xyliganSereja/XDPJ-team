...
MemoryDescriptorRef::MemoryDescriptorRef(MemoryDescriptorView &view_,
    ...
    : m_parent(&view_), m_span(span_) {}

MemoryDescriptorRef::~MemoryDescriptorRef() {}

MemoryDescriptorView *MemoryDescriptorRef::getView() { return m_parent; }
...
memory::DynMdSize MemoryDescriptorRef::size() const { return span().size(); }
...
std::ostream &operator<<(std::ostream &os, const MemoryDescriptorRef &ref) {
  if (nullptr == ref) {
    ...
    os << std::endl
       << "view: " << ref.getView() << " (" << ref.getView()->span() << "), ";
    os << std::endl
       << "descriptor: " << ref.getDescriptor() << " ("
       << ref.getDescriptor()->dimensions() << ") ";
    os << "}";
  }
  return os;
}
