...
void ExecutionEngine::addPrePassCallback(
    ExecutionEngine::PassCallback callback) {
  ADE_ASSERT(nullptr != callback);
  m_prePassCallbacks.callbacks.emplace_back(std::move(callback));
}

void ExecutionEngine::addPostPassCallback(
    ExecutionEngine::PassCallback callback) {
  ADE_ASSERT(nullptr != callback);
...
void ExecutionEngine::addBackend(std::unique_ptr<ExecutionBackend> &&backend) {
  ...
  ADE_ASSERT(m_backends.end() ==
             std::find(m_backends.begin(), m_backends.end(), backend));
...
struct GraphListenerSetter final {
  ...
  GraphListenerSetter(Graph &gr, IGraphListener *l) : graph(gr), listener(l) {
...
ExecutionEngine::createExecutable(const Graph &graph) {
...
struct ExecExceptionHandler {
  ...
  ExecExceptionHandler(std::vector<std::unique_ptr<BackendExecutable>> &execs)
      : handledVector(execs) {}
  ~ExecExceptionHandler() {
    ...
    for (auto i = util::checked_cast<int>(handledVector.size()) - 1; i >= count;
         i--) {
...
void ExecutableImpl::run() { run(nullptr); }
...
void ExecutableImpl::runAsync() { runAsync(nullptr); }
...
void ExecutableImpl::wait() {
  ...
  ExecExceptionHandler handler(execs);
  handler.passedCount =
      util::checked_cast<decltype(handler.passedCount)>(execs.size());
  mainExec->wait();
  for (auto &e : util::toRange(execs)) {
    handler.passedCount--;
    e->wait();
...
void ExecutionEngineSetupContext::addExecutableDependency(
    const std::string &lazyPassName) {
...
ExecutionEngineSetupContext::StagesRange
ExecutionEngineSetupContext::passStages() const {
  return m_engine.passStages();
}

void ExecutionEngine::CallbackList::call(
    const ExecutionEngine::PassDesc &desc,
...
bool ExecutionEngine::CallbackList::empty() const { return callbacks.empty(); }
...
ExecutionEngine::LazyPassWrapper *
ExecutionEngine::LazyPasses::getPass(const std::string &name) const {
...
bool ExecutionEngine::LazyPassWrapper::isValid() const { return m_valid; }
...
ExecutionEngine::LazyPassWrapper::LazyPassWrapper(
    ExecutionEngine::LazyPassWrapper *prev)
    : m_prev(prev) {}
