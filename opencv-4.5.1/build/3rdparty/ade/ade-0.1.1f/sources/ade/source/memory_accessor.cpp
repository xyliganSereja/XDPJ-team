...
void MemoryAccessor::removeListener(IMemoryAccessListener *listener) {
  ...
  auto it =
      std::find(m_accessListeners.begin(), m_accessListeners.end(), listener);
  ADE_ASSERT(m_accessListeners.end() != it);
  *it = m_accessListeners.back();
  m_accessListeners.pop_back();
}

MemoryAccessor::AccessHandle
MemoryAccessor::access(const MemoryDescriptor &desc,
                       const memory::DynMdSpan &span,
                       MemoryAccessType accessType) {
  ADE_ASSERT(nullptr != m_memory);
  return m_activeHandles.emplace(m_activeHandles.end(), this, desc, span,
                                 accessType);
}
