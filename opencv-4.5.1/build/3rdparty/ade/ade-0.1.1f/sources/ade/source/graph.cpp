...
EdgeHandle Graph::link(const EdgeHandle &src_edge, const NodeHandle &dst_node) {
  ...
  if (nullptr != m_listener) {
    m_listener->edgeAboutToBeRelinked(*this, src_edge, edge->srcNode(),
                                      dst_node);
  }
...
EdgeHandle Graph::link(const NodeHandle &src_node, const EdgeHandle &dst_edge) {
  ...
  ADE_ASSERT_STRONG(node != nullptr);
  if (nullptr != m_listener) {
    m_listener->edgeAboutToBeRelinked(*this, dst_edge, src_node,
                                      edge->dstNode());
  }
...
details::Metadata &Graph::metadata() { return geMetadataImpl(nullptr); }
...
void Graph::setListener(IGraphListener *listener) { m_listener = listener; }

IGraphListener *Graph::getListener() const { return m_listener; }
...
void Graph::removeNode(Node *node) {
  ...
  auto it = std::find_if(m_nodes.begin(), m_nodes.end(),
                         [node](const NodePtr &n) { return n.get() == node; });
...
void Graph::removeEdge(Edge *edge) {
  ...
  auto it = std::find_if(m_edges.begin(), m_edges.end(),
                         [edge](const EdgePtr &e) { return e.get() == edge; });
  // TODO: get rid of linear search
...
void Graph::ElemDeleter::operator()(Node *node) const { delete node; }

void Graph::ElemDeleter::operator()(Edge *edge) const { delete edge; }
