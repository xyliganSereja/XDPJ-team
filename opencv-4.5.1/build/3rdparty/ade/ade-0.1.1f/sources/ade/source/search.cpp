...
void dfsHelper(const NodeHandle &node, Visitor &&visitor,
               Traverse &&direction) {
  direction(*node, [&](const NodeHandle &nextNode) {
    ADE_ASSERT(nullptr != nextNode);
    if (visitor(nextNode)) {
      dfsHelper(nextNode, std::forward<Visitor>(visitor),
...
void TransitiveClosureHelperImpl(
    details::TransitiveClosureHelper::CacheT &cache, const NodeHandle &node,
    Traverse &&direction) {
  ADE_ASSERT(nullptr != node);
  ...
  direction(*node, [&](const NodeHandle &outNode) {
    ...
    if (!util::contains(cache, outNode)) {
      TransitiveClosureHelperImpl(cache, outNode,
                                  std::forward<Traverse>(direction));
    }
