...
void filterMatchesF(const std::vector<cv::DMatch> &matches,
                    const std::vector<cv::KeyPoint> keypoints_query,
                    const std::vector<cv::KeyPoint> keypoints_train,
                    const cv::Matx33d &F, std::vector<cv::DMatch> &result,
                    double threshold_px) {
  result.clear();
...
bool checkFmatrixSpectralProperty(const matrix3d &Fcv) {
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(F, Eigen::ComputeFullU |
                                               Eigen::ComputeFullV);
  Eigen::VectorXd s = svd.singularValues();

  std::cout << "checkFmatrixSpectralProperty: s: " << s.transpose()
            << std::endl;
...
// Essential matrix has to be of rank 2, and two non-zero singular values have
// to be equal. See Hartley & Zisserman, p.257
bool checkEmatrixSpectralProperty(const matrix3d &Fcv) {
  ...
  std::cout << "checkEmatrixSpectralProperty: s: " << s.transpose()
            << std::endl;
...
template <typename MAT> double matRMS(const MAT &a, const MAT &b) {
  MAT d = (a - b);
  d = d.mul(d);
...
TEST(SFM, EmatrixSimple) {
  ...
  std::srand(1);
  for (int i = 0; i < 8; ++i) {
    pts0.push_back({(double)(std::rand() % calib.width()),
                    (double)(std::rand() % calib.height())});
    pts1.push_back({(double)(std::rand() % calib.width()),
                    (double)(std::rand() % calib.height())});
  }
...
TEST(SFM, TriangulationSimple) {
  ...
  matrix3d R = {c, 0, s, 0, 1, 0, -s, 0, c};
  vector3d T = -R * O;
  matrix34d P1 = {R(0, 0), R(0, 1), R(0, 2), T[0],    R(1, 0), R(1, 1),
...
TEST(SFM, FmatrixMatchFiltering) {
  ...
  double threshold_px = 3;
  {
    ...
    filterMatchesF(good_matches, keypoints1, keypoints2, F, good_matches_f,
                   threshold_px);
  }
  {
    std::vector<cv::Vec2d> points1, points2;
    ...
    matrix3d F = phg::findFMatrix(points1, points2, threshold_px);
    filterMatchesF(good_matches_gms, keypoints1, keypoints2, F,
                   good_matches_gms_plus_f, threshold_px);
  }

  drawMatches(img1, img2, keypoints1, keypoints2, good_matches_gms,
              "data/debug/test_sfm/matches_GMS.jpg");
  drawMatches(img1, img2, keypoints1, keypoints2, good_matches_f,
              "data/debug/test_sfm/matches_F.jpg");
  drawMatches(img1, img2, keypoints1, keypoints2, good_matches_gms_plus_f,
              "data/debug/test_sfm/matches_GMS_plus_F.jpg");

  std::cout << "n matches gms: " << good_matches_gms.size() << std::endl;
  std::cout << "n matches F: " << good_matches_f.size() << std::endl;
  std::cout << "n matches gms + F: " << good_matches_gms_plus_f.size()
            << std::endl;
...
TEST(SFM, RelativePosition2View) {
  ...
  for (int i = 0; i < (int)good_matches_gms.size(); ++i) {
    vector3d ms[2] = {calib0.unproject(points1[i]),
                      calib1.unproject(points2[i])};
    ...
    point_cloud_colors.push_back(
        img1.at<cv::Vec3b>(points1[i][1], points1[i][0]));
  }
  ...
  phg::exportPointCloud(point_cloud,
                        "data/debug/test_sfm/point_cloud_2_cameras.ply",
                        point_cloud_colors);
}
...
TEST(SFM, ReconstructNViews) {
  ...
  for (int i = 0; i < n_imgs; ++i) {
    ...
    for (int j = 0; j < n_imgs; ++j) {
      ...
      std::cout << "filtering matches GMS..." << std::endl;
      std::vector<DMatch> good_matches_gms;
      phg::filterMatchesGMS(good_matches, keypoints[i], keypoints[j],
                            imgs[i].size(), imgs[j].size(), good_matches_gms);
  ...
  phg::exportPointCloud(tie_points,
                        "data/debug/test_sfm/point_cloud_N_cameras.ply",
                        tie_points_colors);
}
